@php($logsSrc ??= route('webhook.logs.index'))
            ...
            >
                <template #body="{ columns, records, performAction }">
                    ...
                    >
                        ...
                        <p v-html="record.webhook_id"></p>

                        <p v-text="record.sku || '—'"></p>

                        <p v-text="record.event || '—'"></p>
                            ...
                            <div class="w-full max-h-[96%] overflow-y-auto z-[999] absolute ltr:left-1/2 rtl:right-1/2 top-1/2 rounded-lg bg-white dark:bg-gray-900 box-shadow max-md:w-[90%] ltr:-translate-x-1/2 rtl:translate-x-1/2 -translate-y-1/2 max-w-2xl">
                                ...
                                <div v-else-if="log" class="px-4 py-3 border-b dark:border-gray-800">
                                    <div class="grid grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-300 mb-4">
                                        <div>
                                            <span class="font-semibold block">@lang('webhook::app.configuration.webhook.logs.index.datagrid.webhook')</span>
                                            <span v-text="log.webhook || '—'"></span>
                                        </div>

                                        <div>
                                            <span class="font-semibold block">@lang('webhook::app.configuration.webhook.logs.index.datagrid.event')</span>
                                            <span v-text="log.event || '—'"></span>
                                        </div>
                    ...
                    this.$axios.get(url)
                        ...
                        .catch(error => {
                            ...
                            this.$emitter.emit('add-flash', {
                                type: 'error',
                                message: error.response?.data?.message ?? "@lang('webhook::app.configuration.webhook.logs.index.load-failed')",
                            });
                            this.closeLog();
                        });
                },
