...
it('persists the debug row to its original core-config key (no code relocation)', function () {
    // Saving from the hub writes the existing config('core') codes, never a relocated system.debug.*.
    ...
    // getConfigData memoises per request; the PUT cached a pre-write miss, so drop it before re-reading.
