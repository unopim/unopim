...
it('feeds the saved value back into the multi-select option attribute filter', function () use ($views) {
    $filters = file_get_contents($views.'/components/datagrid/filters.blade.php');

    expect($filters)
        ->toContain('multiple="true"')
        ->and($filters)->toContain(':value="attributeOptionValue(column)"')
        ->and($filters)->toContain('@input="setAttributeOptionValue(column, $event)"');
});
