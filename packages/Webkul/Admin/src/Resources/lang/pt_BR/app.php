<?php

return [
    'users' => [
        'sessions' => [
            'subtitle'                   => 'Entre na sua conta para continuar',
            'tagline'                    => 'Gerencie todas as informações dos seus produtos em um só lugar',
            'remember-me'                => 'Lembrar de mim',
            'toggle-password'            => 'Mostrar ou ocultar a senha',
            'too-many-attempts'          => 'Muitas tentativas de login. Tente novamente em instantes.',
            'email'                      => 'Endereço de e-mail',
            'forget-password-link'       => 'Esqueceu a senha?',
            'password'                   => 'Senha',
            'sso-divider'                => 'or',
            'sso-sign-in-with-microsoft' => 'Sign in with Microsoft',
            'submit-btn'                 => 'Entrar',
            'title'                      => 'Entrar',
            'powered-by'                 => 'Desenvolvido por :unopim,',
            'open-source-project-by'     => 'Um projeto de código aberto de :webkul.',
        ],
        'forget-password' => [
            'create' => [
                'subtitle'               => 'Digite seu e-mail e enviaremos um link para redefinir a senha.',
                'email'                  => 'E-mail registrado',
                'email-not-exist'        => 'E-mail não encontrado',
                'page-title'             => 'Esqueceu a senha',
                'reset-link-sent'        => 'Se existir uma conta com esse e-mail, um link para redefinição de senha foi enviado.',
                'email-settings-error'   => 'O e-mail não pode ser enviado. Verifique as configurações do seu e-mail',
                'sign-in-link'           => 'Voltar para a tela de login?',
                'submit-btn'             => 'Redefinir',
                'title'                  => 'Recuperar senha',
                'powered-by'             => 'Desenvolvido por :unopim,',
                'open-source-project-by' => 'Um projeto de código aberto de :webkul.',
            ],
        ],
        'reset-password' => [
            'subtitle'               => 'Escolha uma nova senha para sua conta.',
            'invalid-link'           => 'Este link de redefinição de senha é inválido ou expirou.',
            'back-link-title'        => 'Voltar para a tela de login?',
            'confirm-password'       => 'Confirmar senha',
            'email'                  => 'E-mail registrado',
            'password'               => 'Senha',
            'submit-btn'             => 'Redefinir senha',
            'title'                  => 'Redefinir senha',
            'powered-by'             => 'Desenvolvido por :unopim,',
            'open-source-project-by' => 'Um projeto de código aberto de :webkul.',
        ],
    ],
    'notifications' => [
        'description-text' => 'Lista de todas as notificações',
        'marked-success'   => 'Notificação marcada como lida',
        'no-record'        => 'Nenhum registro encontrado',
        'of'               => 'de',
        'read-all'         => 'Marcar como lida',
        'showing'          => 'Exibindo',
        'title'            => 'Notificações',
        'view-all'         => 'Visualizar todas',
        'status'           => [
            'all'        => 'Todas',
            'canceled'   => 'Cancelado',
            'closed'     => 'Fechado',
            'completed'  => 'Completado',
            'pending'    => 'Pendente',
            'processing' => 'Processando',
            'read'       => 'Lidas',
            'unread'     => 'Não lidas',
        ],
    ],
    'account' => [
        'edit' => [
            'back-btn'                 => 'Voltar',
            'change-password'          => 'Alterar senha',
            'confirm-password'         => 'Confirmar senha',
            'current-password'         => 'Senha atual',
            'current-password-info'    => 'Necessária apenas quando você define uma nova senha. Confirme a alteração com a senha da conta com a qual você está conectado.',
            'email'                    => 'E-mail',
            'general'                  => 'Geral',
            'invalid-password'         => 'A senha atual está incorreta.',
            'name'                     => 'Nome',
            'password-whitespace'      => 'A senha não pode consistir apenas em espaços.',
            'password'                 => 'Senha',
            'gravatar-preview'         => 'Usando sua imagem do Gravatar.com',
            'use-gravatar'             => 'Usar imagem do Gravatar',
            'use-gravatar-info'        => 'Exibida quando nenhuma imagem de perfil é enviada. Crie ou altere em :link.',
            'profile-image'            => 'Imagem de perfil',
            'save-btn'                 => 'Salvar conta',
            'title'                    => 'Minha conta',
            'ui-locale'                => 'Localização do UI',
            'update-success'           => 'Conta atualizada com sucesso',
            'upload-image-info'        => 'Envie uma imagem de perfil (110px X 110px), ou use sua imagem do Gravatar.com vinculada ao seu e-mail.',
            'user-timezone'            => 'Fuso horário',
            'catalog-locale'           => 'Idioma do catálogo',
            'catalog-locale-info'      => 'O idioma em que o conteúdo do seu catálogo é escrito. É separado do idioma da interface acima.',
            'default-channel'          => 'Canal padrão',
        ],
    ],
    'dashboard' => [
        'invalid-type' => 'Tipo de widget do painel inválido.',
        'index'        => [
            'title'                => 'Painel de Controle',
            'user-info'            => 'Monitoramento rápido do que é importante no seu PIM',
            'user-name'            => 'Olá! :user_name',
            'catalog-details'      => 'Catálogo',
            'total-families'       => 'Total de famílias',
            'total-attributes'     => 'Total de atributos',
            'total-groups'         => 'Total de grupos de atributos',
            'total-categories'     => 'Total de categorias',
            'total-products'       => 'Total de produtos',
            'settings-details'     => 'Estrutura do catálogo',
            'total-locales'        => 'Total de locais',
            'total-currencies'     => 'Total de moedas',
            'total-channels'       => 'Total de canais',
            'completeness'         => 'Completude',
            'active'               => 'Ativo',
            'inactive'             => 'Inativo',
            'simple'               => 'Simples',
            'configurable'         => 'Configurável',
            'product-stats'        => 'Estatísticas de Produtos',
            'product-type-dist'    => 'Distribuição por Tipo',
            'products-by-status'   => 'Produtos por Status',
            'products-over-time'   => 'Atividade de Produtos (Últimos 7 dias)',
            'legend-created'       => 'Criados',
            'legend-updated'       => 'Atualizados',
            'recent-activity'      => 'Atividade Recente',
            'no-activity'          => 'Nenhuma atividade recente encontrada.',
            'updated'              => 'atualizado',
            'created'              => 'criado',
            'deleted'              => 'excluído',
            'ago'                  => 'atrás',
            'just-now'             => 'agora mesmo',
            'data-transfer'        => 'Transferência de Dados',
            'no-jobs'              => 'Nenhum job de importação/exportação recente.',
            'job-type-import'      => 'Importação',
            'job-type-export'      => 'Exportação',
            'job-state-completed'  => 'Concluído',
            'job-state-failed'     => 'Falhou',
            'job-state-processing' => 'Processando',
            'job-state-pending'    => 'Pendente',
            'job-state-validated'  => 'Validado',
            'rows-processed'       => 'Linhas Processadas',
            'errors'               => 'Erros',
            'quick-links'          => 'Links Rápidos',
            'create-product'       => 'Criar Produto',
            'import-data'          => 'Importar Dados',
            'export-data'          => 'Exportar Dados',
            'manage-attributes'    => 'Gerenciar Atributos',
            'view-all-activity'    => 'Ver Tudo',
            'view-all-jobs'        => 'Ver Todos os Jobs',
            'new-this-week'        => 'Novos Esta Semana',
            'with-variants'        => 'Com Variantes',
            'avg-completeness'     => 'Completude Média',
            'enriched'             => 'Enriquecidos',
            'analytics-section'    => 'Análises',
            'operations-section'   => 'Operações',
            'needs-attention'      => 'Precisa de Atenção',
            'unenriched-products'  => 'Produtos Não Enriquecidos',
            'failed-jobs-24h'      => 'Jobs com Falha (24h)',
            'low-completeness'     => 'Baixa Completude',
            'all-good'             => 'Tudo parece bem!',
            'channel-readiness'    => 'Prontidão do Canal',
            'ready-to-publish'     => 'pronto para publicar',
            'of'                   => 'de',
            'products-ready'       => 'produtos prontos',
            'no-readiness-data'    => 'Nenhum dado de completude disponível.',
            'no-products'          => 'Ainda não há produtos.',
            'last-7-days'          => 'Últimos 7 dias',

            'days' => [
                'sun' => 'Dom',
                'mon' => 'Seg',
                'tue' => 'Ter',
                'wed' => 'Qua',
                'thu' => 'Qui',
                'fri' => 'Sex',
                'sat' => 'Sáb',
            ],
            'entity-types'         => [
                'product'          => 'Produto',
                'category'         => 'Categoria',
                'attribute'        => 'Atributo',
                'attribute-family' => 'Família',
                'attribute-group'  => 'Grupo',
                'category-field'   => 'Campo de categoria',
                'channel'          => 'Canal',
                'role'             => 'Função',
                'job-instance'     => 'Tarefa',
                'webhook'          => 'Webhook',
                'api-key'          => 'Chave API',
            ],
        ],
    ],
    'catalog' => [
        'products' => [
            'upload-failure' => 'Não foi possível encontrar a mídia.',
            'index'          => [
                'already-taken'             => ':name já está em uso.',
                'variant-sku-already-taken' => 'O SKU da variante ":sku" já está em uso.',
                'variant-sku-label'         => 'SKU da variante :position',
                'create-btn'                => 'Criar produto',
                'title'                     => 'Produtos',
                'magic-ai-error'            => 'Verifique as credenciais do Magic AI e certifique-se de que o modelo correto está selecionado.',
                'magic-ai-validate-error'   => 'Por favor, verifique as credenciais do Magic AI.',
                'magic-ai-validate-success' => 'Credenciais do Magic AI validadas com sucesso.',
                'create'                    => [
                    'back-btn'                          => 'Voltar',
                    'configurable-attributes'           => 'Atributos configuráveis',
                    'create-btn'                        => 'Criar produto',
                    'family'                            => 'Família',
                    'next-btn'                          => 'Próximo',
                    'save-btn'                          => 'Salvar produto',
                    'sku'                               => 'SKU',
                    'title'                             => 'Criar novo produto',
                    'type'                              => 'Tipo',
                    'variant-structure'                 => 'Estrutura de variantes',
                    'not-config-family-error'           => 'Esta família não possui atributos configuráveis',
                    'no-variant-structure'              => 'Esta família não possui uma estrutura de variantes. Crie uma antes de adicionar um produto configurável.',
                    'invalid-variant-structure'         => 'A estrutura de variantes selecionada não pertence a esta família.',
                    'not-config-super-attributes-error' => 'O super atributo ":super_attributes" não foi encontrado',
                    'sku-exists-error'                  => 'O SKU ":sku" já está em uso',
                ],
                'datagrid' => [
                    'categories'              => 'Categorias',
                    'active'                  => 'Ativo',
                    'attribute-family'        => 'Família de Atributos',
                    'attribute-family-value'  => 'Família de Atributos - :attribute_family',
                    'category'                => 'Categoria',
                    'copy-of'                 => 'Cópia de :value',
                    'copy-of-slug'            => 'copia-:value',
                    'delete'                  => 'Excluir',
                    'edit'                    => 'Editar',
                    'copy'                    => 'Copiar',
                    'disable'                 => 'Desativar',
                    'id'                      => 'ID',
                    'id-value'                => 'ID - :id',
                    'image'                   => 'Imagem',
                    'mass-delete-queued'      => 'Os produtos selecionados estão sendo excluídos em segundo plano',
                    'mass-delete-success'     => 'Os produtos selecionados foram excluídos com sucesso',
                    'mass-update-queued'      => 'Os produtos selecionados estão sendo atualizados em segundo plano',
                    'mass-update-success'     => 'Os produtos selecionados foram atualizados com sucesso',
                    'name'                    => 'Nome',
                    'out-of-stock'            => 'Fora de estoque',
                    'price'                   => 'Preço',
                    'product-image'           => 'Imagem do produto',
                    'qty'                     => 'Quantidade',
                    'qty-value'               => ':qty em estoque',
                    'sku'                     => 'SKU',
                    'sku-value'               => 'SKU - :sku',
                    'status'                  => 'Status',
                    'type'                    => 'Tipo',
                    'update-status'           => 'Atualizar status',
                    'parent'                  => 'Pai',
                    'parent-identifier'       => 'Pai: :sku',
                    'created-at'              => 'Criado em',
                    'updated-at'              => 'Atualizado em',
                ],
            ],
            'edit' => [
                'navigation' => 'Navegação do produto',
                'preview'    => 'Visualizar',
                'remove'     => 'Remover',
                'save-btn'   => 'Salvar produto',
                'title'      => 'Editar produto',
                'categories' => [
                    'title'            => 'Categorias',
                    'root-not-allowed' => 'A categoria raiz não pode ser atribuída a um produto.',
                ],

                'attribute-groups' => [
                    'label'              => 'Grupo de atributos:',
                    'title'              => 'Grupos de atributos',
                    'search-placeholder' => 'Pesquisar grupos',
                    'empty'              => 'Nenhum grupo de atributos corresponde à sua pesquisa.',
                    'attributes-count'   => ':count atributos',
                    'loading'            => 'Carregando...',
                    'retry'              => 'Tentar novamente',
                    'load-more'          => 'Carregar mais',
                    'unsaved-title'      => 'Alterações não salvas',
                    'unsaved-message'    => 'Salve suas alterações em :group antes de continuar, ou descarte-as.',
                    'save-and-continue'  => 'Salvar e continuar',
                    'discard-and-leave'  => 'Descartar e continuar',
                    'cancel'             => 'Cancelar',
                ],
                'images' => [
                    'info'  => 'As imagens devem ter a resolução de 560px X 609px',
                    'title' => 'Imagens',
                ],
                'videos' => [
                    'error' => ':attribute não pode ser maior que :max kilobytes. Selecione um arquivo menor.',
                    'info'  => 'O tamanho máximo do vídeo é :size',
                    'title' => 'Vídeos',
                ],
                'links' => [
                    'related-products' => [
                        'empty-info' => 'Adicione produtos relacionados.',
                        'info'       => 'Além do produto exibido, os produtos relacionados também serão mostrados.',
                        'title'      => 'Produtos Relacionados',
                    ],
                    'up-sells' => [
                        'empty-info' => 'Adicione produtos de venda adicional.',
                        'info'       => 'Os clientes verão produtos premium ou alternativas de maior qualidade do que o produto exibido.',
                        'title'      => 'Produtos de venda adicional',
                    ],
                    'cross-sells' => [
                        'empty-info' => 'Adicione produtos de venda cruzada.',
                        'info'       => 'Além do produto no carrinho, os clientes verão outros produtos que podem comprar.',
                        'title'      => 'Produtos de venda cruzada',
                    ],
                    'add-btn'           => 'Adicionar',
                    'add-type-btn'      => 'Adicionar tipo de associação',
                    'delete'            => 'Excluir',
                    'empty-info'        => 'Adicione produtos :type de uma vez.',
                    'empty-title'       => 'Adicionar produto',
                    'image-placeholder' => 'Imagem do produto',
                    'more-types'        => 'Mais',
                    'no-types-info'     => 'Use “Adicionar tipo de associação” para vincular este produto a um tipo de associação.',
                    'no-types-title'    => 'Nenhuma associação adicionada',
                    'remove-product'    => 'Remover produto',
                    'search-types'      => 'Pesquisar tipos de associação',
                    'sku'               => 'SKU - :sku',
                    'title'             => 'Produtos',
                ],
                'types' => [
                    'configurable' => [
                        'add-btn'                            => 'Adicionar variação',
                        'default'                            => 'Padrão',
                        'delete-btn'                         => 'Excluir',
                        'edit-btn'                           => 'Editar',
                        'empty-info'                         => 'Crie um produto com várias combinações',
                        'empty-title'                        => 'Adicionar variação',
                        'image-placeholder'                  => 'Imagem do produto',
                        'info'                               => 'Produtos configuráveis são baseados em todas as combinações possíveis',
                        'qty'                                => ':qty Quantidade',
                        'sku'                                => 'SKU - :sku',
                        'title'                              => 'Variação',
                        'variant-exists'                     => 'Esta variação ou SKU já existe.',
                        'variant-attribute-option-not-found' => 'Opção de atributo de variação não encontrada :attributes',
                        'supper-attribute-not-found'         => 'Super atributo não encontrado :attribute',
                        'variant-given-exists'               => 'Esta variação já existe :variants',
                        'variant-group-has-children'         => 'Este grupo de variações ainda possui variações. Remova-as ou reatribua-as a outro grupo antes de excluí-lo.',
                        'variant-group-combination-exists'   => 'Já existe um grupo de variações com :values neste produto. Escolha uma combinação diferente.',
                        'variant-combination-exists'         => 'Já existe uma variação com :values neste produto. Escolha uma combinação diferente.',
                        'create'                             => [
                            'description'            => 'Descrição',
                            'name'                   => 'Nome',
                            'save-btn'               => 'Adicionar',
                            'title'                  => 'Adicionar variação',
                            'variant-already-exists' => 'Esta variação já existe.',
                        ],
                        'edit' => [
                            'disabled'        => 'Desativado',
                            'edit-info'       => 'Para atualizar as informações detalhadas do produto, vá para a página',
                            'edit-link-title' => 'Página do produto',
                            'enabled'         => 'Ativado',
                            'images'          => 'Imagens',
                            'name'            => 'Nome',
                            'price'           => 'Preço',
                            'quantities'      => 'Quantidades',
                            'save-btn'        => 'Salvar',
                            'sku'             => 'SKU',
                            'status'          => 'Status',
                            'title'           => 'Produto',
                            'weight'          => 'Peso',
                        ],
                        'mass-edit' => [
                            'add-images'          => 'Adicionar imagens',
                            'apply-to-all-btn'    => 'Aplicar para todos',
                            'apply-to-all-name'   => 'Aplicar nome para todas as variações',
                            'apply-to-all-sku'    => 'Aplicar SKU para todas as variações',
                            'apply-to-all-status' => 'Aplicar status para todas as variações',
                            'apply-to-all-weight' => 'Aplicar peso para todas as variações',
                            'edit-inventories'    => 'Editar inventários',
                            'edit-names'          => 'Editar nomes',
                            'edit-prices'         => 'Editar preços',
                            'edit-sku'            => 'Editar SKU',
                            'edit-status'         => 'Editar status',
                            'edit-weight'         => 'Editar peso',
                            'name'                => 'Nome',
                            'price'               => 'Preço',
                            'remove-images'       => 'Remover imagens',
                            'remove-variants'     => 'Remover variações',
                            'select-action'       => 'Selecione a ação',
                            'select-variants'     => 'Selecione as variações',
                            'status'              => 'Status',
                            'variant-name'        => 'Nome da variação',
                            'variant-sku'         => 'SKU da variação',
                            'weight'              => 'Peso',
                        ],
                        'variant-inheritance' => [
                            'locked'              => 'Este atributo pode ser atualizado nos atributos :level.',
                            'level-common'        => 'comuns',
                            'level-sub_parent'    => 'subpai',
                            'level-variant'       => 'variante',
                        ],
                    ],
                ],
                'translate' => [
                    'translate-btn'               => 'Traduzir',
                    'title'                       => 'Traduzir',
                    'source-channel'              => 'Canal de Origem',
                    'source-locale'               => 'Idioma de origem',
                    'locale'                      => 'Local de Origem',
                    'translated-content'          => 'Conteúdo Traduzido',
                    'translating'                 => 'Traduzindo',
                    'apply'                       => 'Aplicar',
                    'back'                        => 'Voltar',
                    'cancel'                      => 'Cancelar',
                    'target-locales'              => 'Locais de Destino',
                    'target-channel'              => 'Canal de Destino',
                    'attributes'                  => 'Atributos',
                    'tranlated-job-processed'     => 'Tarefa de armazenamento de conteúdo traduzido processada.',
                    'step'                        => 'Etapa',
                    'select-source'               => 'Selecionar origem',
                    'select-target'               => 'Selecionar destino',
                    'first-step-title'            => 'Etapa 1: Selecionar canal de origem, idioma e atributos',
                    'first-step-title-attributes' => 'Etapa 1: Selecionar canal de origem e idioma',
                    'source-content'              => 'Conteúdo de Origem',
                    'second-step-title'           => 'Etapa 2: Selecionar canal de destino e idiomas',
                    'target-content'              => 'Conteúdo de destino',
                    'next'                        => 'Próximo',
                    'empty-translation-data'      => 'Dados não disponíveis para tradução com base no canal e idioma de origem',
                    'attributes-to-translate'     => 'Atributos a traduzir',
                    'no-value'                    => 'Sem valor',
                    'change'                      => 'Alterar',
                    'overwrite-warning'           => 'O conteúdo existente nos idiomas de destino selecionados será substituído.',
                    'incomplete-source'           => 'Selecione um canal de origem, um idioma e pelo menos um atributo para continuar.',
                ],
                'product-info' => [
                    'title'        => 'Informações do Produto',
                    'status'       => 'Status',
                    'family'       => 'Família',
                    'parent'       => 'Produto Pai',
                    'updated-at'   => 'Última Atualização',
                    'created-at'   => 'Data de Criação',
                    'product-type' => 'Tipo de Produto',
                ],
                'sections' => [
                    'view'    => 'Visualizar',
                    'unsaved' => 'Alterações não salvas',
                ],

                'workspace' => [

                    'add-selected' => 'Adicionar selecionados',
                    'close'        => 'Fechar',
                    'clear-search' => 'Limpar pesquisa',
                    'categories'   => [
                        'subtitle'        => 'Atribua este produto a categorias do catálogo.',
                        'search'          => 'Pesquisar categorias…',
                        'searching'       => 'Pesquisando…',
                        'no-results'      => 'Nenhuma categoria correspondente encontrada',
                        'load-more'       => 'Carregar mais',
                        'none-selected'   => 'Nenhuma categoria selecionada ainda',
                        'and-more'        => 'e mais :count',
                        'review-selected' => 'Mostrar categorias selecionadas',
                        'selected'        => 'selecionado(s)',
                    ],
                    'associations' => [
                        'subtitle' => 'Vincule produtos relacionados, de upsell, cross-sell e personalizados.',
                        'linked'   => 'produtos vinculados',
                    ],
                ],

                'more'         => 'Mais',
                'more-actions' => 'Mais ações',
                'variations'   => [
                    'title'          => 'Variações',
                    'common'         => 'Comum',
                    'search'         => 'Pesquisar',
                    'switching'      => 'Alternando...',
                    'variant-count'  => ':count variante',
                    'variants-count' => ':count variantes',
                    'complete-count' => ':done de :total completos',
                    'select'         => 'Selecionar :attribute',
                    'add-new-btn'    => 'Adicionar novo',
                    'add-new'        => 'Adicionar um novo :attribute',
                    'empty'          => 'Ainda não há variantes de :attribute.',
                    'loading'        => 'Carregando...',
                    'loading-more'   => 'Carregando mais...',
                    'sku'            => 'SKU',
                    'cancel'         => 'Cancelar',
                    'create'         => 'Criar',
                    'creating'       => 'Criando...',
                    'create-error'   => 'Não foi possível criar a variante',
                ],
            ],
            'bulk-edit' => [
                'validation' => [
                    'decimal'                    => 'Por favor, insira um número decimal válido.',
                    'number'                     => 'Por favor, insira um número inteiro válido.',
                    'email'                      => 'Por favor, insira um endereço de e-mail válido.',
                    'url'                        => 'Por favor, insira uma URL válida.',
                    'regex'                      => 'O valor não corresponde ao padrão exigido.',
                    'invalid-pattern'            => 'Padrão personalizado inválido fornecido.',
                    'numeric'                    => 'O valor do atributo numérico ":attribute" deve ser um número válido.',
                    'select-attribute-or-family' => 'Selecione pelo menos um atributo ou uma família de atributos.',
                    'duplicate-axis'             => 'Duas variantes do mesmo produto pai não podem compartilhar uma combinação de eixos, portanto este salvamento será rejeitado.',
                    'failed'                     => 'A validação falhou.',
                ],
                'modal' => [
                    'title'            => 'Atributos para edição em massa',
                    'btn-cancel'       => 'Cancelar',
                    'btn-proceed'      => 'Prosseguir',
                    'attribute-family' => 'Selecionar família de atributos',
                ],
                'index' => [
                    'no-product' => 'Nenhum produto selecionado para edição em massa.',
                ],
                'filter' => [
                    'many-product'     => 'Produtos demais selecionados.',
                    'redirect-message' => 'Redirecionando para a visualização de edição em massa.',
                ],
                'handle-save' => [
                    'edit-success' => 'Edição em massa realizada com sucesso.',
                ],
                'id'                           => 'ID',
                'no-changes'                   => 'Nenhuma alteração para salvar.',
                'locked-cell'                  => 'Pertence a outro nível – não editável aqui.',
                'invalid-datetime'             => 'Informe uma data e hora válidas.',
                'resize-column'                => 'Arraste para redimensionar a coluna',
                'success'                      => 'A tarefa foi executada com sucesso.',
                'fetch-failed'                 => 'Falha ao buscar.',
                'action'                       => 'Edição em massa',
                'description'                  => 'Edite vários produtos de uma vez. As alterações são processadas em segundo plano.',
                'gallery-preview'              => 'Pré-visualização da Galeria',
                'img-preview'                  => 'Pré-visualização da Imagem',
                'no-image'                     => 'Sem Imagem',
                'img-fail'                     => 'Falha no envio da imagem',
                'no-option'                    => 'Sem opções',
                'clear-selection'              => 'Limpar seleção',
                'keyboard-shortcuts'           => 'Atalhos de teclado',
                'shortcuts-navigation'         => 'Navegação',
                'shortcuts-editing'            => 'Edição',
                'shortcuts-selection'          => 'Seleção',
                'shortcuts-clipboard'          => 'Área de transferência e preenchimento',
                'shortcuts-move-cell'          => 'Mover entre células',
                'shortcuts-move-down'          => 'Mover para baixo / confirmar edição',
                'shortcuts-move-up'            => 'Mover para cima',
                'shortcuts-move-right-left'    => 'Mover para a direita / esquerda',
                'shortcuts-home-end'           => 'Primeira / última coluna na linha',
                'shortcuts-ctrl-home-end'      => 'Primeira / última célula na grade',
                'shortcuts-extend-selection'   => 'Ampliar seleção',
                'shortcuts-select-all'         => 'Selecionar todas as células',
                'shortcuts-enter-edit'         => 'Entrar no modo de edição',
                'shortcuts-confirm-move-down'  => 'Confirmar + mover para baixo',
                'shortcuts-confirm-move-right' => 'Confirmar + mover para a direita',
                'shortcuts-escape-revert'      => 'Reverter valor + sair da edição',
                'shortcuts-clear-cell'         => 'Limpar célula',
                'shortcuts-copy'               => 'Copiar',
                'shortcuts-cut'                => 'Recortar',
                'shortcuts-paste'              => 'Colar',
                'shortcuts-fill-down'          => 'Preencher para baixo',
                'shortcuts-fill-right'         => 'Preencher para a direita',
                'shortcuts-undo'               => 'Desfazer',
                'shortcuts-redo'               => 'Refazer',
                'shortcuts-help'               => 'Mostrar/ocultar atalhos de teclado',
            ],
            'create-success'                  => 'Produto criado com sucesso',
            'delete-failed'                   => 'Falha ao excluir o produto',
            'delete-success'                  => 'Produto excluído com sucesso',
            'product-copied'                  => 'Produto copiado com sucesso',
            'saved-inventory-message'         => 'Produto salvo com sucesso',
            'update-success'                  => 'Produto atualizado com sucesso',
            'unique-validation'               => 'Este valor já está em uso.',
            'invalid-type'                    => 'Tipo de produto ":type" não encontrado',
            'product-not-found'               => 'Produto com SKU ":sku" não encontrado',
            'parent-not-found'                => 'Pai com SKU ":sku" não encontrado',
            'upload-success'                  => 'Arquivo do produto carregado com sucesso.',
            'update-failure'                  => 'Produto não pôde ser atualizado.',
            'immutable-fields'                => 'Os seguintes campos não podem ser modificados: :fields.',
            'variant-structure-not-found'     => 'A estrutura de variantes com o código ":code" não foi encontrada na família ":family"',
            'variant-structure-axis-mismatch' => 'Os superatributos informados não correspondem aos eixos da estrutura de variantes ":code". Esperado: :axes',
        ],
        'attributes' => [
            'index' => [
                'create-btn' => 'Criar atributo',
                'title'      => 'Atributos',
                'datagrid'   => [
                    'channel-based'            => 'Baseado no canal',
                    'code'                     => 'Código',
                    'created-at'               => 'Criado em',
                    'delete'                   => 'Excluir',
                    'edit'                     => 'Editar',
                    'id'                       => 'ID',
                    'locale-based'             => 'Baseado na localidade',
                    'mass-delete-success'      => 'Atributos selecionados excluídos com sucesso',
                    'name'                     => 'Nome',
                    'required'                 => 'Obrigatório',
                    'type'                     => 'Tipo',
                    'unique'                   => 'Único',
                    'delete-failed'            => 'Não foi possível excluir este atributo.',
                    'mass-delete-failed'       => 'Não foi possível excluir os atributos selecionados.',
                    'mass-delete-partial'      => 'Não foi possível excluir :count atributo(s) porque estão em uso em produtos configuráveis.',
                    'delete-attribute-failure' => 'Este atributo não pode ser removido porque está em uso para produtos configuráveis',
                ],
            ],
            'create' => [
                'add-attribute-options'  => 'Adicionar opções de atributo',
                'add-option'             => 'Adicionar opção',
                'add-options-info'       => 'Crie combinações de opções para o atributo.',
                'add-row'                => 'Adicionar linha',
                'admin'                  => 'Administrador',
                'admin-name'             => 'Nome do Administrador',
                'back-btn'               => 'Voltar',
                'boolean'                => 'Booleano',
                'checkbox'               => 'Caixa de seleção',
                'code'                   => 'Código',
                'color'                  => 'Cor',
                'configuration'          => 'Configuração',
                'create-empty-option'    => 'Criar opção padrão vazia',
                'date'                   => 'Data',
                'datetime'               => 'Data e hora',
                'decimal'                => 'Decimal',
                'default-value'          => 'Valor padrão',
                'email'                  => 'E-mail',
                'enable-wysiwyg'         => 'Ativar editor WYSIWYG',
                'file'                   => 'Arquivo',
                'general'                => 'Geral',
                'gallery'                => 'Galeria',
                'image'                  => 'Imagem',
                'input-options'          => 'Opções de entrada',
                'input-validation'       => 'Validação de entrada',
                'is-comparable'          => 'Este atributo é comparável',
                'is-configurable'        => 'Usado para criar produtos configuráveis',
                'is-required'            => 'Obrigatório',
                'is-unique'              => 'Único',
                'is-visible-on-front'    => 'Visível na página de detalhes do produto (front-end)',
                'label'                  => 'Etiqueta',
                'multiselect'            => 'Seleção múltipla',
                'no'                     => 'Não',
                'number'                 => 'Número',
                'options'                => 'Opções',
                'position'               => 'Posição',
                'price'                  => 'Preço',
                'regex'                  => 'Regex',
                'save-btn'               => 'Salvar atributo',
                'select'                 => 'Selecionar',
                'select-type'            => 'Selecionar tipo de atributo',
                'swatch'                 => 'Amostra tipo',
                'text'                   => 'Texto',
                'textarea'               => 'Área de texto',
                'title'                  => 'Adicionar atributo',
                'type'                   => 'Tipo',
                'url'                    => 'URL',
                'use-in-flat'            => 'Criar na tabela de produtos planos',
                'validations'            => 'Validações',
                'value-per-channel'      => 'Valor por canal',
                'value-per-locale'       => 'Valor por localidade',
                'yes'                    => 'Sim',
                'same-code-error'        => 'O código da opção precisa ser único.',
                'code-hint'              => 'Identificador técnico único usado em importações, exportações e na API. Não pode ser alterado depois.',
                'type-hint'              => 'Define como os valores são inseridos e armazenados. Não pode ser alterado depois.',
                'is-unique-hint'         => 'Cada produto deve ter um valor único para este atributo.',
                'value-per-locale-hint'  => 'Armazene um valor diferente para cada idioma.',
                'value-per-channel-hint' => 'Armazene um valor diferente para cada canal de vendas.',
                'usable-in-grid'         => 'Usável na grade',
                'is-filterable'          => 'É filtrável',
                'ai-translate'           => 'Tradução IA',
                'invalid-swatch-type'    => 'O :attribute não é permitido para o tipo de atributo :type com o tipo de amostra :swatch_type.',
                'single-object-only'     => 'Cada requisição de criação deve enviar um único objeto de atributo.',
                'option'                 => [
                    'color'    => 'Amostra de cor',
                    'dropdown' => 'Menu suspenso',
                    'image'    => 'Amostra de imagem',
                    'save-btn' => 'Salvar opção',
                    'text'     => 'Amostra de texto',
                ],
            ],
            'edit' => [
                'add-attribute-options' => 'Adicionar opções de atributo',
                'add-option'            => 'Adicionar opção',
                'add-options-info'      => 'Crie combinações de opções para o atributo.',
                'add-row'               => 'Adicionar linha',
                'admin'                 => 'Administrador',
                'admin-name'            => 'Nome do Administrador',
                'back-btn'              => 'Voltar',
                'boolean'               => 'Booleano',
                'checkbox'              => 'Caixa de seleção',
                'code'                  => 'Código',
                'color'                 => 'Cor',
                'configuration'         => 'Configuração',
                'create-empty-option'   => 'Criar opção padrão vazia',
                'date'                  => 'Data',
                'datetime'              => 'Data e hora',
                'decimal'               => 'Decimal',
                'default-value'         => 'Valor padrão',
                'email'                 => 'E-mail',
                'enable-wysiwyg'        => 'Ativar editor WYSIWYG',
                'file'                  => 'Arquivo',
                'general'               => 'Geral',
                'image'                 => 'Imagem',
                'input-options'         => 'Opções de entrada',
                'input-validation'      => 'Validação de entrada',
                'is-comparable'         => 'Este atributo é comparável',
                'is-configurable'       => 'Usado para criar produtos configuráveis',
                'is-required'           => 'Obrigatório',
                'is-unique'             => 'Único',
                'is-visible-on-front'   => 'Visível na página de detalhes do produto (front-end)',
                'label'                 => 'Etiqueta',
                'multiselect'           => 'Seleção múltipla',
                'no'                    => 'Não',
                'number'                => 'Número',
                'options'               => 'Opções',
                'position'              => 'Posição',
                'price'                 => 'Preço',
                'regex'                 => 'Regex',
                'save-btn'              => 'Salvar atributo',
                'select'                => 'Selecionar',
                'select-type'           => 'Selecionar tipo de atributo',
                'swatch'                => 'Amostra tipo',
                'text'                  => 'Texto',
                'textarea'              => 'Área de texto',
                'title'                 => 'Editar atributo',
                'type'                  => 'Tipo',
                'url'                   => 'URL',
                'use-in-flat'           => 'Criar na tabela de produtos planos',
                'validations'           => 'Validações',
                'value-per-channel'     => 'Valor por canal',
                'value-per-locale'      => 'Valor por localidade',
                'yes'                   => 'Sim',
                'same-code-error'       => 'O código da opção precisa ser único.',
                'locked-after-creation' => 'Esta configuração é definida na criação e não pode ser alterada posteriormente.',
                'usable-in-grid'        => 'Usável na grade',
                'is-filterable'         => 'É filtrável',
                'ai-translate'          => 'Tradução IA',
                'option'                => [
                    'save-btn'             => 'Salvar opção',
                    'create-success'       => 'Opção de atributo criada com sucesso',
                    'update-success'       => 'Opção de atributo atualizada com sucesso',
                    'sort-update-success'  => 'Ordem de classificação atualizada com sucesso',
                    'sort-update-failure'  => 'Falha ao atualizar a ordem de classificação',
                    'delete-success'       => 'Opção de atributo excluída com sucesso',
                    'invalid-swatch-image' => 'A imagem da amostra é inválida ou contém conteúdo não seguro.',
                    'color'                => 'Amostra de cor',
                    'dropdown'             => 'Menu suspenso',
                    'image'                => 'Amostra de imagem',
                    'text'                 => 'Amostra de texto',
                ],
            ],
            'create-success'    => 'Atributo criado',
            'delete-failed'     => 'Falha ao excluir atributo',
            'delete-success'    => 'Atributo excluído',
            'update-success'    => 'Atributo atualizado',
            'user-define-error' => 'Não é possível excluir atributo do sistema',
            'immutable-fields'  => 'Os seguintes campos não podem ser modificados: :fields.',
            'not-found'         => 'Atributo com o código ":code" não encontrado',
        ],
        'attribute-options' => [
            'delete-success'       => 'Opção de atributo excluída com sucesso',
            'swatch-not-found'     => 'Nenhuma imagem de amostra encontrada para esta opção.',
            'create-success'       => 'Opção de atributo criada com sucesso',
            'update-success'       => 'Opção de atributo atualizada com sucesso',
            'usable-in-grid-error' => 'Somente 4 atributos podem ser marcados como usáveis na grade.',
        ],
        'attribute-groups' => [
            'index' => [
                'create-btn' => 'Criar grupo de atributos',
                'title'      => 'Grupos de atributos',
                'datagrid'   => [
                    'code'                => 'Código',
                    'delete'              => 'Excluir',
                    'mass-delete-success' => 'Grupos de atributos selecionados foram excluídos',
                    'edit'                => 'Editar',
                    'id'                  => 'ID',
                    'inactive'            => 'Inativo',
                    'name'                => 'Nome',
                    'position'            => 'Posição',
                    'update-status'       => 'Atualizar status',
                ],
            ],
            'create' => [
                'title'    => 'Adicionar grupo de atributos',
                'save-btn' => 'Salvar grupo de atributos',
                'back-btn' => 'Voltar',
                'general'  => 'Geral',
                'code'     => 'Código',
                'label'    => 'Etiqueta',
            ],
            'edit' => [
                'title'    => 'Editar grupo de atributos',
                'back-btn' => 'Voltar',
                'save-btn' => 'Salvar grupo de atributos',
                'general'  => 'Geral',
                'code'     => 'Código',
                'label'    => 'Etiqueta',
            ],
            'create-success'        => 'Grupo de atributos criado com sucesso',
            'delete-failed'         => 'Falha ao excluir grupo de atributos',
            'delete-success'        => 'Grupo de atributos excluído com sucesso',
            'update-success'        => 'Grupo de atributos atualizado com sucesso',
            'user-define-error'     => 'Você não pode excluir grupos de atributos do sistema',
            'attribute-group-error' => 'Grupo está sendo usado nas famílias.',
            'not-found'             => 'Grupo de atributos com código ":code" não encontrado',
        ],
        'categories' => [
            'index' => [
                'add-btn'  => 'Criar categoria',
                'title'    => 'Categorias',
                'datagrid' => [
                    'active'         => 'Ativo',
                    'code'           => 'Código',
                    'delete'         => 'Excluir',
                    'delete-success' => 'Os :resource selecionados foram excluídos',
                    'edit'           => 'Editar',
                    'id'             => 'ID',
                    'inactive'       => 'Inativo',
                    'name'           => 'Nome exibido',
                    'category-name'  => 'Nome',
                    'no-of-products' => 'Número de produtos',
                    'position'       => 'Posição',
                    'status'         => 'Visível no menu',
                    'update-status'  => 'Atualizar status',
                ],
            ],
            'browse' => [
                'categories-count'      => ':count categorias',
                'trees'                 => 'Árvores',
                'channel-root'          => 'Raiz do canal',
                'subcategories-count'   => ':count subcategorias',
                'tree-view'             => 'Árvore',
                'list-view'             => 'Lista',
                'add-root'              => 'Adicionar categoria raiz',
                'add-category'          => 'Adicionar categoria',
                'add-child'             => 'Adicionar subcategoria',
                'search-placeholder'    => 'Pesquisar categorias',
                'searching'             => 'Pesquisando...',
                'no-results'            => 'Nenhuma categoria corresponde à sua pesquisa.',
                'load-more'             => 'Carregar mais',
                'loading'               => 'Carregando...',
                'empty'                 => 'Ainda não há categorias.',
                'new-category'          => 'Nova categoria',
                'root-level'            => 'Nível raiz',
                'roots-count'           => ':count árvores',
                'expand-all'            => 'Expandir',
                'collapse-all'          => 'Recolher',
                'expand-all-hint'       => 'Abre mais um nível da árvore.',
                'search-failed'         => 'Não foi possível pesquisar as categorias.',
                'children-failed'       => 'Não foi possível carregar as subcategorias.',
                'descendants-too-large' => 'Esta categoria tem muitas subcategorias para selecionar todas de uma vez.',
            ],

            'create' => [
                'back-btn'            => 'Voltar',
                'code'                => 'Código',
                'general'             => 'Geral',
                'meta-description'    => 'Meta descrição',
                'meta-keywords'       => 'Meta palavras-chave',
                'meta-title'          => 'Meta título',
                'name'                => 'Nome',
                'parent-category'     => 'Categoria pai',
                'position'            => 'Posição',
                'save-btn'            => 'Salvar categoria',
                'select-display-mode' => 'Escolher modo de exibição',
                'seo-details'         => 'Detalhes SEO',
                'settings'            => 'Configurações',
                'slug'                => 'Slug',
                'title'               => 'Adicionar nova categoria',
                'visible-in-menu'     => 'Visível no menu',
                'right-section'       => 'Seção de configuração',
                'left-section'        => 'Seção geral',
            ],
            'edit' => [
                'back-btn'               => 'Voltar',
                'code'                   => 'Código',
                'display-mode'           => 'Modo de exibição',
                'enter-position'         => 'Digite a posição',
                'general'                => 'Geral',
                'name'                   => 'Nome',
                'save-btn'               => 'Salvar categoria',
                'select-display-mode'    => 'Escolher modo de exibição',
                'select-parent-category' => 'Escolher categoria pai',
                'seo-details'            => 'Detalhes SEO',
                'settings'               => 'Configurações',
                'slug'                   => 'Slug',
                'title'                  => 'Editar categoria',
                'visible-in-menu'        => 'Visível no menu',
                'right-section'          => 'Seção de configuração',
                'left-section'           => 'Seção geral',
            ],
            'category'             => 'Categoria',
            'create-success'       => 'Categoria criada com sucesso.',
            'delete-category-root' => 'Não é possível excluir a categoria raiz associada a um canal.',
            'delete-failed'        => 'Falha ao excluir a categoria',
            'delete-success'       => 'Categoria excluída com sucesso.',
            'update-success'       => 'Categoria atualizada com sucesso.',
            'can-not-update'       => 'Esta categoria raiz está associada a um canal e não pode ter uma categoria pai.',
            'invalid-parent'       => 'A categoria pai selecionada é inválida. Uma categoria não pode ser seu próprio pai ou um descendente de si mesma.',
            'unique-validation'    => 'Este valor já está em uso.',
            'not-found'            => 'Categoria com código ":code" não encontrada',
            'unknown-fields'       => 'Campo de categoria com código ":fields" não encontrado',
            'upload-success'       => 'Arquivo de categoria enviado com sucesso.',
            'upload-failure'       => 'Não foi possível enviar o arquivo da categoria.',
            'create-failure'       => 'Categoria não pôde ser criada.',
            'update-failure'       => 'Categoria não pôde ser atualizada.',
        ],
        'category_fields' => [
            'index' => [
                'create-btn' => 'Criar Campo de Categoria',
                'add-btn'    => 'Criar Campo de Categoria',
                'title'      => 'Campos de Categoria',
                'datagrid'   => [
                    'channel-based'       => 'Baseado no Canal',
                    'code'                => 'Código',
                    'created-at'          => 'Criado Em',
                    'delete'              => 'Excluir',
                    'edit'                => 'Editar',
                    'id'                  => 'ID',
                    'locale-based'        => 'Baseado na Localidade',
                    'mass-delete-success' => 'Os campos de categoria selecionados foram excluídos com sucesso',
                    'name'                => 'Nome',
                    'required'            => 'Requerido',
                    'type'                => 'Tipo',
                    'unique'              => 'Único',
                    'disable'             => 'Desativar',
                    'active'              => 'Ativo',
                    'activated'           => 'Ativado',
                    'disabled'            => 'Desativado',
                    'update-status'       => 'Atualizar Status',
                    'mass-update-success' => 'Atualizado com sucesso',
                    'status'              => 'Status',
                    'position'            => 'Posição',
                    'delete-failed'       => 'Este campo de categoria não pode ser excluído.',
                    'mass-delete-failed'  => 'Os campos de categoria selecionados não podem ser excluídos.',
                ],
            ],
            'create' => [
                'add-field-options'   => 'Adicionar Opções ao Campo de Categoria',
                'add-option'          => 'Adicionar Opção',
                'add-options-info'    => 'Para criar combinações diferentes de opções para o campo de categoria de uma vez.',
                'add-row'             => 'Adicionar Linha',
                'admin'               => 'Administrador',
                'admin-name'          => 'Nome do Administrador',
                'back-btn'            => 'Voltar',
                'boolean'             => 'Booleano',
                'checkbox'            => 'Checkbox',
                'code'                => 'Código',
                'color'               => 'Cor',
                'configuration'       => 'Configuração',
                'create-empty-option' => 'Criar opção vazia padrão',
                'date'                => 'Data',
                'datetime'            => 'Data e Hora',
                'decimal'             => 'Decimal',
                'default-value'       => 'Valor Padrão',
                'email'               => 'E-mail',
                'enable-wysiwyg'      => 'Habilitar editor WYSIWYG',
                'file'                => 'Arquivo',
                'general'             => 'Geral',
                'image'               => 'Imagem',
                'input-options'       => 'Opções de Entrada',
                'input-validation'    => 'Validação de Entrada',
                'is-required'         => 'Requerido',
                'is-unique'           => 'Único',
                'label'               => 'Etiqueta',
                'multiselect'         => 'Seleção Múltipla',
                'no'                  => 'Não',
                'number'              => 'Número',
                'options'             => 'Opções',
                'position'            => 'Posição',
                'regex'               => 'Regex',
                'save-btn'            => 'Salvar Campo de Categoria',
                'select'              => 'Selecionar',
                'select-type'         => 'Selecionar Tipo',
                'swatch'              => 'Swatch',
                'text'                => 'Texto',
                'textarea'            => 'Textarea',
                'title'               => 'Adicionar Campo de Categoria',
                'type'                => 'Tipo',
                'url'                 => 'URL',
                'use-in-flat'         => 'Criar na Tabela de Produto',
                'validations'         => 'Validações',
                'value-per-channel'   => 'Valor por Canal',
                'value-per-locale'    => 'Valor por Localidade',
                'yes'                 => 'Sim',
                'status'              => 'Status',
                'set-section'         => 'Seção de Exibição',
                'set-section-left'    => 'Seção Geral',
                'set-section-right'   => 'Seção de Configuração',
                'settings'            => 'Configurações',
                'same-code-error'     => 'O código da opção deve ser único.',
                'option'              => [
                    'dropdown' => 'Caixa suspensa',
                    'save-btn' => 'Salvar opção',
                    'color'    => 'Amostra de cor',
                    'image'    => 'Amostra de imagem',
                    'text'     => 'Amostra de texto',
                ],
            ],
            'edit' => [
                'add-field-options'   => 'Adicionar Opções ao Campo de Categoria',
                'add-option'          => 'Adicionar Opção',
                'add-options-info'    => 'Para criar combinações diferentes de opções para o campo de categoria de uma vez.',
                'add-row'             => 'Adicionar Linha',
                'admin'               => 'Administrador',
                'admin-name'          => 'Nome do Administrador',
                'back-btn'            => 'Voltar',
                'boolean'             => 'Booleano',
                'checkbox'            => 'Checkbox',
                'code'                => 'Código',
                'color'               => 'Cor',
                'configuration'       => 'Configuração',
                'create-empty-option' => 'Criar opção vazia padrão',
                'date'                => 'Data',
                'datetime'            => 'Data e Hora',
                'decimal'             => 'Decimal',
                'default-value'       => 'Valor Padrão',
                'email'               => 'E-mail',
                'enable-wysiwyg'      => 'Habilitar editor WYSIWYG',
                'file'                => 'Arquivo',
                'general'             => 'Geral',
                'image'               => 'Imagem',
                'input-options'       => 'Opções de Entrada',
                'input-validation'    => 'Validação de Entrada',
                'is-filterable'       => 'Usar na navegação hierárquica',
                'is-required'         => 'Requerido',
                'is-unique'           => 'Único',
                'label'               => 'Etiqueta',
                'multiselect'         => 'Seleção Múltipla',
                'no'                  => 'Não',
                'number'              => 'Número',
                'options'             => 'Opções',
                'position'            => 'Posição',
                'regex'               => 'Regex',
                'save-btn'            => 'Salvar Campo de Categoria',
                'select'              => 'Selecionar',
                'select-type'         => 'Selecionar Tipo',
                'swatch'              => 'Swatch',
                'text'                => 'Texto',
                'textarea'            => 'Textarea',
                'title'               => 'Editar Campo de Categoria',
                'type'                => 'Tipo',
                'url'                 => 'URL',
                'use-in-flat'         => 'Criar na Tabela de Produto',
                'validations'         => 'Validações',
                'value-per-channel'   => 'Valor por Canal',
                'value-per-locale'    => 'Valor por Localidade',
                'yes'                 => 'Sim',
                'status'              => 'Status',
                'set-section'         => 'Seção de Exibição',
                'set-section-left'    => 'Seção Geral',
                'set-section-right'   => 'Seção de Configuração',
                'settings'            => 'Configurações',
                'same-code-error'     => 'O código da opção deve ser único.',
                'option'              => [
                    'dropdown' => 'Caixa suspensa',
                    'save-btn' => 'Salvar opção',
                    'color'    => 'Amostra de cor',
                    'image'    => 'Amostra de imagem',
                    'text'     => 'Amostra de texto',
                ],
            ],
            'create-success'    => 'Campo de categoria criado com sucesso',
            'delete-failed'     => 'Falha ao excluir campo de categoria',
            'delete-success'    => 'Campo de categoria excluído com sucesso',
            'update-success'    => 'Campo de categoria atualizado com sucesso',
            'user-define-error' => 'Não é possível excluir o campo de categoria do sistema',
            'not-found'         => 'Campo de categoria com o código ":code" não encontrado',
            'immutable-fields'  => 'Os seguintes campos não podem ser modificados: :fields.',
        ],
        'association_types' => [
            'index' => [
                'title'      => 'Tipos de associação',
                'create-btn' => 'Criar Tipo de Associação',
                'add-btn'    => 'Criar Tipo de Associação',

                'datagrid' => [
                    'code'                => 'Código',
                    'name'                => 'Nome',
                    'status'              => 'Status',
                    'position'            => 'Posição',
                    'edit'                => 'Editar',
                    'delete'              => 'Excluir',
                    'activated'           => 'Ativado',
                    'disabled'            => 'Desativado',
                    'active'              => 'Ativo',
                    'disable'             => 'Desativar',
                    'update-status'       => 'Atualizar Status',
                    'delete-failed'       => 'Este tipo de associação não pode ser excluído.',
                    'mass-delete-success' => 'Os tipos de associação selecionados foram excluídos com sucesso',
                    'mass-delete-failed'  => 'Os tipos de associação selecionados não podem ser excluídos.',
                    'mass-update-success' => 'Atualizado com sucesso',
                ],
            ],

            'create-success'    => 'Tipo de associação criado com sucesso',
            'update-success'    => 'Tipo de associação atualizado com sucesso',
            'delete-success'    => 'Tipo de associação excluído com sucesso',
            'delete-failed'     => 'Falha ao excluir tipo de associação',
            'user-define-error' => 'Não é possível excluir o tipo de associação do sistema',
            'unknown-fields'    => 'Campo do tipo de associação com código ":fields" não encontrado',
            'not-found'         => 'Tipo de associação com o código ":code" não encontrado',
            'immutable-fields'  => 'Os seguintes campos não podem ser modificados: :fields.',

            'create' => [
                'title'      => 'Criar tipo de associação',
                'name'       => 'Nome',
                'enter-name' => 'Digite o nome',
                'code'       => 'Código',
                'enter-code' => 'Digite o código',
                'save-btn'   => 'Salvar tipo de associação',
            ],

            'edit' => [
                'title'    => 'Editar tipo de associação',
                'save-btn' => 'Atualizar tipo de associação',
            ],

            'fields' => [
                'title'                => 'Campos',
                'info'                 => 'Adicione campos personalizados para capturar dados adicionais para este tipo de associação.',
                'add-field-btn'        => 'Adicionar campo',
                'modal-title'          => 'Adicionar campo',
                'edit-modal-title'     => 'Editar campo',
                'save-field-btn'       => 'Salvar campo',
                'same-code-error'      => 'O código do campo deve ser único.',
                'add-fields-info'      => 'Para criar campos personalizados para este tipo de associação.',
                'create-success'       => 'Campo do tipo de associação criado com sucesso',
                'update-success'       => 'Campo do tipo de associação atualizado com sucesso',
                'delete-success'       => 'Campo do tipo de associação excluído com sucesso',
                'not-found'            => 'Campo do tipo de associação com código ":code" não encontrado',
            ],
        ],

        'category-fields-options' => [
            'delete-success'      => 'Opção de campo de categoria excluída com sucesso',
            'create-success'      => 'Opção de campo da categoria criada',
            'update-success'      => 'Opção de campo da categoria atualizada',
            'update-unknown-code' => 'Não existe opção de campo de categoria com o código ":code".',
        ],
        'families' => [
            'index' => [
                'add'      => 'Criar grupo de atributos',
                'title'    => 'Famílias de atributos',
                'create'   => [
                    'name'          => 'Nome',
                    'enter-name'    => 'Digite o nome',
                    'title'         => 'Criar grupo de atributos',
                    'code'          => 'Código',
                    'enter-code'    => 'Digite o código',
                    'based-on'      => 'Baseado em',
                    'select-family' => 'Selecione um grupo para copiar a estrutura',
                    'based-on-info' => 'Copia os grupos, atributos e a estrutura de variantes do grupo selecionado. Deixe em branco para começar com um grupo Geral contendo o SKU.',
                    'save-btn'      => 'Salvar grupo de atributos',
                ],
                'datagrid' => [
                    'code'           => 'Código',
                    'delete'         => 'Excluir',
                    'delete-success' => 'O(s) :resource selecionado(s) foi(ram) excluído(s)',
                    'edit'           => 'Editar',
                    'id'             => 'ID',
                    'method-error'   => 'Erro! Método inválido detectado, verifique a configuração de tarefas em massa',
                    'name'           => 'Nome',
                    'no-resource'    => 'Recursos insuficientes para esta tarefa',
                    'partial-action' => 'Algumas ações não foram realizadas devido a restrições do sistema em :resource',
                    'update-success' => 'O(s) :resource selecionado(s) foi(ram) atualizado(s)',
                ],
            ],
            'edit' => [
                'locale'                           => 'Idioma',
                'level-parent'                     => 'Produto pai',
                'level-sub-parent'                 => 'Subpai',
                'level-variant'                    => 'Variante',
                'common-to-all'                    => 'Comum a todas as variantes',
                'axis'                             => 'Eixo',
                'drop-here'                        => 'Solte aqui os atributos atribuídos',
                'drag-into-level'                  => 'Arraste um atributo para um nível.',
                'search-attributes'                => 'Buscar atributos...',
                'assign-group-to-level'            => 'Atribuir um grupo inteiro a um nível:',
                'add-sub-parent'                   => '+ Adicionar nível subpai',
                'remove-level'                     => 'Remover este nível',
                'select-page'                      => 'Selecionar tudo na página',
                'select-all-matching'              => 'Selecionar todos os :total atributos',
                'all-selected'                     => 'Todos os :total atributos selecionados',
                'selected-to'                      => 'selecionado(s) → para',
                'assign'                           => 'Atribuir',
                'clear'                            => 'limpar',
                'too-large'                        => 'O filtro é grande demais para ser salvo',
                'variant-saved'                    => 'Estrutura da variante salva.',
                'variant-structure-locked'         => 'Esta estrutura de variantes já possui variantes, portanto seus eixos e níveis não podem mais ser alterados. Exclua primeiro as variantes existentes.',
                'variant-structure-deleted'        => 'Estrutura de variantes excluída.',
                'variant-structure-in-use'         => 'Esta estrutura de variantes ainda é referenciada por produtos, portanto não pode ser excluída. Reatribua ou exclua esses produtos primeiro.',
                'no-axis-options'                  => 'Adicione pelo menos um atributo de seleção sem escopo a esta família antes de criar variantes.',
                'parent-child'                     => 'Pai → Filho',
                'parent-sub-parent-child'          => 'Pai → Subpai → Filho',
                'structure'                        => 'Estrutura',
                'level-1-axes'                     => 'Eixos do nível 1',
                'level-2-axes'                     => 'Eixos do nível 2',
                'back-to-variants'                 => 'Voltar às variantes',
                'variant-setup'                    => 'Configuração de variantes',
                'edit-variant'                     => 'Editar variante',
                'delete-variant'                   => 'Excluir variante',
                'parent-product'                   => 'Comum',
                'parent-product-info'              => 'Valores preenchidos uma vez e herdados por cada variante.',
                'sub-parent-product'               => 'Produto subpai',
                'sub-parent-product-info'          => 'Valores compartilhados pelos produtos de cada grupo de nível 1.',
                'variant-child-product'            => 'Produto filho',
                'variant-child-product-info'       => 'Valores preenchidos separadamente para cada variante final.',
                'move-to-parent'                   => 'Mover para Comum',
                'move-to-sub-parent'               => 'Mover para o subpai',
                'move-to-child'                    => 'Mover para o filho',
                'no-parent-attributes'             => 'Nenhum atributo comum',
                'no-parent-attributes-info'        => 'Todos os atributos correspondentes estão atribuídos a níveis de variante ou usados como eixos.',
                'no-sub-parent-attributes'         => 'Nenhum atributo de subpai',
                'no-child-attributes'              => 'Nenhum atributo de filho',
                'axis-badge'                       => 'Eixo',
                'level-badge'                      => 'Nível :number',
                'add-to-level'                     => 'Adicionar ao nível :number',
                'add-attributes'                   => 'Adicionar atributos',
                'move-to-level-info'               => 'Os atributos selecionados saem de Comum e passam a ser mantidos neste nível.',
                'level-empty-info'                 => 'Por enquanto, apenas o eixo é mantido aqui.',
                'cancel'                           => 'Cancelar',
                'axis-locked-info'                 => 'Eixo de variante. Sempre mantido neste nível e não pode ser movido.',
                'drop-or-search-info'              => 'Solte atributos aqui ou ajuste a busca.',
                'name-code-required'               => 'O nome e o código são obrigatórios.',
                'select-axis-warning'              => 'Selecione pelo menos um atributo de eixo diferente para cada nível.',
                'unable-save-variant-structure'    => 'Não foi possível salvar a estrutura da variante.',
                'level-1-attribute'                => 'Atributo do nível 1',
                'level-2-attribute'                => 'Atributo do nível 2',
                'no-axis-selected'                 => 'Nenhum eixo selecionado',
                'variant-name-placeholder'         => 'Exemplo: Cor + Tamanho',
                'variant-code-placeholder'         => 'color_size',
                'attributes-selected'              => 'atributos selecionados',
                'assign-selected-to'               => 'Atribuir os atributos selecionados a um grupo',
                'differ-per'                       => 'Diferente por',
                'differ-sub-help'                  => 'Atributos que mudam neste nível (por ex. uma imagem compartilhada para cada cor).',
                'differ-variant-help'              => 'Atributos exclusivos de cada variante (por ex. SKU, preço, estoque).',
                'drop-attributes-here'             => 'Solte os atributos aqui',
                'main-column-info'                 => 'Grupos e atributos incluídos nesta família.',
                'no-assigned-groups'               => 'Nenhum grupo atribuído',
                'no-assigned-groups-info'          => 'Atribua um grupo de atributos para começar a construir esta família.',
                'remove-group-btn'                 => 'Remover da família',
                'search'                           => 'Pesquisar',
                'select-destination-group'         => 'Selecionar grupo',
                'variant-help'                     => 'Defina como os produtos desta família se dividem em variantes. Arraste um atributo para o nível ao qual ele pertence.',
                'variant-help-easy'                => 'Por padrão, cada atributo é compartilhado por todas as variantes. Arraste apenas os diferentes para um nível abaixo.',
                'variant-levels'                   => 'Níveis de variante',
                'variants'                         => 'Variantes',
                'variant-structure'                => 'Estrutura de variantes',
                'assign-group-btn'                 => 'Atribuir grupo de atributos',
                'assign-group-title'               => 'Atribuir grupo de atributos',
                'back-btn'                         => 'Voltar',
                'create-group'                     => 'Criar este grupo',
                'code'                             => 'Código',
                'label'                            => 'Rótulo',
                'column'                           => 'Coluna',
                'delete-group-btn'                 => 'Excluir grupo',
                'edit-group-info'                  => 'Clique duas vezes para editar o grupo',
                'enter-code'                       => 'Digite o código',
                'enter-name'                       => 'Digite o nome',
                'general'                          => 'Geral',
                'family-attributes'                => 'Atributos da família',
                'group-code-already-exists'        => 'Código do grupo de atributos já existe.',
                'group-contains-system-attributes' => 'Este grupo contém atributos do sistema. Mova os atributos do sistema para outro grupo e tente novamente.',
                'group-name-already-exists'        => 'Nome do grupo de atributos já existe.',
                'groups'                           => 'Grupos',
                'attribute-groups'                 => 'Grupos de atributos',
                'groups-info'                      => 'Gerenciar grupos de atributos na família de atributos',
                'main-column'                      => 'Coluna principal',
                'name'                             => 'Nome',
                'removal-not-possible'             => 'Não é possível remover atributos do sistema da família de atributos.',
                'right-column'                     => 'Coluna direita',
                'save-btn'                         => 'Salvar grupo de atributos',
                'select-group'                     => 'Selecionar grupo de atributos',
                'title'                            => 'Editar grupo de atributos',
                'unassigned-attributes'            => 'Atributos não atribuídos',
                'unassigned-attributes-info'       => 'Arraste um único atributo para um grupo ou marque vários e atribua-os juntos.',
                'unassigned-attribute-groups'      => 'Grupos de atributos não atribuídos',
                'assign-first-attribute-group'     => 'Atribua um grupo à família.',
                'variant-options-label'            => 'Opções de configuração',
                'select-variant'                   => 'Selecionar atributos',
                'add-variant'                      => 'Adicionar variante',
                'all-common'                       => 'Todos os atributos são comuns a todas as variantes.',
                'common-attributes'                => 'Atributos comuns',
                'common-subtitle'                  => 'Compartilhados por todas as variantes. Arraste os que diferem para um nível.',
                'differ-rest-common'               => 'atributo(s) diferem · todo o restante é comum.',
                'drop-differ'                      => 'Arraste aqui os atributos que diferem',
                'family-variants'                  => 'Variantes da família',
                'family-variants-info'             => 'Pesquise variantes e abra uma para gerenciar seus atributos.',
                'move'                             => 'Mover',
                'move-group'                       => 'Mover um grupo inteiro:',
                'move-to'                          => 'mover para',
                'no-unassigned-attributes'         => 'Todos os atributos estão atribuídos',
                'no-unassigned-attributes-info'    => 'Remova um grupo ou atributo da família para torná-lo disponível aqui.',
                'one-level'                        => '1 nível · Pai → Variante',
                'save-variant'                     => 'Salvar variante',
                'search-assigned'                  => 'Pesquisar grupos ou atributos atribuídos',
                'shown'                            => 'exibidos',
                'two-level'                        => '2 níveis · Pai → Subpai → Variante',
                'variant'                          => 'Variante',
                'variants-info'                    => 'Crie e gerencie como os produtos desta família se dividem em variantes pai, subpai e filha.',
            ],
            'attribute-family'                => 'Família de atributos',
            'attribute-product-error'         => 'A família está sendo usada em produtos.',
            'create-success'                  => 'Família criada com sucesso.',
            'delete-failed'                   => 'Erro ao excluir a família.',
            'delete-success'                  => 'Família excluída com sucesso.',
            'family'                          => 'Família',
            'last-delete-error'               => 'É necessário pelo menos uma família.',
            'update-success'                  => 'Família atualizada com sucesso.',
            'user-define-error'               => 'Você não pode excluir uma família de atributos do sistema.',
            'not-found'                       => 'Família de atributos com o código ":code" não encontrada.',
            'can-not-update-variant-options'  => 'Não é possível atualizar as opções de configuração, pois esta família já possui variantes de produtos.',
            'variant-axis-attribute-required' => 'O atributo ":attributes" não pode ser removido porque é usado como eixo de variação nesta família.',
        ],
        'history' => [
            'view'  => 'Ver detalhes da versão',
            'index' => [
                'datagrid' => [
                    'version'   => 'Versão',
                    'user'      => 'Usuário',
                    'date_time' => 'Data / Hora',
                ],
            ],
        ],
    ],
    'settings' => [
        'locales' => [
            'index' => [
                'not-found'              => 'Localidade com o código :code não encontrada.',
                'create-btn'             => 'Criar Localidade',
                'locale'                 => 'Localidade',
                'title'                  => 'Localidades',
                'can-not-delete-error'   => 'Você não pode excluir uma localidade vinculada a um canal ou usuário',
                'can-not-disable-error'  => 'Você não pode desativar uma localidade vinculada a um canal ou usuário',
                'partial-delete-success' => 'As localidades selecionadas foram excluídas, exceto :count vinculadas a um canal ou usuário.',
                'partial-update-success' => 'As localidades selecionadas foram atualizadas, exceto :count vinculadas a um canal ou usuário.',

                'datagrid'              => [
                    'actions'     => 'Ações',
                    'code'        => 'Código',
                    'delete'      => 'Excluir',
                    'direction'   => 'Direção',
                    'edit'        => 'Editar',
                    'id'          => 'ID',
                    'ltr'         => 'LTR',
                    'name'        => 'Nome de Exibição',
                    'rtl'         => 'RTL',
                    'mass-update' => 'Atualizar Status',
                    'status'      => [
                        'title'    => 'Status',
                        'active'   => 'Ativar',
                        'inactive' => 'Desativar',
                    ],
                ],
                'create' => [
                    'code'             => 'Código',
                    'code-hint'        => 'Use o código de localidade ISO, por exemplo en_US ou fr_FR.',
                    'code-placeholder' => 'Use o código ISO, ex.: en_US',
                    'direction'        => 'Direção',
                    'locale-logo'      => 'Logotipo da Localidade',
                    'name'             => 'Nome',
                    'save-btn'         => 'Salvar Localidade',
                    'select-direction' => 'Selecionar Direção',
                    'title'            => 'Criar Localidade',
                    'status'           => 'Status',
                ],
                'edit' => [
                    'title' => 'Editar Localidade',
                ],
                'create-success'    => 'Localidade criada com sucesso.',
                'delete-failed'     => 'Falha ao excluir a localidade.',
                'delete-success'    => 'Localidade excluída com sucesso.',
                'delete-warning'    => 'Tem certeza de que deseja executar esta ação?',
                'last-delete-error' => 'Pelo menos uma localidade é necessária.',
                'update-success'    => 'Localidade atualizada com sucesso.',
            ],
        ],
        'currencies' => [
            'index' => [
                'not-found'              => 'Moeda com o código :code não encontrada.',
                'create-btn'             => 'Criar Moeda',
                'currency'               => 'Moeda',
                'title'                  => 'Moedas',
                'can-not-delete-error'   => 'Você não pode excluir uma moeda vinculada a um canal',
                'can-not-disable-error'  => 'Você não pode desativar uma moeda vinculada a um canal',
                'partial-update-success' => 'As moedas selecionadas foram atualizadas, exceto :count vinculadas a um canal.',
                'datagrid'               => [
                    'actions'        => 'Ações',
                    'code'           => 'Código',
                    'delete'         => 'Excluir',
                    'edit'           => 'Editar',
                    'id'             => 'ID',
                    'method-error'   => 'Erro! Método incorreto detectado, verifique a configuração da ação em massa',
                    'name'           => 'Nome de Exibição',
                    'no-resource'    => 'O recurso fornecido é insuficiente para a ação',
                    'partial-action' => 'Algumas ações não foram realizadas devido a restrições do sistema no :resource',
                    'update-success' => ':resource selecionados foram atualizados com sucesso',
                    'mass-update'    => 'Atualizar Status',
                    'status'         => [
                        'title'    => 'Status',
                        'active'   => 'Ativar',
                        'inactive' => 'Desativar',
                    ],
                ],
                'create' => [
                    'code'             => 'Código',
                    'code-hint'        => 'Use o código de moeda ISO 4217, por exemplo USD ou EUR.',
                    'code-placeholder' => 'Use o código ISO, ex.: USD',
                    'create-btn'       => 'Criar Moeda',
                    'decimal'          => 'Decimal',
                    'delete-warning'   => 'Tem certeza de que deseja executar esta ação?',
                    'general'          => 'Geral',
                    'name'             => 'Nome',
                    'save-btn'         => 'Salvar Moeda',
                    'symbol'           => 'Símbolo',
                    'title'            => 'Criar Nova Moeda',
                    'status'           => 'Status',
                ],
                'edit' => [
                    'title' => 'Editar Moeda',
                ],
                'create-success'    => 'Moeda criada com sucesso.',
                'delete-failed'     => 'Falha ao excluir a moeda.',
                'delete-success'    => 'Moeda excluída com sucesso.',
                'last-delete-error' => 'Pelo menos uma moeda é necessária.',
                'update-success'    => 'Moeda atualizada com sucesso.',
                'cannot-delete'     => 'A moeda não pode ser excluída.',
            ],
        ],
        'data-transfer' => [
            'tracker' => [
                'index' => [
                    'title'    => 'Rastreador de Tarefas',
                    'datagrid' => [
                        'id'                   => 'ID',
                        'job_code'             => 'Tarefa',
                        'type'                 => 'Tipo',
                        'job_type'             => 'Tipo de Tarefa',
                        'state'                => 'Status',
                        'processed_rows_count' => 'Linhas Processadas',
                        'invalid_rows_count'   => 'Linhas Inválidas',
                        'started_at'           => 'Iniciado em',
                        'completed_at'         => 'Concluído em',
                        'user'                 => 'Usuário',
                        'created_at'           => 'Criado em',
                        'updated_at'           => 'Atualizado em',
                        'pending'              => 'Pendente',
                        'validating'           => 'Validando',
                        'validated'            => 'Validado',
                        'processing'           => 'Processando',
                        'processed'            => 'Processado',
                        'linking'              => 'Vinculando',
                        'linked'               => 'Vinculado',
                        'indexing'             => 'Indexando',
                        'indexed'              => 'Indexado',
                        'completed'            => 'Concluído',
                        'paused'               => 'Pausado',
                        'cancelled'            => 'Cancelado',
                        'failed'               => 'Falhou',
                        'view'                 => 'Ver',
                    ],
                ],
                'import' => [
                    'back-btn'                => 'Voltar',
                    'completed-batches'       => 'Total de Lotes Concluídos:',
                    'download-error-report'   => 'Baixar Relatório Completo',
                    'edit-btn'                => 'Editar',
                    'imported-info'           => 'Parabéns! Sua importação foi bem-sucedida.',
                    'exporting-info'          => 'Exportação em andamento',
                    'indexing-info'           => 'Indexação de Recursos (Preço, Elasticsearch) em Andamento',
                    'linking-info'            => 'Vinculação de Recursos em Andamento',
                    'progress'                => 'Progresso:',
                    'title'                   => 'Exportar',
                    'total-batches'           => 'Total de Lotes:',
                    'created'                 => 'Total de Registros Criados:',
                    'total-deleted'           => 'Total de Registros Excluídos:',
                    'total-errors'            => 'Total de Erros:',
                    'total-invalid-rows'      => 'Total de Linhas Inválidas:',
                    'total-rows-processed'    => 'Total de Linhas Processadas:',
                    'total-updated'           => 'Total de Registros Atualizados:',
                    'validate-info'           => 'Clique em "Validar Dados" para verificar sua importação.',
                    'validate'                => 'Validar',
                    'validating-info'         => 'Os dados começaram a ser lidos e validados',
                    'validation-failed-info'  => 'Sua importação é inválida. Corrija os erros abaixo e tente novamente.',
                    'validation-success-info' => 'Sua importação é válida. Clique em "Exportar" para iniciar o processo de importação.',
                ],
                'summary' => [
                    'total-batches' => 'Total de Lotes:',
                    'created'       => 'Total de Registros Criados:',
                    'deleted'       => 'Total de Registros Excluídos:',
                    'errors'        => 'Total de Erros:',
                    'invalid-rows'  => 'Total de Linhas Inválidas:',
                    'processed'     => 'Total de Linhas Processadas:',
                    'updated'       => 'Total de Registros Atualizados:',
                    'skipped'       => 'Total de Registros Ignorados:',
                ],
                'info'                  => 'Parabéns! Sua tarefa foi executada com sucesso.',
                'running-time'          => 'Tempo de execução',
                'estimated-remaining'   => 'Tempo restante estimado',
                'total-duration'        => 'Duração total',
                'status'                => 'Status',
                'waiting'               => 'Aguardando',
                'total-records'         => 'Total de registros',
                'job-id'                => 'ID do trabalho',
                'records-created'       => 'Registros criados',
                'records-updated'       => 'Registros atualizados',
                'records-deleted'       => 'Registros excluídos',
                'run-again'             => 'Executar novamente',
                'completed-success'     => 'trabalho concluído com sucesso',
                'completed-info-sub'    => 'Todos os registros foram processados e indexados. Seu catálogo está atualizado.',
                'create-success'        => 'Exportação criada com sucesso.',
                'delete-failed'         => 'Falha ao excluir a exportação.',
                'delete-success'        => 'Exportação excluída com sucesso.',
                'not-valid'             => 'Exportação inválida',
                'nothing-to-import'     => 'Não há recursos para importar.',
                'setup-queue-error'     => 'Por favor, altere seu driver de fila para "database" ou "redis" para iniciar o processo de importação.',
                'update-success'        => 'Exportação atualizada com sucesso.',
                'failed-info'           => 'A tarefa falhou. Corrija os erros abaixo e tente novamente.',
                'download-log-file'     => 'Baixar Registro',
                'pause'                 => 'Pausar',
                'resume'                => 'Retomar',
                'cancel'                => 'Cancelar',
                'paused'                => 'A importação foi pausada.',
                'paused-export'         => 'A exportação foi pausada.',
                'resumed'               => 'A importação foi retomada.',
                'resumed-export'        => 'A exportação foi retomada.',
                'zip-failed'            => 'Falha ao criar o arquivo zip.',
                'nothing-to-archive'    => 'Não há arquivos exportados disponíveis para download.',
                'cancelled'             => 'A importação foi cancelada.',
                'cancelled-export'      => 'A exportação foi cancelada.',
                'paused-info'           => 'A importação está pausada. Os lotes restantes estão em espera.',
                'paused-info-export'    => 'A exportação está pausada. Os lotes restantes estão em espera.',
                'cancelled-info'        => 'A importação foi cancelada. Os resultados parciais são exibidos abaixo.',
                'cancelled-info-export' => 'A exportação foi cancelada. Os resultados parciais são exibidos abaixo.',
                'cancel-confirm'        => 'Tem certeza de que deseja cancelar este trabalho? Esta ação não pode ser desfeita.',
                'pause-failed'          => 'Falha ao pausar a importação.',
                'pause-failed-export'   => 'Falha ao pausar a exportação.',
                'resume-failed'         => 'Falha ao retomar a importação.',
                'resume-failed-export'  => 'Falha ao retomar a exportação.',
                'cancel-failed'         => 'Falha ao cancelar a importação.',
                'cancel-failed-export'  => 'Falha ao cancelar a exportação.',
                'live'                  => 'Ao vivo',
                'import-progress'       => 'Progresso da importação',
                'export-progress'       => 'Progresso da exportação',
                'linking-progress'      => 'Progresso da vinculação',
                'indexing-progress'     => 'Progresso da indexação',
                'job-label'             => 'Trabalho',
            ],
            'imports' => [
                'import-start-fail' => 'Falha ao iniciar o processo de importação. Tente novamente.',
                'rerun-no-file'     => 'Este trabalho não possui arquivo de importação, portanto não pode ser executado novamente.',
                'create'            => [
                    'code'                => 'Código',
                    'action'              => 'Ação',
                    'allowed-errors'      => 'Erros Permitidos',
                    'allowed-file-types'  => 'Tipos de arquivo permitidos (CSV, XLSX, XLS)',
                    'back-btn'            => 'Voltar',
                    'create-update'       => 'Criar/Atualizar',
                    'delete'              => 'Excluir',
                    'media'               => 'Arquivo / Imagens',
                    'images'              => 'Imagens',
                    'download-sample'     => 'Baixar CSV de Amostra :resource',
                    'field-separator'     => 'Separador de Campo',
                    'separator-info'      => 'Usar apenas para arquivos CSV',
                    'file-info-example'   => 'Coloque as imagens em storage/app/public/. Para imagens em storage/app/public/import-images, inclua import-images/ no caminho e use apenas o nome do arquivo no arquivo de importação.',
                    'file-info'           => 'Use o caminho relativo para /project-root/storage/app/import, ex. product-images, import-images.',
                    'file'                => 'Arquivo',
                    'general'             => 'Geral',
                    'upload_images'       => 'Carregar Imagens para definir o Caminho',
                    'download-sample-zip' => 'Baixar ZIP de Imagens de Amostra',
                    'zip-uploading'       => 'Enviando e extraindo ZIP...',
                    'zip-upload-success'  => 'ZIP enviado e extraído com sucesso.',
                    'zip-upload-error'    => 'Falha ao enviar o ZIP. Tente novamente.',
                    'invalid-zip'         => 'O arquivo enviado não é um arquivo ZIP válido.',
                    'zip-upload-or'       => 'ou envie um ZIP de imagens',
                    'zip-click-upload'    => 'Clique para enviar um ZIP de imagens',
                    'zip-files-extracted' => 'arquivos extraídos',
                    'zip-drop-hint'       => 'Clique ou arraste e solte um arquivo ZIP aqui',
                    'file-upload-info'    => 'CSV, XLSX, JSON (MÁX. 2MB)',
                    'images-directory'    => 'Caminho',
                    'process-in-queue'    => 'Processar na Fila',
                    'results'             => 'Resultados',
                    'save-btn'            => 'Salvar Importação',
                    'settings'            => 'Configurações',
                    'skip-errors'         => 'Ignorar Erros',
                    'stop-on-errors'      => 'Parar em Erros',
                    'title'               => 'Criar Importação',
                    'type'                => 'Tipo',
                    'validation-strategy' => 'Estratégia de Validação',
                ],
                'edit' => [
                    'images'              => 'Imagens',
                    'action'              => 'Ação',
                    'allowed-errors'      => 'Erros Permitidos',
                    'allowed-file-types'  => 'Tipos de arquivo permitidos (CSV, XLSX, XLS)',
                    'back-btn'            => 'Voltar',
                    'create-update'       => 'Criar/Atualizar',
                    'delete'              => 'Excluir',
                    'download-sample'     => 'Baixar Amostra',
                    'field-separator'     => 'Separador de Campo',
                    'separator-info'      => 'Usar apenas para arquivos CSV',
                    'file-info-example'   => 'Coloque as imagens em storage/app/public/. Para imagens em storage/app/public/import-images, inclua import-images/ no caminho e use apenas o nome do arquivo no arquivo de importação.',
                    'file-info'           => 'Use o caminho relativo para /project-root/storage/app/import, ex. product-images, import-images.',
                    'file'                => 'Arquivo',
                    'general'             => 'Geral',
                    'images-directory'    => 'Caminho',
                    'process-in-queue'    => 'Processar na Fila',
                    'results'             => 'Resultados',
                    'save-btn'            => 'Salvar Importação',
                    'settings'            => 'Configurações',
                    'skip-errors'         => 'Ignorar Erros',
                    'stop-on-errors'      => 'Parar em Erros',
                    'title'               => 'Editar Importação',
                    'type'                => 'Tipo',
                    'validation-strategy' => 'Estratégia de Validação',
                ],
                'batch' => [
                    'title' => 'Importação adicionada na fila',
                ],
                'index' => [
                    'button-title' => 'Criar Importação',
                    'title'        => 'Importações',
                    'datagrid'     => [
                        'actions'               => 'Ações',
                        'completed-at'          => 'Concluído Em',
                        'created'               => 'Criado',
                        'delete'                => 'Excluir',
                        'deleted'               => 'Deletado',
                        'edit'                  => 'Editar',
                        'error-file'            => 'Arquivo de Erro',
                        'id'                    => 'ID',
                        'code'                  => 'Código',
                        'type'                  => 'Tipo',
                        'action'                => 'Ação',
                        'started-at'            => 'Iniciado Em',
                        'state'                 => 'Estado',
                        'summary'               => 'Resumo',
                        'updated'               => 'Atualizado',
                        'uploaded-file'         => 'Arquivo Carregado',
                        'images_directory_path' => 'Arquivo de Imagens',
                        'import'                => 'Importar',
                    ],
                ],
                'import' => [
                    'action'                  => 'Modo de Ação',
                    'back-btn'                => 'Voltar',
                    'completed-batches'       => 'Total de Lotes Concluídos:',
                    'download-error-report'   => 'Baixar Relatório Completo',
                    'edit-btn'                => 'Editar',
                    'exporting-info'          => 'Exportação em andamento',
                    'exporting-info-sub'      => 'Gerando arquivo de exportação e gravando registros. Por favor, aguarde.',
                    'file_path'               => 'Caminho do Arquivo',
                    'imported-info'           => 'Parabéns! Sua importação foi bem-sucedida.',
                    'import-now'              => 'Importar Agora',
                    'importing-info'          => 'Importação em Processo',
                    'importing-info-sub'      => 'Processando lotes e gravando registros no banco de dados.',
                    'indexing-info'           => 'Indexação de Recursos (Preço, Elastic Search) Em Andamento',
                    'indexing-info-sub'       => 'Reconstruindo índices de busca e caches de preços após a importação. Esta etapa é executada automaticamente.',
                    'indexing-type'           => 'Preço + ES',
                    'linking-info'            => 'Vinculação de Recursos Em Andamento',
                    'linking-info-sub'        => 'Vinculando recursos relacionados e associações entre registros.',
                    'pending-info'            => 'Trabalho na fila — aguardando o início do trabalhador...',
                    'pending-info-sub'        => 'Seu trabalho foi adicionado à fila de processamento. Um worker irá processá-lo em breve.',
                    'pending-step-complete'   => 'Concluído',
                    'pending-step-export'     => 'Exportando',
                    'pending-step-import'     => 'Importando',
                    'pending-step-queue'      => 'Na fila',
                    'pending-step-validate'   => 'Validando',
                    'profile'                 => 'Perfil de importação',
                    'progress'                => 'Progresso:',
                    'title'                   => 'Importar',
                    'total-batches'           => 'Total de lotes:',
                    'total-created'           => 'Total de registros criados:',
                    'total-deleted'           => 'Total de registros excluídos:',
                    'total-errors'            => 'Total de erros:',
                    'total-invalid-rows'      => 'Total de linhas inválidas:',
                    'total-rows-processed'    => 'Total de linhas processadas:',
                    'total-updated'           => 'Total de registros atualizados:',
                    'validate'                => 'Validar',
                    'validate-info'           => 'Clique em Validar Dados para verificar sua importação.',
                    'validating-info'         => 'Lendo arquivo e validando dados...',
                    'validating-info-sub'     => 'Verificando a integridade do esquema, mapeamentos de campos e tipos de dados antes de iniciar a importação.',
                    'validation-failed-info'  => 'Sua importação é inválida. Corrija os erros a seguir e tente novamente.',
                    'validation-success-info' => 'Sua importação é válida. Iniciando a importação automaticamente...',
                ],
                'create-success'             => 'Importação criada com sucesso.',
                'delete-failed'              => 'A exclusão da importação falhou inesperadamente.',
                'delete-success'             => 'Importação excluída com sucesso.',
                'invalid-zip'                => 'O arquivo enviado não é um arquivo ZIP válido.',
                'not-valid'                  => 'Importação é inválida',
                'zip-compression-suspicious' => 'A entrada ":entry" se expande muito além do seu tamanho compactado, então o arquivo foi rejeitado por não ser seguro.',
                'zip-contents-too-large'     => 'As imagens deste ZIP somam mais de :limit MB depois de extraídas. Divida-as em arquivos menores.',
                'zip-exceeds-server-limit'   => 'Este ZIP é maior que os :limit KB que este servidor aceita em um único envio. Aumente upload_max_filesize e post_max_size no php.ini, ou divida o arquivo.',
                'zip-too-many-entries'       => 'Este ZIP contém :count arquivos, e no máximo :limit podem ser importados de uma vez. Divida-o em arquivos menores ou aumente IMAGE_IMPORT_MAX_ENTRIES.',
                'nothing-to-import'          => 'Não há recursos para importar.',
                'setup-queue-error'          => 'Por favor, altere seu driver de fila para "database" ou "redis" para iniciar o processo de importação.',
                'update-success'             => 'Importação atualizada com sucesso.',
                'zip-upload-success'         => 'ZIP enviado e extraído com sucesso.',
            ],
            'exports' => [
                'create' => [
                    'code'                      => 'Código',
                    'action'                    => 'Ação',
                    'allowed-errors'            => 'Erros Permitidos',
                    'back-btn'                  => 'Voltar',
                    'create-update'             => 'Criar/Atualizar',
                    'delete'                    => 'Excluir',
                    'media'                     => 'Arquivo / Imagens',
                    'images'                    => 'Imagens',
                    'download-sample'           => 'Baixar Exemplo :resource',
                    'field-separator'           => 'Separador de Campo',
                    'file-info-example'         => 'Por exemplo, no caso de imagens de produtos, os arquivos devem ser colocados na pasta /project-root/storage/app/export/product-images.',
                    'file-info'                 => 'Use caminho relativo para /project-root/storage/app/export, ex: product-images, export-images.',
                    'file'                      => 'Arquivo',
                    'general'                   => 'Geral',
                    'upload_images'             => 'Enviar Imagens para Definir Caminho',
                    'download-sample-zip'       => 'Baixar Exemplo de Imagens em ZIP',
                    'images-directory'          => 'Caminho',
                    'process-in-queue'          => 'Processar na Fila',
                    'results'                   => 'Resultados',
                    'save-btn'                  => 'Salvar Exportação',
                    'settings'                  => 'Configurações',
                    'scope-filters'             => 'Dados para exportar',
                    'product-filters'           => 'Filtros de produtos',
                    'output'                    => 'Saída',
                    'format-settings'           => 'Configurações de formato',
                    'custom-attribute-select'   => 'Selecionar atributo',
                    'custom-attribute-value'    => 'Valor',
                    'custom-attribute-remove'   => 'Remover',
                    'attribute-conditions'      => 'Condições de atributo',
                    'attribute-conditions-info' => 'Apenas os produtos que atendem a todas as condições são exportados.',
                    'no-categories'             => 'Nenhuma categoria disponível.',
                    'search-categories'         => 'Pesquisar categorias por nome ou código',
                    'add-condition'             => 'Adicionar outra condição de atributo',
                    'operator-select'           => 'Condição',
                    'no-value-needed'           => 'Nenhum valor necessário',
                    'value-number-placeholder'  => 'ex.: 49.99',
                    'range-from'                => 'De',
                    'range-to'                  => 'Até',
                    'condition-yes'             => 'Sim',
                    'condition-no'              => 'Não',
                    'operators'                 => [
                        'in'                 => 'Na lista',
                        'not_in'             => 'Fora da lista',
                        'empty'              => 'Está vazio',
                        'not_empty'          => 'Não está vazio',
                        'contains'           => 'Contém',
                        'equals'             => 'Igual a',
                        'less_than'          => 'Menor que',
                        'less_than_equal'    => 'Menor ou igual a',
                        'greater_than'       => 'Maior que',
                        'greater_than_equal' => 'Maior ou igual a',
                        'between'            => 'Entre',
                        'before'             => 'Antes',
                        'after'              => 'Depois',
                    ],
                    'filters'             => 'Filtros',
                    'skip-errors'         => 'Pular Erros',
                    'stop-on-errors'      => 'Parar nos Erros',
                    'title'               => 'Criar Exportação',
                    'type'                => 'Tipo',
                    'validation-strategy' => 'Estratégia de Validação',
                ],
                'edit' => [
                    'action'              => 'Ação',
                    'allowed-errors'      => 'Erros Permitidos',
                    'back-btn'            => 'Voltar',
                    'create-update'       => 'Criar/Atualizar',
                    'delete'              => 'Excluir',
                    'download-sample'     => 'Baixar Exemplo',
                    'field-separator'     => 'Separador de Campo',
                    'file-info-example'   => 'Por exemplo, no caso de imagens de produtos, os arquivos devem ser colocados na pasta /project-root/storage/app/export/product-images.',
                    'file-info'           => 'Use caminho relativo para /project-root/storage/app/export, ex: product-images, export-images.',
                    'file'                => 'Arquivo',
                    'general'             => 'Geral',
                    'images-directory'    => 'Caminho do Diretório de Imagens',
                    'process-in-queue'    => 'Processar na Fila',
                    'results'             => 'Resultados',
                    'save-btn'            => 'Salvar Exportação',
                    'settings'            => 'Configurações',
                    'skip-errors'         => 'Pular Erros',
                    'stop-on-errors'      => 'Parar nos Erros',
                    'title'               => 'Editar Exportação',
                    'type'                => 'Tipo',
                    'validation-strategy' => 'Estratégia de Validação',
                ],
                'batch' => [
                    'title' => 'Exportação adicionada na fila',
                ],
                'index' => [
                    'button-title' => 'Criar Exportação',
                    'title'        => 'Exportações',
                    'datagrid'     => [
                        'actions'               => 'Ações',
                        'completed-at'          => 'Concluído em',
                        'created'               => 'Criado',
                        'delete'                => 'Excluir',
                        'deleted'               => 'Excluído',
                        'edit'                  => 'Editar',
                        'error-file'            => 'Arquivo de Erro',
                        'id'                    => 'ID',
                        'code'                  => 'Código',
                        'type'                  => 'Tipo',
                        'action'                => 'Ação',
                        'started-at'            => 'Iniciado em',
                        'state'                 => 'Estado',
                        'summary'               => 'Resumo',
                        'updated'               => 'Atualizado',
                        'uploaded-file'         => 'Arquivo Enviado',
                        'images_directory_path' => 'Caminho do Arquivo de Imagens',
                        'export'                => 'Exportar',
                    ],
                ],
                'export' => [
                    'back-btn'                => 'Voltar',
                    'completed-batches'       => 'Total de Lotes Concluídos:',
                    'download-error-report'   => 'Baixar Relatório Completo',
                    'download-created-file'   => 'Baixar Arquivos Exportados',
                    'edit-btn'                => 'Editar',
                    'exported-info'           => 'Parabéns! Sua exportação foi bem-sucedida.',
                    'exporting-info'          => 'Exportação em andamento',
                    'export-now'              => 'Exportar Agora',
                    'indexing-info'           => 'Indexação de Recursos (Preço, Elastic Search) em andamento',
                    'linking-info'            => 'Linkagem de Recursos em andamento',
                    'progress'                => 'Progresso:',
                    'title'                   => 'Exportar',
                    'total-batches'           => 'Total de Lotes:',
                    'total-created'           => 'Total de Registros Criados:',
                    'total-deleted'           => 'Total de Registros Excluídos:',
                    'total-errors'            => 'Total de Erros:',
                    'total-invalid-rows'      => 'Total de Linhas Inválidas:',
                    'total-rows-processed'    => 'Total de Linhas Processadas:',
                    'total-updated'           => 'Total de Registros Atualizados:',
                    'validate-info'           => 'Clique em Validar Dados para verificar sua exportação.',
                    'validate'                => 'Validar',
                    'validating-info'         => 'Lendo e Validando Dados',
                    'validation-failed-info'  => 'Sua exportação é inválida. Corrija os erros abaixo e tente novamente.',
                    'validation-success-info' => 'Sua exportação é válida. Clique em Exportar para iniciar o processo.',
                    'profile'                 => 'Perfil de Exportação',
                    'file-format'             => 'Formato de Arquivo',
                    'with-media'              => 'Com Mídia',
                ],
                'create-success'    => 'Exportação criada com sucesso.',
                'delete-failed'     => 'Falha ao excluir a exportação inesperadamente.',
                'delete-success'    => 'Exportação excluída com sucesso.',
                'not-valid'         => 'Exportação inválida',
                'nothing-to-export' => 'Não há recursos para exportar.',
                'setup-queue-error' => 'Altere o driver da fila para "database" ou "redis" para iniciar o processo de exportação.',
                'update-success'    => 'Exportação atualizada com sucesso.',
            ],
        ],
        'channels' => [
            'index' => [
                'not-found'            => 'Canal com o código :code não encontrado.',
                'create-btn'           => 'Criar Canal',
                'delete-failed'        => 'Falha ao excluir canal',
                'delete-success'       => 'Canal excluído com sucesso.',
                'last-delete-error'    => 'Não é possível excluir o canal padrão.',
                'can-not-delete-error' => 'Não é possível excluir o canal ":channel", pois o seu PIM precisa ter pelo menos um canal.',
                'title'                => 'Canais',
                'datagrid'             => [
                    'code'          => 'Código',
                    'delete'        => 'Excluir',
                    'edit'          => 'Editar',
                    'host-name'     => 'Nome do Host',
                    'id'            => 'ID',
                    'name'          => 'Nome',
                    'root-category' => 'Categoria Raiz',
                ],
            ],
            'create' => [
                'allowed-ips'            => 'IPs permitidos',
                'cancel'                 => 'Voltar',
                'code'                   => 'Código',
                'create-success'         => 'Canal criado com sucesso.',
                'currencies'             => 'Moedas',
                'currencies-and-locales' => 'Moedas e Locales',
                'description'            => 'Descrição',
                'design'                 => 'Design',
                'ui-locale'              => 'Localização da UI',
                'favicon'                => 'Favicon',
                'favicon-size'           => 'A resolução da imagem deve ser de 16px X 16px',
                'general'                => 'Geral',
                'hostname'               => 'Nome do Host',
                'hostname-placeholder'   => 'https://www.exemplo.com (Não adicione barra no final.)',
                'last-delete-error'      => 'Pelo menos um canal é necessário.',
                'locales'                => 'Localizações',
                'logo'                   => 'Logo',
                'logo-size'              => 'A resolução da imagem deve ser de 192px X 50px',
                'maintenance-mode-text'  => 'Mensagem',
                'name'                   => 'Nome',
                'root-category'          => 'Categoria Raiz',
                'save-btn'               => 'Salvar Canal',
                'select-root-category'   => 'Selecionar Categoria Raiz',
                'select-theme'           => 'Selecionar Tema',
                'seo'                    => 'SEO da Página Inicial',
                'seo-description'        => 'Descrição Meta',
                'seo-keywords'           => 'Palavras-chave Meta',
                'seo-title'              => 'Título Meta',
                'settings'               => 'Configurações',
                'status'                 => 'Status',
                'theme'                  => 'Tema',
                'title'                  => 'Criar Canal',
            ],
            'edit' => [
                'allowed-ips'            => 'IPs permitidos',
                'back-btn'               => 'Voltar',
                'code'                   => 'Código',
                'currencies'             => 'Moedas',
                'currencies-and-locales' => 'Moedas e Locales',
                'default-currency'       => 'Moeda Padrão',
                'description'            => 'Descrição',
                'ui-locale'              => 'Localização da UI',
                'design'                 => 'Design',
                'favicon'                => 'Favicon',
                'favicon-size'           => 'A resolução da imagem deve ser de 16px X 16px',
                'general'                => 'Geral',
                'hostname'               => 'Nome do Host',
                'hostname-placeholder'   => 'https://www.exemplo.com (Não adicione barra no final.)',
                'last-delete-error'      => 'Pelo menos um canal é necessário.',
                'locales'                => 'Localizações',
                'select-locales'         => 'Selecionar Localizações',
                'select-currencies'      => 'Selecionar Moedas',
                'logo'                   => 'Logo',
                'logo-size'              => 'A resolução da imagem deve ser de 192px X 50px',
                'maintenance-mode'       => 'Modo de Manutenção',
                'maintenance-mode-text'  => 'Mensagem',
                'name'                   => 'Nome',
                'root-category'          => 'Categoria Raiz',
                'save-btn'               => 'Salvar Canal',
                'seo'                    => 'SEO da Página Inicial',
                'seo-description'        => 'Descrição Meta',
                'seo-keywords'           => 'Palavras-chave Meta',
                'seo-title'              => 'Título Meta',
                'status'                 => 'Status',
                'name-translations'      => 'Traduções de Nome',
                'theme'                  => 'Tema',
                'title'                  => 'Editar Canal',
                'update-success'         => 'Canal atualizado com sucesso',
            ],
        ],
        'users' => [
            'index' => [
                'admin'  => 'Administrador',
                'title'  => 'Usuários',
                'user'   => 'Usuário',

                'confirm-password-before-delete' => 'Confirmar senha antes de excluir',
                'enter-current-password'         => 'Digite a senha atual',
                'password'                       => 'Senha',
                'confirm-delete-account'         => 'Confirmar exclusão desta conta',
                'create'                         => [
                    'confirm-password'    => 'Confirmar Senha',
                    'email'               => 'E-mail',
                    'name'                => 'Nome',
                    'password'            => 'Senha',
                    'role'                => 'Função',
                    'save-btn'            => 'Salvar Usuário',
                    'status'              => 'Status',
                    'title'               => 'Criar Usuário',
                    'upload-image-info'   => 'Envie uma imagem de perfil (110px X 110px), ou use sua imagem do Gravatar.com vinculada ao seu e-mail.',
                    'user-timezone'       => 'Fuso horário',
                    'catalog-locale'      => 'Idioma do catálogo',
                    'catalog-locale-info' => 'O idioma em que este usuário escreve o conteúdo do catálogo. Separado do idioma de interface dele.',
                    'default-channel'     => 'Canal padrão',
                    'select'              => 'Selecionar',
                ],
                'datagrid' => [
                    'actions'  => 'Ações',
                    'active'   => 'Ativo',
                    'delete'   => 'Excluir',
                    'edit'     => 'Editar',
                    'email'    => 'E-mail',
                    'id'       => 'ID',
                    'inactive' => 'Inativo',
                    'name'     => 'Nome',
                    'role'     => 'Função',
                    'status'   => 'Status',
                    'image'    => 'Imagem',
                ],
                'edit' => [
                    'title' => 'Editar Usuário',
                ],

                'delete-account' => [
                    'confirm-password-title' => 'Confirmar senha antes de excluir',
                    'enter-current-password' => 'Digite a senha atual',
                    'password'               => 'Senha',
                    'confirm-delete'         => 'Confirmar exclusão desta conta',
                ],
            ],
            'edit' => [
                'back-btn'         => 'Voltar',
                'confirm-password' => 'Confirmar Senha',
                'email'            => 'E-mail',
                'general'          => 'Geral',
                'name'             => 'Nome',
                'password'         => 'Senha',
                'role'             => 'Função',
                'save-btn'         => 'Salvar Usuário',
                'status'           => 'Status',
                'title'            => 'Editar Usuário',
            ],
            'activate-warning'                 => 'Sua conta ainda não foi ativada. Por favor, entre em contato com o administrador.',
            'cannot-change'                    => 'O usuário não pode ser alterado',
            'cannot-assign-unheld-permissions' => 'Você não pode atribuir uma função que contenha permissões que você não possui.',
            'cannot-escalate-role'             => 'Você não tem permissão para atribuir uma função com acesso total.',
            'create-success'                   => 'Usuário criado com sucesso.',
            'delete-failed'                    => 'Falha ao excluir o usuário.',
            'delete-success'                   => 'Usuário excluído com sucesso.',
            'delete-warning'                   => 'Tem certeza de que deseja realizar esta ação?',
            'incorrect-password'               => 'Senha incorreta',
            'last-delete-error'                => 'Falha ao excluir o último usuário',
            'login-error'                      => 'Verifique suas credenciais e tente novamente.',
            'update-success'                   => 'Usuário atualizado com sucesso.',
            'current-user-delete-error'        => 'Não é possível excluir o usuário logado.',
            'last-all-access-delete-error'     => 'O último administrador ativo com acesso total não pode ser excluído.',
        ],
        'system-settings' => [
            'info'               => 'Gerencie as configurações do sistema em um único lugar.',
            'title'              => 'Configurações do sistema',
            'back'               => 'Voltar',
            'search-placeholder' => 'Pesquisar configurações',
            'save-btn'           => 'Salvar',
            'save-message'       => 'Configurações salvas com sucesso.',
            'system'             => [
                'title' => 'Sistema',
                'info'  => 'Configurações principais da aplicação.',
            ],
            'email'              => [
                'title' => 'E-mail',
                'info'  => 'Remetente SMTP e credenciais.',
            ],
            'debug'              => [
                'title' => 'Depuração',
                'info'  => 'Depuração e registro da aplicação.',
            ],
            'microsoft-sso'      => [
                'title' => 'Microsoft SSO',
                'info'  => 'Logon único da Microsoft Entra ID para o acesso do administrador.',
            ],
        ],

        'appearance' => [
            'title'          => 'Appearance',
            'info'           => 'Logotipo e favicon.',
            'section-title'  => 'Visual Branding',
            'section-info'   => 'Customize how the admin panel looks by uploading your own logo and favicon. Changes preview instantly and apply once saved.',
            'logo'           => 'Logo',
            'logo-size'      => 'Recommended image resolution: 192px X 50px',
            'favicon'        => 'Favicon',
            'favicon-size'   => 'Recommended image resolution: 16px X 16px',
            'no-logo'        => 'No logo uploaded yet.',
            'no-favicon'     => 'No favicon uploaded yet.',
            'upload-cta'     => 'Click to upload',
            'upload-hint'    => ' or drag and drop',
            'save-btn'       => 'Save Appearance',
            'update-success' => 'Appearance updated successfully.',
        ],
        'roles' => [
            'index' => [
                'create-btn' => 'Criar Função',
                'title'      => 'Funções',
                'datagrid'   => [
                    'delete'          => 'Excluir',
                    'edit'            => 'Editar',
                    'id'              => 'ID',
                    'name'            => 'Nome',
                    'permission-type' => 'Tipo de Permissão',
                ],
            ],
            'create' => [
                'access-control'     => 'Controle de Acesso',
                'all'                => 'Todos',
                'all-description'    => 'Concede acesso total a todas as permissões.',
                'back-btn'           => 'Voltar',
                'custom'             => 'Personalizado',
                'custom-description' => 'Selecione permissões individuais na árvore abaixo.',
                'description'        => 'Descrição',
                'general'            => 'Geral',
                'name'               => 'Nome',
                'permissions'        => 'Permissões',
                'save-btn'           => 'Salvar Função',
                'title'              => 'Criar Função',
            ],
            'edit' => [
                'access-control'     => 'Controle de Acesso',
                'all'                => 'Todos',
                'all-description'    => 'Concede acesso total a todas as permissões.',
                'back-btn'           => 'Voltar',
                'custom'             => 'Personalizado',
                'custom-description' => 'Selecione permissões individuais na árvore abaixo.',
                'description'        => 'Descrição',
                'general'            => 'Geral',
                'name'               => 'Nome',
                'permissions'        => 'Permissões',
                'save-btn'           => 'Salvar Função',
                'title'              => 'Editar Função',
            ],
            'being-used'                      => 'Função já está sendo usada por um usuário administrador',
            'being-used-by'                   => 'Função já está sendo usada por :name',
            'cannot-grant-unheld-permissions' => 'Você não pode conceder permissões que não possui.',
            'create-success'                  => 'Função criada com sucesso',
            'delete-failed'                   => 'Falha ao excluir a função',
            'delete-success'                  => 'Função excluída com sucesso',
            'last-delete-error'               => 'A última função não pode ser excluída',
            'update-success'                  => 'Função atualizada com sucesso',
        ],
    ],
    'configuration' => [
        'index' => [
            'delete'          => 'Excluir',
            'no-result-found' => 'Nenhum resultado encontrado',
            'save-btn'        => 'Salvar Configuração',
            'save-message'    => 'Configuração salva com sucesso',
            'search'          => 'Buscar',
            'title'           => 'Configuração',
            'emails'          => [
                'info'      => 'Configure the mail sender identity and SMTP transport used for outgoing emails.',
                'title'     => 'Email',
                'configure' => [
                    'info'           => '',
                    'title'          => 'Configure',
                    'email-settings' => [
                        'title'           => 'Email Settings',
                        'info'            => 'Sender identity and SMTP server credentials for outgoing mail. Leave SMTP fields blank to keep the values from your environment file.',
                        'sender-name'     => 'Sender Name',
                        'sender-email'    => 'Sender Email',
                        'admin-name'      => 'Admin Name',
                        'admin-email'     => 'Admin Email',
                        'smtp-host'       => 'SMTP Host',
                        'smtp-port'       => 'SMTP Port',
                        'smtp-username'   => 'SMTP Username',
                        'smtp-password'   => 'SMTP Password',
                        'smtp-encryption' => 'SMTP Encryption',
                    ],
                ],
            ],
            'general' => [
                'info'    => '',
                'title'   => 'Geral',
                'general' => [
                    'info'  => '',
                    'title' => 'Geral',
                ],
                'debug' => [
                    'info'     => 'Enable verbose debug output for specific IP addresses without exposing it to everyone.',
                    'title'    => 'Debug',
                    'settings' => [
                        'info'        => 'When enabled, requests from the allow-listed IPs render with debug output and the debug bar.',
                        'title'       => 'Debug Settings',
                        'enabled'     => 'Enable IP-based Debug',
                        'allowed-ips' => 'Allowed IP Addresses (comma-separated)',
                    ],
                ],
                'microsoft-sso' => [
                    'info'  => 'Configure o logon único da Microsoft (Entra ID) para o acesso do administrador.',
                    'title' => 'Microsoft SSO',

                    'settings' => [
                        'info'                => 'Registre um aplicativo no Microsoft Entra ID e cole as credenciais dele aqui. Deixe em branco para usar os valores do ambiente. URI de redirecionamento: /admin/login/sso/microsoft/callback.',
                        'title'               => 'Configurações do Microsoft SSO',
                        'enabled'             => 'Ativar Microsoft SSO',
                        'tenant'              => 'Tenant ID',
                        'client-id'           => 'Client ID',
                        'client-secret'       => 'Client Secret',
                        'redirect-uri'        => 'URI de redirecionamento',
                        'redirect-uri-info'   => 'Registre exatamente esta URL no registro do aplicativo do Microsoft Entra ID em Autenticação → URIs de redirecionamento, usando a plataforma Web.',
                        'redirect-uri-copy'   => 'Copiar',
                        'redirect-uri-copied' => 'Copiado para a área de transferência',
                    ],
                ],

                'magic-ai' => [
                    'info'     => 'Defina as opções do Magic AI.',
                    'title'    => 'Magic AI',
                    'settings' => [
                        'enabled'         => 'Habilitado',
                        'ai-platform'     => 'Plataforma padrão',
                        'ai-model'        => 'Modelo padrão',
                        'max-tokens'      => 'Máximo de tokens de saída',
                        'max-tokens-info' => 'Limite padrão do quanto uma única geração de IA pode produzir. A saída HTML gasta tokens com marcação, então aumente este valor se o conteúdo gerado for cortado no meio da frase.',
                        'title'           => 'Configurações Gerais',
                        'title-info'      => 'Melhore sua experiência com o Magic AI inserindo sua chave API exclusiva e especificando a organização pertinente para integração sem problemas. Controle suas credenciais OpenAI e ajuste as configurações conforme suas necessidades específicas.',
                    ],
                    'image-generation' => [
                        'enabled'     => 'Ativado',
                        'ai-platform' => 'Plataforma padrão',
                        'ai-model'    => 'Modelo padrão',
                        'title'       => 'Geração de imagens',
                        'title-info'  => 'Configure a plataforma e modelo de IA padrão para gerar imagens de produtos. Apenas plataformas que suportam geração de imagens (OpenAI, Gemini, xAI) estão listadas.',
                    ],
                    'translation' => [
                        'title'                       => 'Tradução',
                        'enabled'                     => 'Habilitado',
                        'title-info'                  => 'Este recurso ativará o Magic AI para traduzir textos, permitindo que você converta textos de um idioma para outro de forma fácil e fluida.',
                        'ai-platform'                 => 'Plataforma padrão',
                        'global-source-channel'       => 'Canal de Origem',
                        'global-source-locale'        => 'Localidade de Origem',
                        'translation-model'           => 'Modelo de Tradução',
                        'replace-existing-value'      => 'Substituir Valor Existente',
                        'target-channel'              => 'Canal de Destino',
                        'target-locales'              => 'Localidades de Destino',
                        'replace-existing-value-info' => 'Substituir o valor existente se ele já existir.',
                    ],
                    'agentic-pim' => [
                        'title'                     => 'PIM Agêntico',
                        'title-info'                => 'Configure o chat do agente IA, os fluxos de trabalho de enriquecimento autônomo e o monitoramento de qualidade. Esses recursos permitem o gerenciamento conversacional de produtos e a manutenção proativa do catálogo.',
                        'enabled'                   => 'Ativar chat do agente IA',
                        'enabled-info'              => 'Ative o widget de chat IA flutuante para gerenciamento conversacional de produtos com mais de 27 ações de ferramentas.',
                        'open-by-default'           => 'Abrir o painel de chat ao carregar a página',
                        'open-by-default-info'      => 'Quando ativado, o painel de chat do Agente de IA abre automaticamente na primeira vez que um administrador acessa uma página. Os usuários ainda podem fechá-lo manualmente — a preferência é lembrada pelo restante da sessão do navegador.',
                        'max-steps'                 => 'Etapas máx. do agente por turno',
                        'max-steps-info'            => 'Número máximo de iterações de chamadas de ferramentas que o agente IA pode executar por mensagem do usuário. Valores mais altos permitem operações de várias etapas mais complexas.',
                        'daily-token-budget'        => 'Orçamento diário de tokens',
                        'daily-token-budget-info'   => 'Tokens IA máximos (entrada + saída) que podem ser consumidos por dia em todos os usuários. Defina como 0 para ilimitado. Ajuda a controlar os custos de API.',
                        'auto-enrichment'           => 'Auto-enriquecimento na criação de produto',
                        'auto-enrichment-info'      => 'Gere automaticamente descrições, categorias e conteúdo SEO ausentes quando novos produtos são criados ou importados.',
                        'quality-monitor'           => 'Monitor de qualidade do catálogo',
                        'quality-monitor-info'      => 'Execute varreduras programadas para detectar produtos incompletos, traduções ausentes, entradas duplicadas e anomalias de preços.',
                        'confidence-threshold'      => 'Limite de confiança',
                        'confidence-threshold-info' => 'Pontuação mínima de confiança (0-1) para que alterações geradas por IA sejam aplicadas automaticamente. Abaixo desse limite, as alterações são colocadas em fila para revisão humana.',
                        'approval-mode'             => 'Modo de aprovação de alterações',
                        'approval-mode-info'        => 'Controla como o agente IA lida com operações de escrita. "Confirmar e aplicar": o agente propõe valores no chat, aguarda confirmação e depois executa. "Confirmação rigorosa": o mesmo, mas também verifica após cada alteração. "Apenas sugerir": descreve as alterações, mas nunca executa (somente leitura).',
                        'custom-instructions'       => 'Instruções personalizadas do agente',
                        'custom-instructions-info'  => 'Instruções adicionais anexadas ao prompt de sistema do agente de IA — use para convenções da empresa, tom ou regras de catálogo que o agente deve sempre seguir.',
                    ],
                ],
            ],
        ],
        'integrations' => [
            'index' => [
                'create-btn' => 'Criar',
                'title'      => 'Integrações',
                'datagrid'   => [
                    'delete'          => 'Excluir',
                    'edit'            => 'Editar',
                    'id'              => 'ID',
                    'name'            => 'Nome',
                    'user'            => 'Usuário',
                    'client-id'       => 'ID do Cliente',
                    'permission-type' => 'Tipo de Permissão',
                ],
            ],
            'create' => [
                'access-control' => 'Controle de Acesso',
                'all'            => 'Todos',
                'back-btn'       => 'Voltar',
                'custom'         => 'Personalizado',
                'general'        => 'Geral',
                'name'           => 'Nome',
                'permissions'    => 'Permissões',
                'save-btn'       => 'Salvar',
                'title'          => 'Nova Integração',
            ],
            'edit' => [
                'access-control'              => 'Controle de Acesso',
                'all'                         => 'Todos',
                'back-btn'                    => 'Voltar',
                'custom'                      => 'Personalizado',
                'general'                     => 'Geral',
                'name'                        => 'Nome',
                'credentials'                 => 'Credenciais',
                'client-id'                   => 'ID do Cliente',
                'secret-key'                  => 'Chave Secreta',
                'api-username'                => 'Nome de usuário da API',
                'api-password'                => 'Senha da API',
                'credentials-once-note'       => 'Essas credenciais são exibidas apenas uma vez. Guarde-as com segurança — a senha não poderá ser recuperada posteriormente.',
                'credentials-info'            => 'Use estas credenciais para autenticar sua integração.',
                'password-forgot-note'        => 'Esqueceu a senha? Gere uma nova aqui — a nova senha é exibida apenas uma vez.',
                'copy'                        => 'Copiar',
                'copied'                      => 'Copiado para a área de transferência',
                'regenerate-password-success' => 'Senha da API regenerada com sucesso.',
                'regenerate-password-btn'     => 'Gerar nova senha',
                'api-docs-link'               => 'Ver documentação da API',
                'generate-btn'                => 'Gerar',
                're-secret-btn'               => 'Regenerar Chave Secreta',
                'permissions'                 => 'Permissões',
                'save-btn'                    => 'Salvar',
                'title'                       => 'Editar Integração',
            ],
            'being-used'                     => 'A Integração API já está sendo usada pelo usuário Admin',
            'create-success'                 => 'Integração API criada com sucesso',
            'delete-failed'                  => 'Integração API não excluída',
            'delete-success'                 => 'Integração API excluída com sucesso',
            'last-delete-error'              => 'A última Integração API não pode ser excluída',
            'update-success'                 => 'Integração API atualizada com sucesso',
            'generate-key-success'           => 'Chave API gerada com sucesso',
            're-generate-secret-key-success' => 'Chave secreta da API regenerada com sucesso',
            'client-not-found'               => 'Cliente não encontrado',
        ],
        'prompt' => [
            'index' => [
                'title' => 'Prompts',
            ],
            'datagrid' => [
                'id'               => 'ID',
                'title'            => 'Título',
                'prompt'           => 'Prompt',
                'type'             => 'Tipo',
                'purpose'          => 'Finalidade',
                'text-generation'  => 'Geração de texto',
                'image-generation' => 'Geração de imagens',
                'translation'      => 'Tradução',
                'created-at'       => 'Criado em',
                'updated-at'       => 'Atualizado em',
                'actions'          => 'Ações',
                'edit'             => 'Editar',
                'delete'           => 'Excluir',
                'product'          => 'Produto',
                'category'         => 'Categoria',
            ],
            'create' => [
                'title'               => 'Prompt',
                'create-btn'          => 'Criar Prompt',
                'edit-title'          => 'Editar Prompt',
                'create-title'        => 'Criar Novo Prompt',
                'label-title'         => 'Título',
                'purpose'             => 'Finalidade',
                'text-generation'     => 'Geração de texto',
                'image-generation'    => 'Geração de imagens',
                'select-purpose'      => 'Selecionar finalidade',
                'select-model'        => 'Selecionar modelo',
                'loading-models'      => 'Carregando modelos...',
                'no-models-available' => 'Nenhum modelo disponível. Selecione uma plataforma ou configure uma na aba Plataformas.',
                'type'                => 'Tipo',
                'prompt'              => 'Prompt',
                'save-btn'            => 'Salvar Prompt',
            ],
            'message' => [
                'save-success'   => 'Prompt salvo com sucesso.',
                'update-success' => 'Prompt atualizado com sucesso.',
                'delete-success' => 'Prompt excluído com sucesso.',
                'delete-fail'    => 'Falha ao excluir o prompt.',
            ],
        ],
        'system-prompt' => [
            'index' => [
                'title' => 'Prompts do sistema',
            ],
            'datagrid' => [
                'id'          => 'ID',
                'title'       => 'Título',
                'tone'        => 'Tom',
                'max-tokens'  => 'Tokens máximos',
                'temperature' => 'Temperatura',
                'status'      => 'Status',
                'created-at'  => 'Criado em',
                'updated-at'  => 'Atualizado em',
                'actions'     => 'Ações',
                'save'        => 'Salvar',
                'edit'        => 'Editar',
                'delete'      => 'excluir',
            ],
            'create' => [
                'title'               => 'Prompt do sistema',
                'create-btn'          => 'Criar prompt do sistema',
                'edit-title'          => 'Editar prompt do sistema',
                'create-title'        => 'Criar novo prompt do sistema',
                'label-title'         => 'Título',
                'max-tokens'          => 'Tokens de saída máximos',
                'temperature'         => 'Temperatura',
                'type'                => 'Tipo',
                'prompt'              => 'Prompt',
                'save-btn'            => 'Salvar prompt',
                'prompt-tone'         => 'Tom do prompt',
                'max-tokens-info'     => 'Intervalo máximo permitido de tokens de saída: 100 a 5000 tokens.',
                'temperature-info'    => 'A temperatura controla a criatividade. Intervalo: 0 a 2. Valores mais baixos (por exemplo, 0,4) geram respostas mais precisas e focadas.',
                'max-tokens-tooltip'  => 'Intervalo permitido de tokens de saída máximos: de 100 a 5000 tokens',
                'temperature-tooltip' => 'A temperatura controla a criatividade. Intervalo: de 0 a 2. Valores mais baixos (ex.: 0,4) fornecem respostas mais precisas e focadas.',
            ],
            'message' => [
                'save-success'   => 'Prompt do sistema salvo com sucesso.',
                'update-success' => 'Prompt do sistema atualizado com sucesso.',
                'delete-success' => 'Prompt do sistema excluído com sucesso.',
                'delete-fail'    => 'Falha ao excluir o prompt do sistema.',
            ],
        ],
        'platform' => [
            'title'           => 'Plataformas de IA',
            'create-btn'      => 'Adicionar plataforma',
            'create-title'    => 'Adicionar plataforma de IA',
            'edit-title'      => 'Editar plataforma de IA',
            'save-btn'        => 'Salvar',
            'saving'          => 'Salvando',
            'test-connection' => 'Testar conexão',
            'setup'           => [
                'title'              => 'Comece a usar o Magic AI',
                'description'        => 'Conecte seu provedor de IA para desbloquear a geração de conteúdo, criação de imagens e tradução com IA. Adicione sua primeira plataforma para começar.',
                'step-1'             => 'Escolha um provedor',
                'step-2'             => 'Insira sua chave API',
                'step-3'             => 'Selecione modelos e salve',
                'add-first'          => 'Adicione sua primeira plataforma',
                'no-default-warning' => 'Nenhuma plataforma padrão foi definida. Defina uma plataforma como padrão para habilitar os recursos de IA em toda a aplicação.',
                'no-platform-hint'   => 'Nenhuma plataforma configurada ainda. Vá até a aba Plataformas para adicionar seu primeiro provedor de IA.',
                'lightning-icon'     => 'Magic AI',
                'warning-icon'       => 'Aviso',
            ],

            'errors' => [
                'no-models-returned' => 'Nenhum modelo retornado. Adicione modelos manualmente abaixo.',
                'fetch-failed'       => 'Falha ao buscar. Você pode adicionar modelos manualmente abaixo.',
                'save-failed'        => 'Falha ao salvar. Tente novamente.',
                'generic'            => 'Ocorreu um erro.',
            ],
            'testing'         => 'Testando',
            'fetch-models'    => 'Buscar modelos',
            'fetching-models' => 'Buscando',
            'set-default'     => 'Definir como padrão',
            'managed-badge'   => 'Gerenciada',
            'datagrid'        => [
                'label'      => 'Rótulo',
                'provider'   => 'Provedor',
                'models'     => 'Modelos',
                'default'    => 'Padrão',
                'status'     => 'Status',
                'created-at' => 'Criado em',
                'edit'       => 'Editar',
                'delete'     => 'Excluir',
            ],
            'fields' => [
                'label'                    => 'Rótulo',
                'provider'                 => 'Provedor',
                'select-provider'          => 'Selecionar provedor',
                'api-url'                  => 'URL da API',
                'api-url-hint'             => 'Preenchido com o endpoint padrão. Edite apenas se estiver usando um proxy ou implantação personalizada.',
                'api-key'                  => 'Chave API',
                'models'                   => 'Modelos',
                'custom-model-placeholder' => 'Digite um ID de modelo personalizado...',
                'add'                      => 'Adicionar',
                'enter-key-to-fetch'       => 'Insira sua chave API acima para buscar automaticamente os modelos disponíveis.',
                'search-models'            => 'Buscar modelos...',
                'no-models-match'          => 'Nenhum modelo corresponde à sua busca.',
                'azure-deployment'         => 'Nome da implantação',
                'azure-api-version'        => 'Versão da API',
                'is-default'               => 'Definir como padrão',
                'status'                   => 'Status',
                'use-default'              => 'Usar plataforma padrão',
                'use-default-hint'         => 'Deixe vazio para usar a plataforma marcada como padrão. Plataformas marcadas com * são as padrões.',
                'no-image-platform'        => 'Nenhuma plataforma com capacidade de imagem configurada. Adicione primeiro uma plataforma OpenAI, Gemini ou xAI.',
                'image-platform-hint'      => 'Apenas plataformas com suporte a geração de imagens são listadas (OpenAI, Gemini, xAI).',
                'remove-model'             => 'Remover modelo :model',
                'invalid-model-name'       => 'Nome de modelo inválido. Use apenas letras, números, hifens, pontos, dois pontos e barras (ex. gpt-4o, claude-3-sonnet).',
            ],
            'message' => [
                'save-success'                       => 'Plataforma salva com sucesso.',
                'update-success'                     => 'Plataforma atualizada com sucesso.',
                'delete-success'                     => 'Plataforma excluída com sucesso.',
                'delete-fail'                        => 'Falha ao excluir a plataforma.',
                'not-found'                          => 'A plataforma selecionada não existe mais. Atualize a página e tente novamente.',
                'cannot-delete-default'              => 'Não é possível excluir a única plataforma padrão.',
                'set-default-success'                => 'Plataforma definida como padrão com sucesso.',
                'test-success'                       => 'Conexão verificada com sucesso!',
                'test-fail'                          => 'Falha no teste de conexão',
                'no-test-model'                      => 'Selecione pelo menos um modelo compatível com texto para executar o teste de conexão.',
                'custom-api-url-required'            => 'A API URL é obrigatória para provedores personalizados para que o teste de conexão alcance o seu próprio endpoint.',
                'unsafe-api-url'                     => 'A API URL aponta para um endereço restrito ou inválido e não pode ser usada.',
                'extras-reserved-key'                => 'O campo de extras não pode substituir :keys. Use os campos de URL da API e chave da API.',
                'extras-invalid-json'                => 'O campo de extras deve conter um objeto JSON válido.',
                'extras-too-large'                   => 'O campo de extras é muito grande.',
                'custom-provider-error'              => 'Erro do provedor personalizado',
                'ai-response-timeout'                => 'A resposta da IA está demorando mais que o esperado. Tente novamente.',
                'no-model-configured'                => 'Nenhum modelo configurado para a plataforma selecionada.',
                'no-platform-configured'             => 'Nenhuma plataforma de IA configurada. Adicione uma plataforma em Configuração > Magic AI > Plataformas.',
                'images-unsupported'                 => 'O provedor :provider não oferece suporte à geração de imagens. Use OpenAI, Gemini ou xAI.',
                'api-key-corrupted'                  => 'A chave de API armazenada não pôde ser descriptografada (:error). A chave de criptografia do aplicativo pode ter sido alterada. Insira novamente sua chave de API e salve.',
                'fetch-models-fail'                  => 'Não foi possível buscar os modelos',
                'fetch-models-http-error'            => 'O endpoint em :host retornou HTTP :status.',
                'fetch-models-unreachable'           => 'Não foi possível acessar o endpoint em :host.',
                'invalid-model-names'                => 'Nomes de modelo inválidos: :names. Os nomes de modelo devem começar com uma letra ou número e conter apenas letras, números, hifens, pontos, dois pontos e barras.',
                'managed-connection-locked'          => 'Esta chave de API só pode ser usada com o próprio provedor e endpoint.',
                'managed-models-only'                => 'Esta chave de API só pode usar estes modelos: :allowed.',
                'managed-not-configured'             => 'A plataforma de IA gerenciada não está configurada. Defina MAGIC_AI_MANAGED_MODELS.',
                'managed-provisioned'                => 'A plataforma de IA gerenciada ":label" está pronta.',
                'managed-cannot-delete'              => 'A plataforma gerenciada não pode ser excluída.',
                'managed-key-prompt'                 => 'Chave de API da plataforma gerenciada (deixe em branco para manter a chave salva)',
                'managed-key-required'               => 'É necessária uma chave de API para criar a plataforma gerenciada. Execute o comando no modo interativo e informe a chave.',
                'default-requires-enabled'           => 'Uma plataforma desativada não pode ser definida como padrão. Ative a plataforma primeiro.',
            ],
        ],
    ],
    'components' => [
        'accordion' => [
            'expand'   => 'Expandir',
            'collapse' => 'Recolher',
        ],

        'associations' => [
            'type-search' => [
                'add-btn'            => 'Adicionar',
                'already-added'      => 'Adicionado',
                'empty-info'         => 'Tente uma pesquisa diferente.',
                'empty-title'        => 'Nenhum tipo de associação encontrado',
                'search-placeholder' => 'Pesquisar por nome ou código',
                'select-all'         => 'Selecionar tudo',
                'title'              => 'Adicionar tipo de associação',
            ],
            'product-picker' => [
                'add-btn'  => 'Adicionar selecionados',
                'selected' => ':count produtos selecionados',
                'title'    => 'Selecionar produtos',
            ],
        ],
        'pagination' => [
            'page' => 'Página',
        ],
        'search' => [
            'placeholder' => 'Buscar',
        ],
        'flash-group' => [
            'close' => 'Fechar',
        ],
        'form' => [
            'translatable-field' => [
                'translated-count' => ':filled de :total traduzidos',
            ],
            'searchable-menu' => [
                'empty'  => 'Nenhum resultado',
                'search' => 'Pesquisar',
            ],
            'tags'            => [
                'placeholder'     => 'Digite ou cole valores separados por vírgulas ou novas linhas',
                'remove'          => 'Remover :value',
                'clear-all'       => 'Limpar tudo',
                'tag-placeholder' => 'Pressione Enter para adicionar',
            ],
            'ajax-error'      => 'Algo deu errado ao salvar. Por favor, tente novamente.',
            'file-uploader'   => [
                'upload-cta'  => 'Clique para enviar',
                'upload-hint' => 'ou arraste e solte',
            ],
            'unsaved-changes' => [
                'title'           => 'Você tem alterações não salvas',
                'sections'        => ':count seções modificadas',
                'fields'          => ':count campos modificados',
                'section'         => ':count seção modificada',
                'field'           => ':count campo modificado',
                'discard'         => 'Descartar',
                'save'            => 'Salvar alterações',
                'saving'          => 'Salvando…',
                'save-failed'     => 'Suas alterações não foram salvas. Revise os campos destacados.',
                'field-badge'     => 'Não salvo',
                'leave-message'   => 'Sair desta página? Suas alterações não salvas serão perdidas.',
                'stay'            => 'Permanecer na página',
                'leave'           => 'Sair',
                'discard-title'   => 'Descartar alterações?',
                'discard-message' => 'Isto reverterá todas as suas alterações não salvas neste formulário.',
                'cancel'          => 'Cancelar',
            ],
        ],
        'layouts' => [
            'side-rail' => [
                'collapse' => 'Recolher painel',
                'expand'   => 'Expandir painel',
            ],
            'breadcrumbs' => [
                'label' => 'Trilha de navegação',
            ],

            'header' => [
                'logo'          => 'Logotipo',
                'account-title' => 'Conta',
                'app-version'   => 'Versão : :version',
                'logout'        => 'Sair',
                'my-account'    => 'Minha Conta',
                'notifications' => 'Notificações',
                'visit-shop'    => 'Visitar Loja',
                'theme-auto'    => 'Theme: Auto',
                'theme-dark'    => 'Theme: Dark',
                'theme-light'   => 'Theme: Light',
            ],
            'sidebar' => [
                'attribute-families' => 'Famílias de Atributos',
                'attribute-groups'   => 'Grupos de Atributos',
                'attributes'         => 'Atributos',
                'history'            => 'História',
                'edit-section'       => 'Dados',
                'general'            => 'Geral',
                'catalog'            => 'Catálogo',
                'categories'         => 'Categorias',
                'category_fields'    => 'Campos de Categoria',
                'association_types'  => 'Tipos de Associação',
                'channels'           => 'Canais',
                'collapse'           => 'Reduzir',
                'configure'          => 'Configuração',
                'currencies'         => 'Moedas',
                'dashboard'          => 'Painel',
                'data-transfer'      => 'Transferência de Dados',
                'groups'             => 'Grupos',
                'help'               => 'Ajuda',
                'tracker'            => 'Rastreador de Tarefas',
                'imports'            => 'Importações',
                'exports'            => 'Exportações',
                'locales'            => 'Locais',
                'magic-ai'           => 'Magic AI',
                'mode'               => 'Modo Escuro',
                'products'           => 'Produtos',
                'roles'              => 'Papel',
                'settings'           => 'Configurações',
                'system'             => 'System',
                'system-information' => 'Informações do sistema',
                'system-settings'    => 'Configurações do sistema',
                'smtp'               => 'SMTP Configuration',
                'debug'              => 'Debug',
                'appearance'         => 'Appearance',
                'themes'             => 'Temas',
                'users'              => 'Usuários',
                'integrations'       => 'Integrações',
                'prompt'             => 'Prompt',
                'general-tab'        => 'Configuração',
            ],
        ],
        'datagrid' => [
            'index' => [
                'no-records-selected'              => 'Nenhum registro selecionado.',
                'must-select-a-mass-action-option' => 'Você deve selecionar uma opção de ação em massa.',
                'must-select-a-mass-action'        => 'Você deve selecionar uma ação em massa.',
                'link-copied'                      => 'Link copiado para a área de transferência.',
                'copy-failed'                      => 'Falha ao copiar. Por favor, copie o link manualmente.',
            ],
            'toolbar' => [
                'length-of'           => ':length de',
                'of'                  => 'de',
                'per-page'            => 'Por Página',
                'results'             => ':total Resultados',
                'selected'            => ':total Selecionados',
                'select-all-matching' => 'Selecionar todos :total',
                'all-selected'        => 'Todos os registros correspondentes selecionados',
                'clear-selection'     => 'Limpar seleção',
                'selection-options'   => 'Opções de seleção',
                'mass-actions'        => [
                    'submit'        => 'Enviar',
                    'select-option' => 'Selecionar Opção',
                    'select-action' => 'Selecionar Ação',
                ],
                'filter' => [
                    'title' => 'Filtro',
                ],
                'search_by' => [
                    'code'        => 'Buscar por código',
                    'code_or_id'  => 'Buscar por código ou id',
                    'sku_or_user' => 'Buscar por SKU ou usuário',
                ],
                'search' => [
                    'title' => 'Buscar',
                ],
                'manage-columns' => [
                    'title' => 'colunas',
                ],
                'pagination' => [
                    'first-page'    => 'Primeira página',
                    'previous-page' => 'Página anterior',
                    'next-page'     => 'Próxima página',
                    'last-page'     => 'Última página',
                    'page-number'   => 'Número da página',
                ],
            ],
            'filters' => [
                'select'          => 'Selecionar',
                'title'           => 'Aplicar Filtros',
                'save'            => 'Aplicar',
                'add-filter'      => 'Adicionar filtro',
                'back'            => 'Voltar',
                'done'            => 'Concluído',
                'remove-filter'   => 'Remover filtro',
                'search-filter'   => 'Pesquisar...',
                'no-value'        => 'Todos',
                'values-selected' => ':count selecionados',
                'saved-filters'   => [
                    'title'        => 'Filtros salvos',
                    'none'         => 'Nenhum filtro salvo',
                    'unsaved'      => 'Alterações não salvas',
                    'clear'        => 'Limpar todos os filtros',
                    'name'         => 'Nome do filtro',
                    'share'        => 'Compartilhar com todos os administradores',
                    'save'         => 'Salvar filtro',
                    'saved'        => 'Filtro salvo',
                    'deleted'      => 'Filtro excluído',
                    'shared'       => 'Compartilhado',
                    'delete'       => 'Excluir filtro',
                ],
                'dropdown'        => [
                    'searchable' => [
                        'atleast-two-chars' => 'Digite pelo menos 2 caracteres...',
                        'no-results'        => 'Nenhum resultado encontrado...',
                    ],
                ],
                'custom-filters' => [
                    'clear-all' => 'Limpar Todos',
                    'clear'     => 'Limpar',
                    'title'     => 'Filtros Personalizados',
                ],
                'boolean-options' => [
                    'false' => 'Falso',
                    'true'  => 'Verdadeiro',
                ],
                'date-options' => [
                    'last-month'        => 'Último Mês',
                    'last-six-months'   => 'Últimos 6 Meses',
                    'last-three-months' => 'Últimos 3 Meses',
                    'this-month'        => 'Este Mês',
                    'this-week'         => 'Esta Semana',
                    'this-year'         => 'Este Ano',
                    'today'             => 'Hoje',
                    'yesterday'         => 'Ontem',
                ],
            ],
            'table' => [
                'actions'              => 'Ações',
                'no-records-available' => 'Nenhum Registro Disponível.',
                'no-records-filtered'  => 'Nenhum registro corresponde aos filtros aplicados.',
                'thumbnail'            => 'Miniatura',
            ],
            'manage-columns' => [
                'title'             => 'Gerenciar colunas',
                'available-columns' => 'Colunas disponíveis',
                'selected-columns'  => 'Colunas selecionadas',
                'apply-btn'         => 'Aplicar',
            ],
        ],
        'modal' => [
            'confirm' => [
                'agree-btn'    => 'Concordo',
                'disagree-btn' => 'Discordo',
                'message'      => 'Você tem certeza que deseja executar esta ação?',
                'title'        => 'Você tem certeza?',
            ],
            'delete' => [
                'agree-btn'    => 'Excluir',
                'disagree-btn' => 'Cancelar',
                'message'      => 'Você tem certeza que deseja excluir?',
                'title'        => 'Confirmar Exclusão',
            ],
            'history' => [
                'title'           => 'Prévia da História',
                'subtitle'        => 'Reveja rapidamente suas atualizações e mudanças.',
                'close-btn'       => 'Fechar',
                'version-label'   => 'Versão',
                'date-time-label' => 'Data/Hora',
                'user-label'      => 'Usuário',
                'name-label'      => 'Chave',
                'old-value-label' => 'Valor Antigo',
                'new-value-label' => 'Novo Valor',
                'no-history'      => 'Histórico não encontrado',
            ],
        ],
        'products' => [
            'search' => [
                'search-by-sku' => 'Buscar por SKU',
                'add-btn'       => 'Adicionar Produto Selecionado',
                'empty-info'    => 'Nenhum produto disponível para o termo de pesquisa.',
                'empty-title'   => 'Nenhum produto encontrado',
                'product-image' => 'Imagem do Produto',
                'qty'           => ':qty Disponíveis',
                'sku'           => 'SKU - :sku',
                'title'         => 'Selecionar Produtos',
            ],
        ],
        'media' => [
            'images' => [
                'add-image-btn'      => 'Adicionar Imagem',
                'drag-drop-hint'     => 'Arraste e solte ou clique para enviar',
                'add-media-btn'      => 'Adicionar Mídia',
                'generate-with-ai'   => 'Gerar com IA',
                'upload-from-device' => 'Fazer upload do dispositivo',
                'cancel'             => 'Cancelar',
                'ai-add-image-btn'   => 'Magia AI',
                'ai-btn-info'        => 'Gerar Imagem',
                'allowed-types'      => 'gif, jfif, jif, jpeg, jpg, pdf, png, psd, tif, tiff, webp, bmp',
                'not-allowed-error'  => 'Apenas arquivos de imagem (.jpeg, .jpg, .png, ..) são permitidos.',
                'upload-media-btn'   => 'Fazer upload de mídia',
                'preview-image'      => 'Pré-visualizar imagem',
                'close-preview'      => 'Fechar pré-visualização',
                'replace-image'      => 'Substituir imagem',
                'delete-image'       => 'Excluir imagem',
                'ai-generation'      => [
                    '1024x1024'        => '1024x1024',
                    '1024x1792'        => '1024x1792',
                    '1792x1024'        => '1792x1024',
                    'apply'            => 'Aplicar',
                    'dall-e-2'         => 'Dall.E 2',
                    'dall-e-3'         => 'Dall.E 3',
                    'generate'         => 'Gerar',
                    'generating'       => 'Gerando...',
                    'hd'               => 'HD',
                    'model'            => 'Modelo',
                    'number-of-images' => 'Número de Imagens',
                    'prompt'           => 'Prompt',
                    'quality'          => 'Qualidade',
                    'regenerate'       => 'Regenerar',
                    'regenerating'     => 'Regenerando...',
                    'size'             => 'Tamanho',
                    'standard'         => 'Padrão',
                    'title'            => 'Geração de Imagem AI',
                ],
                'placeholders' => [
                    'front'     => 'Frente',
                    'next'      => 'Próximo',
                    'size'      => 'Tamanho',
                    'use-cases' => 'Casos de Uso',
                    'zoom'      => 'Zoom',
                ],
            ],
            'image-viewer' => [
                'actual-size'   => 'Tamanho real',
                'close'         => 'Fechar',
                'fit-to-screen' => 'Ajustar à tela',
                'reset-all'     => 'Redefinir',
                'rotate-left'   => 'Girar à esquerda',
                'rotate-right'  => 'Girar à direita',
                'zoom-in'       => 'Ampliar',
                'zoom-out'      => 'Reduzir',
            ],

            'gallery' => [
                'not-allowed-error' => 'Apenas arquivos de imagem e vídeo são permitidos. (.mp4, .jpg ..)',
            ],
            'videos' => [
                'add-video-btn'     => 'Adicionar Vídeo',
                'allowed-types'     => 'mp4, webm, mkv',
                'not-allowed-error' => 'Apenas arquivos de vídeo (.mp4, .mov, .ogg ..) são permitidos.',
            ],
            'files' => [
                'add-file-btn'        => 'Adicionar Arquivo',
                'allowed-types'       => 'csv, doc, docx, mp3, pdf, ppt, pptx, rtf, svg, txt, wav',
                'not-allowed-error'   => 'Apenas arquivos PDF são permitidos.',
                'preview-unavailable' => 'A pré-visualização não está disponível para este tipo de arquivo.',
            ],
        ],
        'tinymce' => [
            'ai-btn-tile'   => 'Magia AI',
            'ai-generation' => [
                'apply'                  => 'Aplicar',
                'generate'               => 'Gerar',
                'generated-content'      => 'Conteúdo Gerado',
                'generated-content-info' => 'Conteúdo gerado pela AI pode ser enganoso. Revise o conteúdo gerado antes de aplicá-lo.',
                'generating'             => 'Gerando...',
                'prompt'                 => 'Prompt',
                'title'                  => 'Assistência AI',
                'platform'               => 'Plataforma',
                'model'                  => 'Modelo',
                'rich-preview'           => 'Pré-visualização rica',
                'system-prompt'          => 'Prompt do sistema',
                'select-system-prompt'   => 'Selecionar prompt do sistema',
                'tone-instructions'      => 'Tom / Instruções',
                'max-tokens'             => 'Tokens máximos',
                'temperature'            => 'Temperatura',
                'truncated'              => 'O modelo atingiu o limite de tokens, então este conteúdo pode estar cortado. Aumente o máximo de tokens e gere novamente.',
                'gpt-3-5-turbo'          => 'OpenAI gpt-3.5-turbo',
                'llama2'                 => 'Llama 2',
                'mistral'                => 'Mistral',
                'dolphin-phi'            => 'Dolphin Phi',
                'phi'                    => 'Phi-2',
                'starling-lm'            => 'Starling',
                'llama2-uncensored'      => 'Llama 2 uncensored',
                'llama2:13b'             => 'Llama 2 13B',
                'llama2:70b'             => 'Llama 2 70B',
                'orca-mini'              => 'Orca Mini',
                'vicuna'                 => 'Vicuna',
                'llava'                  => 'LLaVA',
                'default-prompt'         => 'Prompt padrão',
                'select-prompt-template' => 'Selecione um modelo de prompt',
            ],
        ],
    ],
    'help' => [
        'index' => [
            'title'       => 'Ajuda e recursos',
            'subtitle'    => 'Tudo o que você precisa para aproveitar ao máximo o UnoPim — hospedagem, suporte e serviços profissionais, além de extensões e documentação para desenvolvedores.',
            'services'    => 'Serviços',
            'resources'   => 'Recursos e documentação',
            'system-info' => 'Informações do sistema',
        ],
        'system-info' => [
            'title'    => 'Informações do sistema',
            'subtitle' => 'Ambiente de execução, configuração do servidor e pacotes instalados para esta instância do UnoPim.',
            'enabled'  => 'Ativado',
            'disabled' => 'Desativado',
            'package'  => 'Pacote',

            'search-packages' => 'Pesquisar pacotes...',
            'no-packages'     => 'Nenhum pacote corresponde à sua pesquisa.',
            'sections'        => [
                'application' => 'Aplicativo',
                'server'      => 'Servidor',
                'database'    => 'Banco de dados',
                'services'    => 'Serviços',
                'extensions'  => 'Extensões PHP',
                'packages'    => 'Pacotes instalados',
            ],
            'fields' => [
                'unopim_version'        => 'Versão do UnoPim',
                'laravel_version'       => 'Versão do Laravel',
                'php_version'           => 'Versão do PHP',
                'environment'           => 'Ambiente',
                'debug_mode'            => 'Modo de depuração',
                'timezone'              => 'Fuso horário',
                'locale'                => 'Localidade',
                'url'                   => 'URL do aplicativo',
                'operating_system'      => 'Sistema operacional',
                'server_software'       => 'Software do servidor',
                'php_sapi'              => 'PHP SAPI',
                'memory_limit'          => 'Limite de memória',
                'max_execution_time'    => 'Tempo máximo de execução',
                'upload_max_filesize'   => 'Tamanho máximo do arquivo de upload',
                'post_max_size'         => 'Tamanho máximo de POST',
                'connection'            => 'Conexão',
                'driver'                => 'Driver',
                'version'               => 'Versão',
                'cache_driver'          => 'Driver de cache',
                'queue_connection'      => 'Conexão da fila',
                'session_driver'        => 'Driver de sessão',
                'mail_mailer'           => 'Serviço de e-mail',
                'elasticsearch_enabled' => 'Elasticsearch',
            ],
        ],
        'cards' => [
            'cloud-hosting' => [
                'title'       => 'Hospedagem em nuvem',
                'description' => 'Hospedagem em nuvem gerenciada e econômica — experimente e coloque o UnoPim no ar na nuvem em minutos, totalmente otimizado e escalável.',
            ],
            'support' => [
                'title'       => 'Suporte e manutenção',
                'description' => 'Suporte técnico dedicado e planos de manutenção contínua para manter seu PIM seguro, atualizado e funcionando sem problemas.',
            ],
            'services' => [
                'title'       => 'Serviços pagos',
                'description' => 'Ajuda especializada para integração de módulos, personalização, migração de dados, atualizações de versão e desenvolvimento sob medida.',
            ],
            'extensions' => [
                'title'       => 'Extensões',
                'description' => 'Explore complementos oficiais e da comunidade para estender o UnoPim com novos conectores, canais e recursos.',
            ],
            'user-guide' => [
                'title'       => 'Guia do usuário',
                'description' => 'Guias para desenvolvedores, tutoriais e os artigos mais recentes para ajudar você a construir, configurar e se manter atualizado.',
            ],
            'api-docs' => [
                'title'       => 'Documentação da API',
                'description' => 'Referência completa da REST API com endpoints, autenticação e exemplos para integrar o UnoPim ao seu ambiente.',
            ],
        ],
        'cta' => [
            'title'  => 'Ainda precisa de ajuda?',
            'sub'    => 'Fale com a equipe do UnoPim sobre hospedagem, desenvolvimento personalizado ou qualquer outra coisa.',
            'button' => 'Fale conosco',
        ],
        'banners' => [
            'cloud' => [
                'tag'     => 'Hospedagem em nuvem',
                'message' => 'Coloque o UnoPim no ar em uma hospedagem em nuvem totalmente gerenciada — <b>planos rápidos, seguros e econômicos.</b>',
                'cta'     => 'Ver planos',
            ],
            'upgrade' => [
                'tag'     => 'Atualização disponível',
                'message' => 'Você está usando o <b>UnoPim v:version.</b> Uma nova versão está disponível — atualize para os recursos &amp; correções mais recentes.',
                'cta'     => 'Escolher serviço de atualização',
            ],
            'dont-show-again' => 'Não mostrar novamente',
            'dismissed'       => 'Preferência salva.',
        ],
    ],
    'acl' => [
        'passports'                => 'Passaportes de produto',
        'publish'                  => 'Publicar',
        'withdraw'                 => 'Retirar',
        'addresses'                => 'Endereços',
        'attribute-families'       => 'Famílias de Atributos',
        'attribute-groups'         => 'Grupos de Atributos',
        'attributes'               => 'Atributos',
        'cancel'                   => 'Cancelar',
        'catalog'                  => 'Catálogo',
        'categories'               => 'Categorias',
        'channels'                 => 'Canais',
        'configure'                => 'Configurar',
        'configuration'            => 'Configuração',
        'copy'                     => 'Copiar',
        'create'                   => 'Criar',
        'currencies'               => 'Moedas',
        'dashboard'                => 'Painel',
        'data-transfer'            => 'Transferência de Dados',
        'delete'                   => 'Excluir',
        'edit'                     => 'Editar',
        'email-templates'          => 'Modelos de E-mail',
        'events'                   => 'Eventos',
        'groups'                   => 'Grupos',
        'help'                     => 'Ajuda',
        'import'                   => 'Importar',
        'imports'                  => 'Importações',
        'invoices'                 => 'Faturas',
        'locales'                  => 'Localidades',
        'magic-ai'                 => 'Magia AI',
        'magic-ai-general'         => 'Geral',
        'magic-ai-prompt'          => 'Prompt',
        'magic-ai-system-prompt'   => 'Prompt do sistema',
        'marketing'                => 'Marketing',
        'newsletter-subscriptions' => 'Assinaturas de Newsletter',
        'note'                     => 'Nota',
        'orders'                   => 'Pedidos',
        'products'                 => 'Produtos',
        'promotions'               => 'Promoções',
        'refunds'                  => 'Reembolsos',
        'reporting'                => 'Relatórios',
        'reviews'                  => 'Avaliações',
        'roles'                    => 'Papéis',
        'appearance'               => 'Appearance',
        'sales'                    => 'Vendas',
        'search-seo'               => 'Busca & SEO',
        'search-synonyms'          => 'Sinônimos de Busca',
        'search-terms'             => 'Termos de Busca',
        'settings'                 => 'Configurações',
        'shipments'                => 'Envios',
        'sitemaps'                 => 'Mapas do Site',
        'subscribers'              => 'Inscritos',
        'smtp'                     => 'SMTP Configuration',
        'debug'                    => 'Debug',
        'system'                   => 'System',
        'system-information'       => 'Informações do sistema',
        'system-settings'          => 'Configurações do sistema',
        'tax-categories'           => 'Categorias de Impostos',
        'tax-rates'                => 'Taxas de Impostos',
        'taxes'                    => 'Impostos',
        'themes'                   => 'Temas',
        'integration'              => 'Integração',
        'url-rewrites'             => 'Reescritas de URL',
        'users'                    => 'Usuários',
        'user-management'          => 'Gerenciamento de usuários',
        'category_fields'          => 'Campos da Categoria',
        'association_types'        => 'Tipos de Associação',
        'association_type_fields'  => 'Campos de Tipo de Associação',
        'view'                     => 'Visualizar',
        'execute'                  => 'Executar',
        'history'                  => 'História',
        'restore'                  => 'Restaurar',
        'integrations'             => 'Integrações',
        'api'                      => 'API',
        'tracker'                  => 'Rastreamento de Tarefas',
        'exports'                  => 'Exportações',
        'mass-update'              => 'Atualização em massa',
        'mass-delete'              => 'Exclusão em massa',
        'bulk-edit'                => 'Edição em massa',
        'quick-export'             => 'Exportação rápida',
        'variant-structures'       => 'Estruturas de variantes',
        'completeness'             => 'Completude',
    ],
    'errors' => [
        'dashboard' => 'Painel',
        'go-back'   => 'Voltar',
        'support'   => 'Se o problema persistir, entre em contato conosco através do <a href=":link" class=":class">:email</a> para assistência.',
        '404'       => [
            'description' => 'Oops! A página que você está procurando está de férias. Parece que não conseguimos encontrar o que você está procurando.',
            'title'       => '404 Página não encontrada',
        ],
        '401' => [
            'description' => 'Oops! Parece que você não tem permissão para acessar esta página. Parece que faltam as credenciais necessárias.',
            'title'       => '401 Não autorizado',
            'message'     => 'A autenticação falhou devido a credenciais inválidas ou token expirado.',
        ],
        '403' => [
            'description' => 'Oops! Esta página está fora dos limites. Parece que você não tem permissão para ver este conteúdo.',
            'title'       => '403 Proibido',
            'message'     => 'Você não tem permissão para acessar esta página. Entre em contato com o administrador se acreditar que isso é um erro.',
        ],
        '413' => [
            'description' => 'Oops! Parece que você está tentando carregar um arquivo muito grande. Se você deseja carregar o mesmo, atualize a configuração do PHP.',
            'title'       => '413 Conteúdo muito grande',
        ],
        '419' => [
            'description' => 'Oops! Sua sessão expirou. Por favor, atualize a página e faça login novamente para continuar.',
            'title'       => '419 Sessão expirou',
        ],
        '405' => [
            'description' => 'Ops! O método de solicitação não é permitido para esta URL. Por favor, use o método correto ou retorne à página anterior.',
            'title'       => '405 Método Não Permitido',
        ],
        '429' => [
            'description' => 'Oops! Too many requests have been made in a short time. Please wait a moment and try again.',
            'title'       => '429 Too Many Requests',
        ],
        '500' => [
            'description' => 'Oops! Algo deu errado. Parece que estamos tendo dificuldades em carregar a página que você está procurando.',
            'title'       => '500 Erro interno do servidor',
        ],
        '503' => [
            'description' => 'Oops! Parece que estamos temporariamente fora do ar para manutenção. Verifique novamente em breve.',
            'title'       => '503 Serviço indisponível',
        ],
    ],
    'export' => [
        'csv'                  => 'CSV',
        'download'             => 'Baixar',
        'export'               => 'Exportação rápida',
        'no-records'           => 'Nada para exportar',
        'xls'                  => 'XLS',
        'xlsx'                 => 'XLSX',
        'with-media'           => 'Com mídia',
        'export-success'       => 'Exportação realizada com sucesso!',
        'error'                => 'Algo deu errado!',
        'product-not-selected' => 'Nenhum produto selecionado para exportação rápida',
    ],
    'validations' => [
        'slug-being-used' => 'Este slug está sendo usado em categorias ou produtos.',
        'slug-reserved'   => 'Este slug é reservado.',
        'invalid-locale'  => 'Localizações inválidas :locales',
    ],
    'footer' => [
        'copy-right' => 'Powered by <a href="https://unopim.com/" target="_blank">UnoPim</a>, Um Projeto Comunitário por <a href="https://webkul.com/" target="_blank">Webkul</a>',
    ],
    'emails' => [
        'dear'   => 'Caro :admin_name',
        'hello'  => 'Olá,',
        'thanks' => 'Se você precisar de qualquer tipo de ajuda, entre em contato conosco em <a href=":link" style=":style">:email</a>.<br/>Obrigado!',
        'admin'  => [
            'forgot-password' => [
                'description'    => 'Você está recebendo este e-mail porque recebemos um pedido de redefinição de senha para sua conta.',
                'greeting'       => 'Esqueceu a Senha!',
                'reset-password' => 'Redefinir Senha',
                'subject'        => 'E-mail de Redefinição de Senha',
            ],
        ],

        'data-transfer' => [
            'body' => 'O :type ":code" está :state.',
        ],
    ],
    'common' => [
        'yes'               => 'Sim',
        'no'                => 'No',
        'true'              => 'Verdadeiro',
        'false'             => 'Falso',
        'enable'            => 'Ativado',
        'disable'           => 'Desativado',
        'no-match-found'    => 'Nenhum resultado encontrado!',
        'unauthorized'      => 'You do not have permission to perform this action.',
        'file-size-exceeds' => 'The uploaded file exceeds the maximum allowed size of :max.',
    ],
    'default-prompt' => [
        'detailed' => [
            'title'  => 'Descrição Detalhada do Produto',
            'prompt' => 'Escreva uma descrição abrangente de @name, incluindo suas características, benefícios, especificações técnicas e instruções de uso.',
        ],
        'overview' => [
            'title'  => 'Visão Geral do Produto',
            'prompt' => 'Crie uma visão geral detalhada de @name, destacando suas principais características, vantagens e público-alvo como @brand e @color.',
        ],
        'features' => [
            'title'  => 'Características e Benefícios do Produto',
            'prompt' => 'Liste as principais características e benefícios de @name, explicando como eles agregam valor à vida do cliente.',
        ],
        'technical' => [
            'title'  => 'Especificações Técnicas',
            'prompt' => 'Forneça uma lista detalhada de especificações técnicas de @name, incluindo dimensões, materiais e compatibilidade.',
        ],
        'care' => [
            'title'  => 'Cuidados e Manutenção',
            'prompt' => 'Escreva um guia sobre como cuidar e manter @name, incluindo dicas para limpeza, armazenamento e solução de problemas.',
        ],
        'tagline' => [
            'title'  => 'Slogan',
            'prompt' => 'Crie um slogan atraente e conciso para @name que capture sua essência e benefícios.',
        ],
        'summary' => [
            'title'  => 'Resumo do Produto',
            'prompt' => 'Resuma as principais características e benefícios de @name em 50-60 palavras.',
        ],
        'headline' => [
            'title'  => 'Manchete do Produto',
            'prompt' => 'Crie uma manchete chamativa para @name que destaque seu diferencial.',
        ],
        'brief' => [
            'title'  => 'Descrição Breve',
            'prompt' => 'Escreva uma descrição breve e concisa de @name, focando em suas principais vantagens e público-alvo.',
        ],
        'elevator' => [
            'title'  => 'Elevator Pitch',
            'prompt' => 'Crie um pitch rápido para @name, resumindo seu propósito, benefícios e diferencial em 30-40 palavras.',
        ],
    ],
];
