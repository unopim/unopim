<?php

return [
    'users' => [
        'sessions' => [
            'subtitle'                   => 'Logga in på ditt konto för att fortsätta',
            'tagline'                    => 'Hantera all din produktinformation på ett ställe',
            'remember-me'                => 'Kom ihåg mig',
            'toggle-password'            => 'Visa eller dölj lösenord',
            'too-many-attempts'          => 'För många inloggningsförsök. Försök igen om en stund.',
            'email'                      => 'E-postadress',
            'forget-password-link'       => 'Glömt lösenord?',
            'password'                   => 'Lösenord',
            'sso-divider'                => 'or',
            'sso-sign-in-with-microsoft' => 'Sign in with Microsoft',
            'submit-btn'                 => 'Logga in',
            'title'                      => 'Logga in',
            'powered-by'                 => 'Drivs av :unopim,',
            'open-source-project-by'     => 'Ett open-source-projekt av :webkul.',
        ],
        'forget-password' => [
            'create' => [
                'subtitle'               => 'Ange din e-postadress så skickar vi en länk för att återställa lösenordet.',
                'email'                  => 'Registrerad e-post',
                'email-not-exist'        => 'E-postadress finns inte',
                'page-title'             => 'Glömt lösenord',
                'reset-link-sent'        => 'Om ett konto med den e-postadressen finns, har en länk för att återställa lösenordet skickats.',
                'email-settings-error'   => 'E-post kunde inte skickas. Kontrollera din e-postkonfiguration',
                'sign-in-link'           => 'Tillbaka till inloggning?',
                'submit-btn'             => 'Återställ',
                'title'                  => 'Återställ lösenord',
                'powered-by'             => 'Drivs av :unopim,',
                'open-source-project-by' => 'Ett open-source-projekt av :webkul.',
            ],
        ],
        'reset-password' => [
            'subtitle'               => 'Välj ett nytt lösenord för ditt konto.',
            'invalid-link'           => 'Den här länken för lösenordsåterställning är ogiltig eller har upphört att gälla.',
            'back-link-title'        => 'Tillbaka till inloggning?',
            'confirm-password'       => 'Bekräfta lösenord',
            'email'                  => 'Registrerad e-post',
            'password'               => 'Lösenord',
            'submit-btn'             => 'Återställ lösenord',
            'title'                  => 'Återställ lösenord',
            'powered-by'             => 'Drivs av :unopim,',
            'open-source-project-by' => 'Ett open-source-projekt av :webkul.',
        ],
    ],
    'notifications' => [
        'description-text' => 'Lista över alla meddelanden',
        'marked-success'   => 'Meddelande markerat som läst',
        'no-record'        => 'Inget fyndat',
        'of'               => 'av',
        'read-all'         => 'Markera alla som lästa',
        'showing'          => 'Visar',
        'title'            => 'Meddelanden',
        'view-all'         => 'Visa alla',
        'status'           => [
            'all'        => 'Alla',
            'canceled'   => 'Avbruten',
            'closed'     => 'Stängd',
            'completed'  => 'Slutförd',
            'pending'    => 'Väntar',
            'processing' => 'Bearbetning',
            'read'       => 'Lästa',
            'unread'     => 'Olästa',
        ],
    ],
    'account' => [
        'edit' => [
            'back-btn'                 => 'Tillbaka',
            'change-password'          => 'Ändra lösenord',
            'confirm-password'         => 'Bekräfta lösenord',
            'current-password'         => 'Aktuellt lösenord',
            'current-password-info'    => 'Krävs endast när du anger ett nytt lösenord. Bekräfta ändringen med lösenordet för kontot du är inloggad med.',
            'email'                    => 'E-postadress',
            'general'                  => 'Allmänt',
            'invalid-password'         => 'Det aktuella lösenordet är felaktigt.',
            'name'                     => 'Namn',
            'password-whitespace'      => 'Lösenordet kan inte bestå av enbart mellanslag.',
            'password'                 => 'Lösenord',
            'gravatar-preview'         => 'Använder din Gravatar.com-bild',
            'use-gravatar'             => 'Använd Gravatar-bild',
            'use-gravatar-info'        => 'Visas när ingen profilbild har laddats upp. Skapa eller ändra den på :link.',
            'profile-image'            => 'Profilbild',
            'save-btn'                 => 'Spara konto',
            'title'                    => 'Mitt konto',
            'ui-locale'                => 'UI-lokal',
            'update-success'           => 'Kontot uppdaterat framgångsrikt',
            'upload-image-info'        => 'Ladda upp en profilbild (110px X 110px), eller använd din Gravatar.com-bild kopplad till din e-post.',
            'user-timezone'            => 'Tidszon',
            'catalog-locale'           => 'Katalogspråk',
            'catalog-locale-info'      => 'Språket ditt katalog­innehåll är skrivet på. Detta är skilt från gränssnittsspråket ovan.',
            'default-channel'          => 'Standardkanal',
        ],
    ],
    'dashboard' => [
        'invalid-type' => 'Ogiltig instrumentpanelswidgettyp.',
        'index'        => [
            'title'                => 'Instrumentbräda',
            'user-info'            => 'Snabbövervakning, vad är viktat i ditt PIM',
            'user-name'            => 'Hej! :user_name',
            'catalog-details'      => 'Katalog',
            'total-families'       => 'Totala familjer',
            'total-attributes'     => 'Totala attribut',
            'total-groups'         => 'Totalt antal attributgrupper',
            'total-categories'     => 'Totala kategorier',
            'total-products'       => 'Totala produkter',
            'settings-details'     => 'Katalogstruktur',
            'total-locales'        => 'Totala lokaler',
            'total-currencies'     => 'Totala valutor',
            'total-channels'       => 'Totala kanaler',
            'completeness'         => 'Fullständighet',
            'active'               => 'Aktiv',
            'inactive'             => 'Inaktiv',
            'simple'               => 'Enkel',
            'configurable'         => 'Konfigurerbar',
            'product-stats'        => 'Produktstatistik',
            'product-type-dist'    => 'Produkttypfördelning',
            'products-by-status'   => 'Produkter efter status',
            'products-over-time'   => 'Produktaktivitet (Senaste 7 dagarna)',
            'legend-created'       => 'Skapad',
            'legend-updated'       => 'Uppdaterad',
            'recent-activity'      => 'Senaste aktivitet',
            'no-activity'          => 'Ingen senaste aktivitet hittades.',
            'updated'              => 'uppdaterad',
            'created'              => 'skapad',
            'deleted'              => 'borttagen',
            'ago'                  => 'sedan',
            'just-now'             => 'nyss',
            'data-transfer'        => 'Datatransfer',
            'no-jobs'              => 'Inga senaste import/export jobb hittades.',
            'job-type-import'      => 'Importera',
            'job-type-export'      => 'Exportera',
            'job-state-completed'  => 'Avslutad',
            'job-state-failed'     => 'Misslyckad',
            'job-state-processing' => 'Bearbetar',
            'job-state-pending'    => 'Väntande',
            'job-state-validated'  => 'Validerad',
            'rows-processed'       => 'Bearbetade rader',
            'errors'               => 'Fel',
            'quick-links'          => 'Snabblänkar',
            'create-product'       => 'Skapa produkt',
            'import-data'          => 'Importera data',
            'export-data'          => 'Exportera data',
            'manage-attributes'    => 'Hantera attribut',
            'view-all-activity'    => 'Visa allt',
            'view-all-jobs'        => 'Visa alla jobb',
            'new-this-week'        => 'Ny den här veckan',
            'with-variants'        => 'Med varianter',
            'avg-completeness'     => 'Genomsnittlig fullständighet',
            'enriched'             => 'Berikad',
            'analytics-section'    => 'Analys',
            'operations-section'   => 'Operationer',
            'needs-attention'      => 'Behöver uppmärksamhet',
            'unenriched-products'  => 'Obehandlade produkter',
            'failed-jobs-24h'      => 'Misslyckade jobb (24h)',
            'low-completeness'     => 'Låg fullständighet',
            'all-good'             => 'Allt ser bra ut!',
            'channel-readiness'    => 'Kanalberedskap',
            'ready-to-publish'     => 'redo att publicera',
            'of'                   => 'of',
            'products-ready'       => 'produkter redo',
            'no-readiness-data'    => 'Ingen fullständighetsdata tillgänglig ännu.',
            'no-products'          => 'Inga produkter ännu.',
            'last-7-days'          => 'Senaste 7 dagarna',
            'days'                 => [
                'sun' => 'Sön',
                'mon' => 'Mån',
                'tue' => 'Tis',
                'wed' => 'Ons',
                'thu' => 'Tor',
                'fri' => 'Fre',
                'sat' => 'Lör',
            ],
            'entity-types'         => [
                'product'          => 'Produkt',
                'category'         => 'Kategori',
                'attribute'        => 'Attribut',
                'attribute-family' => 'Familj',
                'attribute-group'  => 'Grupp',
                'category-field'   => 'Kategorifält',
                'channel'          => 'Kanal',
                'role'             => 'Roll',
                'job-instance'     => 'Jobb',
                'webhook'          => 'Webhook',
                'api-key'          => 'API-nyckel',
            ],
        ],
    ],
    'catalog' => [
        'products' => [
            'upload-failure' => 'Media kunde inte hittas.',
            'index'          => [
                'already-taken'             => ':name är redan i bruk.',
                'variant-sku-already-taken' => 'Variantens SKU ":sku" används redan.',
                'variant-sku-label'         => 'SKU för variant :position',
                'create-btn'                => 'Skapa produkt',
                'title'                     => 'Produkter',
                'magic-ai-error'            => 'Kontrollera Magic AI-uppgifterna och se till att rätt modell är vald.',
                'magic-ai-validate-error'   => 'Vänligen kontrollera Magic AI-referenserna.',
                'magic-ai-validate-success' => 'Magic AI-referenserna validerades framgångsrikt.',
                'create'                    => [
                    'back-btn'                          => 'Tillbaka',
                    'configurable-attributes'           => 'Konfigurerbara attribut',
                    'create-btn'                        => 'Skapa produkt',
                    'family'                            => 'Familj',
                    'next-btn'                          => 'Nästa',
                    'save-btn'                          => 'Spara produkt',
                    'sku'                               => 'SKU',
                    'title'                             => 'Skapa ny produkt',
                    'type'                              => 'Typ',
                    'variant-structure'                 => 'Variantstruktur',
                    'not-config-family-error'           => 'Denna familj har inga konfigurerbara attribut',
                    'no-variant-structure'              => 'Denna familj har ingen variantstruktur. Skapa en innan du lägger till en konfigurerbar produkt.',
                    'invalid-variant-structure'         => 'Den valda variantstrukturen tillhör inte denna familj.',
                    'not-config-super-attributes-error' => 'Superattributet ":super_attributes" hittades inte',
                    'sku-exists-error'                  => 'SKU ":sku" används redan',
                ],
                'datagrid' => [
                    'categories'              => 'Kategorier',
                    'active'                  => 'Aktiv',
                    'attribute-family'        => 'Attributfamilj',
                    'attribute-family-value'  => 'Attributfamilj - :attribute_family',
                    'category'                => 'Kategori',
                    'copy-of'                 => 'Kopia av :value',
                    'copy-of-slug'            => 'kopia-:value',
                    'delete'                  => 'Radera',
                    'edit'                    => 'Redigera',
                    'copy'                    => 'Kopiera',
                    'disable'                 => 'Inaktivera',
                    'id'                      => 'ID',
                    'id-value'                => 'ID - :id',
                    'image'                   => 'Bild',
                    'mass-delete-queued'      => 'De valda produkterna tas bort i bakgrunden',
                    'mass-delete-success'     => 'De valda produkterna har raderats',
                    'mass-update-queued'      => 'De valda produkterna uppdateras i bakgrunden',
                    'mass-update-success'     => 'De valda produkterna har uppdaterats',
                    'name'                    => 'Namn',
                    'out-of-stock'            => 'Slut på lager',
                    'price'                   => 'Pris',
                    'product-image'           => 'Produktbild',
                    'qty'                     => 'Kvantitet',
                    'qty-value'               => ':qty i lager',
                    'sku'                     => 'SKU',
                    'sku-value'               => 'SKU - :sku',
                    'status'                  => 'Status',
                    'type'                    => 'Typ',
                    'update-status'           => 'Uppdatera status',
                    'parent'                  => 'Förälder',
                    'parent-identifier'       => 'Förälder: :sku',
                    'created-at'              => 'Skapad',
                    'updated-at'              => 'Uppdaterad',
                ],
            ],
            'edit' => [
                'navigation' => 'Produktnavigering',
                'preview'    => 'Förhandsgranska',
                'remove'     => 'Ta bort',
                'save-btn'   => 'Spara produkt',
                'title'      => 'Redigera produkt',
                'categories' => [
                    'title'            => 'Kategorier',
                    'root-not-allowed' => 'Rotkategorin kan inte tilldelas en produkt.',
                ],

                'attribute-groups' => [
                    'label'              => 'Attributgrupp:',
                    'title'              => 'Attributgrupper',
                    'search-placeholder' => 'Sök grupper',
                    'empty'              => 'Inga attributgrupper matchar din sökning.',
                    'attributes-count'   => ':count attribut',
                    'loading'            => 'Laddar...',
                    'retry'              => 'Försök igen',
                    'load-more'          => 'Ladda fler',
                    'unsaved-title'      => 'Osparade ändringar',
                    'unsaved-message'    => 'Spara dina ändringar i :group innan du går vidare, eller släng dem.',
                    'save-and-continue'  => 'Spara och fortsätt',
                    'discard-and-leave'  => 'Släng och fortsätt',
                    'cancel'             => 'Avbryt',
                ],
                'images' => [
                    'info'  => 'Bilder måste ha upplösningen 560px X 609px',
                    'title' => 'Bilder',
                ],
                'videos' => [
                    'error' => ':attribute får inte vara större än :max kilobytes. Välj en mindre fil.',
                    'info'  => 'Maximal videostorlek är :size',
                    'title' => 'Videor',
                ],
                'links' => [
                    'related-products' => [
                        'empty-info' => 'Lägg till relaterade produkter.',
                        'info'       => 'Förutom den visade produkten kommer även relaterade produkter att visas.',
                        'title'      => 'Relaterade produkter',
                    ],
                    'up-sells' => [
                        'empty-info' => 'Lägg till uppsäljningsprodukter.',
                        'info'       => 'Kunderna kommer att se premiumprodukter eller högre kvalitet än den visade produkten.',
                        'title'      => 'Upp-säljprodukter',
                    ],
                    'cross-sells' => [
                        'empty-info' => 'Lägg till korsförsäljningsprodukter.',
                        'info'       => 'Förutom varan i kundvagnen kommer kunder att se andra produkter de kan köpa.',
                        'title'      => 'Korsförsäljningsprodukter',
                    ],
                    'add-btn'           => 'Lägg till',
                    'add-type-btn'      => 'Lägg till associationstyp',
                    'delete'            => 'Radera',
                    'empty-info'        => 'Lägg till produkter :type åt gången.',
                    'empty-title'       => 'Lägg till produkt',
                    'image-placeholder' => 'Produktbild',
                    'more-types'        => 'Fler',
                    'no-types-info'     => 'Använd ”Lägg till associationstyp” för att koppla den här produkten till en associationstyp.',
                    'no-types-title'    => 'Inga associationer tillagda',
                    'remove-product'    => 'Ta bort produkt',
                    'search-types'      => 'Sök associationstyper',
                    'sku'               => 'SKU - :sku',
                    'title'             => 'Produkter',
                ],
                'types' => [
                    'configurable' => [
                        'add-btn'                            => 'Lägg till variation',
                        'default'                            => 'Standard',
                        'delete-btn'                         => 'Radera',
                        'edit-btn'                           => 'Redigera',
                        'empty-info'                         => 'Skapa en produkt med flera kombinationer',
                        'empty-title'                        => 'Lägg till variation',
                        'image-placeholder'                  => 'Produktbild',
                        'info'                               => 'Konfigurerbara produkter baseras på alla möjliga kombinationer',
                        'qty'                                => ':qty Kvantitet',
                        'sku'                                => 'SKU - :sku',
                        'title'                              => 'Variation',
                        'variant-exists'                     => 'Denna variation eller SKU finns redan.',
                        'variant-attribute-option-not-found' => 'Variationens attributalternativ hittades inte :attributes',
                        'supper-attribute-not-found'         => 'Superattribut hittades inte :attribute',
                        'variant-given-exists'               => 'Denna variation finns redan :variants',
                        'variant-group-has-children'         => 'Den här variationsgruppen innehåller fortfarande variationer. Ta bort dem eller flytta dem till en annan grupp innan du raderar den.',
                        'variant-group-combination-exists'   => 'Det finns redan en variationsgrupp med :values under den här produkten. Välj en annan kombination.',
                        'variant-combination-exists'         => 'Det finns redan en variation med :values under den här produkten. Välj en annan kombination.',
                        'create'                             => [
                            'description'            => 'Beskrivning',
                            'name'                   => 'Namn',
                            'save-btn'               => 'Lägg till',
                            'title'                  => 'Lägg till variation',
                            'variant-already-exists' => 'Denna variation finns redan.',
                        ],
                        'edit' => [
                            'disabled'        => 'Inaktiverad',
                            'edit-info'       => 'För att uppdatera produktinformation, gå till sidan',
                            'edit-link-title' => 'Produktsida',
                            'enabled'         => 'Aktiverad',
                            'images'          => 'Bilder',
                            'name'            => 'Namn',
                            'price'           => 'Pris',
                            'quantities'      => 'Kvantiteter',
                            'save-btn'        => 'Spara',
                            'sku'             => 'SKU',
                            'status'          => 'Status',
                            'title'           => 'Produkt',
                            'weight'          => 'Vikt',
                        ],
                        'mass-edit' => [
                            'add-images'          => 'Lägg till bilder',
                            'apply-to-all-btn'    => 'Tillämpa för alla',
                            'apply-to-all-name'   => 'Tillämpa namn för alla variationer',
                            'apply-to-all-sku'    => 'Tillämpa SKU för alla variationer',
                            'apply-to-all-status' => 'Tillämpa status för alla variationer',
                            'apply-to-all-weight' => 'Tillämpa vikt för alla variationer',
                            'edit-inventories'    => 'Redigera inventarier',
                            'edit-names'          => 'Redigera namn',
                            'edit-prices'         => 'Redigera priser',
                            'edit-sku'            => 'Redigera SKU',
                            'edit-status'         => 'Redigera status',
                            'edit-weight'         => 'Redigera vikt',
                            'name'                => 'Namn',
                            'price'               => 'Pris',
                            'remove-images'       => 'Ta bort bilder',
                            'remove-variants'     => 'Ta bort variationer',
                            'select-action'       => 'Välj åtgärd',
                            'select-variants'     => 'Välj variationer',
                            'status'              => 'Status',
                            'variant-name'        => 'Variationens namn',
                            'variant-sku'         => 'Variationens SKU',
                            'weight'              => 'Vikt',
                        ],
                        'variant-inheritance' => [
                            'locked'              => 'Det här attributet kan uppdateras i :level-attributen.',
                            'level-common'        => 'gemensamma',
                            'level-sub_parent'    => 'underförälder',
                            'level-variant'       => 'variant',
                        ],
                    ],
                ],
                'translate' => [
                    'translate-btn'               => 'Översätt',
                    'title'                       => 'Översätt',
                    'source-channel'              => 'Källkanal',
                    'source-locale'               => 'Källspråk',
                    'locale'                      => 'Källspråk',
                    'translated-content'          => 'Översatt innehåll',
                    'translating'                 => 'Översätter',
                    'apply'                       => 'Tillämpa',
                    'back'                        => 'Tillbaka',
                    'cancel'                      => 'Avbryt',
                    'target-locales'              => 'Målspråk',
                    'target-channel'              => 'Målkanal',
                    'attributes'                  => 'Attribut',
                    'tranlated-job-processed'     => 'Uppgiften att spara översatt innehåll har bearbetats.',
                    'step'                        => 'Steg',
                    'select-source'               => 'Välj källa',
                    'select-target'               => 'Välj mål',
                    'first-step-title'            => 'Steg 1: Välj källkanal, språk och attribut',
                    'first-step-title-attributes' => 'Steg 1: Välj källkanal och språk',
                    'source-content'              => 'Källinnehåll',
                    'second-step-title'           => 'Steg 2: Välj målkanal och språk',
                    'target-content'              => 'Målinnehåll',
                    'next'                        => 'Nästa',
                    'empty-translation-data'      => 'Data är inte tillgänglig för översättning baserat på källkanal och språk',
                    'attributes-to-translate'     => 'Attribut att översätta',
                    'no-value'                    => 'Inget värde',
                    'change'                      => 'Ändra',
                    'overwrite-warning'           => 'Befintligt innehåll i de valda målspråken kommer att ersättas.',
                    'incomplete-source'           => 'Välj en källkanal, ett språk och minst ett attribut för att fortsätta.',
                ],
                'product-info' => [
                    'title'        => 'Produktinformation',
                    'status'       => 'Status',
                    'family'       => 'Familj',
                    'parent'       => 'Överordnad produkt',
                    'updated-at'   => 'Senast uppdaterad',
                    'created-at'   => 'Skapad datum',
                    'product-type' => 'Produkttyp',
                ],
                'sections' => [
                    'view'    => 'Visa',
                    'unsaved' => 'Osparade ändringar',
                ],

                'workspace' => [

                    'add-selected' => 'Lägg till valda',
                    'close'        => 'Stäng',
                    'clear-search' => 'Rensa sökning',
                    'categories'   => [
                        'subtitle'        => 'Tilldela denna produkt till katalogkategorier.',
                        'search'          => 'Sök kategorier…',
                        'searching'       => 'Söker…',
                        'no-results'      => 'Inga matchande kategorier hittades',
                        'load-more'       => 'Ladda fler',
                        'none-selected'   => 'Inga kategorier har valts ännu',
                        'and-more'        => 'och :count till',
                        'review-selected' => 'Visa valda kategorier',
                        'selected'        => 'vald',
                    ],
                    'associations' => [
                        'subtitle' => 'Länka relaterade, mersälj-, korsförsäljnings- och anpassade produkter.',
                        'linked'   => 'länkade produkter',
                    ],
                ],

                'more'         => 'Mer',
                'more-actions' => 'Fler åtgärder',
                'variations'   => [
                    'title'          => 'Variationer',
                    'common'         => 'Gemensamt',
                    'search'         => 'Sök',
                    'switching'      => 'Växlar...',
                    'variant-count'  => ':count variant',
                    'variants-count' => ':count varianter',
                    'complete-count' => ':done av :total slutförda',
                    'select'         => 'Välj :attribute',
                    'add-new-btn'    => 'Lägg till ny',
                    'add-new'        => 'Lägg till en ny :attribute',
                    'empty'          => 'Inga :attribute-varianter ännu.',
                    'loading'        => 'Läser in...',
                    'loading-more'   => 'Läser in fler...',
                    'sku'            => 'SKU',
                    'cancel'         => 'Avbryt',
                    'create'         => 'Skapa',
                    'creating'       => 'Skapar...',
                    'create-error'   => 'Det gick inte att skapa varianten',
                ],
            ],
            'bulk-edit' => [
                'validation' => [
                    'decimal'                    => 'Ange ett giltigt decimaltal.',
                    'number'                     => 'Ange ett giltigt heltal.',
                    'email'                      => 'Ange en giltig e-postadress.',
                    'url'                        => 'Ange en giltig URL.',
                    'regex'                      => 'Värdet matchar inte det krävs mönstret.',
                    'invalid-pattern'            => 'Ogiltigt anpassat mönster angivet.',
                    'numeric'                    => 'Värdet för det numeriska attributet ":attribute" måste vara ett giltigt tal.',
                    'select-attribute-or-family' => 'Välj minst ett attribut eller en attributfamilj.',
                    'duplicate-axis'             => 'Två varianter under samma överordnade produkt kan inte dela en axelkombination, så denna sparning kommer att avvisas.',
                    'failed'                     => 'Valideringen misslyckades.',
                ],
                'modal' => [
                    'title'            => 'Attribut för massredigering',
                    'btn-cancel'       => 'Avbryt',
                    'btn-proceed'      => 'Fortsätt',
                    'attribute-family' => 'Välj attributfamilj',
                ],
                'index' => [
                    'no-product' => 'Inga produkter valda för massredigering.',
                ],
                'filter' => [
                    'many-product'     => 'För många produkter har valts.',
                    'redirect-message' => 'Omdirigerar till vyn för massredigering.',
                ],
                'handle-save' => [
                    'edit-success' => 'Massredigeringen lyckades.',
                ],
                'id'                           => 'ID',
                'no-changes'                   => 'Inga ändringar att spara.',
                'locked-cell'                  => 'Tillhör en annan nivå – kan inte redigeras här.',
                'invalid-datetime'             => 'Ange ett giltigt datum och klockslag.',
                'resize-column'                => 'Dra för att ändra kolumnbredd',
                'success'                      => 'Uppgiften har utförts.',
                'fetch-failed'                 => 'Misslyckades med att hämta.',
                'action'                       => 'Massredigering',
                'description'                  => 'Redigera flera produkter samtidigt. Ändringar bearbetas i bakgrunden.',
                'gallery-preview'              => 'Galleri Förhandsgranskning',
                'img-preview'                  => 'Bildförhandsvisning',
                'no-image'                     => 'Ingen bild',
                'img-fail'                     => 'Misslyckades med att ladda upp bilden',
                'no-option'                    => 'Inga alternativ',
                'clear-selection'              => 'Rensa markering',
                'keyboard-shortcuts'           => 'Tangentbordsgenvägar',
                'shortcuts-navigation'         => 'Navigering',
                'shortcuts-editing'            => 'Redigering',
                'shortcuts-selection'          => 'Markering',
                'shortcuts-clipboard'          => 'Urklipp och fyllning',
                'shortcuts-move-cell'          => 'Flytta mellan celler',
                'shortcuts-move-down'          => 'Flytta ned / bekräfta redigering',
                'shortcuts-move-up'            => 'Flytta upp',
                'shortcuts-move-right-left'    => 'Flytta höger / vänster',
                'shortcuts-home-end'           => 'Första / sista kolumnen i raden',
                'shortcuts-ctrl-home-end'      => 'Första / sista cellen i rutnätet',
                'shortcuts-extend-selection'   => 'Utöka markering',
                'shortcuts-select-all'         => 'Markera alla celler',
                'shortcuts-enter-edit'         => 'Gå till redigeringsläge',
                'shortcuts-confirm-move-down'  => 'Bekräfta + flytta ned',
                'shortcuts-confirm-move-right' => 'Bekräfta + flytta höger',
                'shortcuts-escape-revert'      => 'Återställ värde + lämna redigering',
                'shortcuts-clear-cell'         => 'Rensa cell',
                'shortcuts-copy'               => 'Kopiera',
                'shortcuts-cut'                => 'Klipp ut',
                'shortcuts-paste'              => 'Klistra in',
                'shortcuts-fill-down'          => 'Fyll nedåt',
                'shortcuts-fill-right'         => 'Fyll åt höger',
                'shortcuts-undo'               => 'Ångra',
                'shortcuts-redo'               => 'Gör om',
                'shortcuts-help'               => 'Visa/dölj tangentbordsgenvägar',
            ],
            'create-success'                  => 'Produkten skapades framgångsrikt',
            'delete-failed'                   => 'Fel vid radering av produkt',
            'delete-success'                  => 'Produkten raderades framgångsrikt',
            'product-copied'                  => 'Produkten kopierades framgångsrikt',
            'saved-inventory-message'         => 'Produkten sparades framgångsrikt',
            'update-success'                  => 'Produkten uppdaterades framgångsrikt',
            'unique-validation'               => 'Detta värde används redan.',
            'invalid-type'                    => 'Produkttypen ":type" hittades inte',
            'product-not-found'               => 'Produkten med SKU ":sku" hittades inte',
            'parent-not-found'                => 'Föräldern med SKU ":sku" hittades inte',
            'upload-success'                  => 'Produktfilen laddades upp framgångsrikt.',
            'update-failure'                  => 'Produkten kunde inte uppdateras.',
            'immutable-fields'                => 'Följande fält kan inte ändras: :fields.',
            'variant-structure-not-found'     => 'Variantstrukturen med koden ":code" hittades inte i familjen ":family"',
            'variant-structure-axis-mismatch' => 'De angivna superattributen matchar inte axlarna i variantstrukturen ":code". Förväntat: :axes',
        ],
        'attributes' => [
            'index' => [
                'create-btn' => 'Skapa attribut',
                'title'      => 'Attribut',
                'datagrid'   => [
                    'channel-based'            => 'Kanalbaserat',
                    'code'                     => 'Kod',
                    'created-at'               => 'Skapad vid',
                    'delete'                   => 'Ta bort',
                    'edit'                     => 'Redigera',
                    'id'                       => 'ID',
                    'locale-based'             => 'Språkbaserat',
                    'mass-delete-success'      => 'Valda attribut har tagits bort',
                    'name'                     => 'Namn',
                    'required'                 => 'Obligatorisk',
                    'type'                     => 'Typ',
                    'unique'                   => 'Unik',
                    'delete-failed'            => 'Det gick inte att ta bort detta attribut.',
                    'mass-delete-failed'       => 'Det gick inte att ta bort de valda attributen.',
                    'mass-delete-partial'      => ':count attribut kunde inte tas bort eftersom de används i konfigurerbara produkter.',
                    'delete-attribute-failure' => 'Detta attribut kan inte tas bort eftersom det används för konfigurerbara produkter',
                ],
            ],
            'create' => [
                'add-attribute-options'  => 'Lägg till attributalternativ',
                'add-option'             => 'Lägg till alternativ',
                'add-options-info'       => 'Skapa kombinationer av alternativ för attributet.',
                'add-row'                => 'Lägg till rad',
                'admin'                  => 'Admin',
                'admin-name'             => 'Administratörens namn',
                'back-btn'               => 'Tillbaka',
                'boolean'                => 'Boolean',
                'checkbox'               => 'Checkbox',
                'code'                   => 'Kod',
                'color'                  => 'Färg',
                'configuration'          => 'Konfiguration',
                'create-empty-option'    => 'Skapa tom standardalternativ',
                'date'                   => 'Datum',
                'datetime'               => 'Datum och tid',
                'decimal'                => 'Decimal',
                'default-value'          => 'Standardvärde',
                'email'                  => 'E-post',
                'enable-wysiwyg'         => 'Aktivera WYSIWYG-redigerare',
                'file'                   => 'Fil',
                'general'                => 'Allmänt',
                'gallery'                => 'Galleri',
                'image'                  => 'Bild',
                'input-options'          => 'Inmatningsalternativ',
                'input-validation'       => 'Inmatningsvalidering',
                'is-comparable'          => 'Detta attribut är jämförbart',
                'is-configurable'        => 'Används för att skapa konfigurerbara produkter',
                'is-required'            => 'Obligatorisk',
                'is-unique'              => 'Unik',
                'is-visible-on-front'    => 'Synlig på produktens detaljsida (frontend)',
                'label'                  => 'Etikett',
                'multiselect'            => 'Multivälj',
                'no'                     => 'Nej',
                'number'                 => 'Nummer',
                'options'                => 'Alternativ',
                'position'               => 'Position',
                'price'                  => 'Pris',
                'regex'                  => 'Regex',
                'save-btn'               => 'Spara attribut',
                'select'                 => 'Välj',
                'select-type'            => 'Välj attributtyp',
                'swatch'                 => 'Prov Typ',
                'text'                   => 'Text',
                'textarea'               => 'Textområde',
                'title'                  => 'Lägg till attribut',
                'type'                   => 'Typ',
                'url'                    => 'URL',
                'use-in-flat'            => 'Skapa i den platta produkt-tabellen',
                'validations'            => 'Valideringar',
                'value-per-channel'      => 'Värde per kanal',
                'value-per-locale'       => 'Värde per språk',
                'yes'                    => 'Ja',
                'same-code-error'        => 'Alternativets kod måste vara unik.',
                'code-hint'              => 'Unik teknisk identifierare som används vid import, export och i API:t. Den kan inte ändras senare.',
                'type-hint'              => 'Avgör hur värden matas in och lagras. Den kan inte ändras senare.',
                'is-unique-hint'         => 'Varje produkt måste ha ett unikt värde för det här attributet.',
                'value-per-locale-hint'  => 'Spara ett eget värde för varje språk.',
                'value-per-channel-hint' => 'Spara ett eget värde för varje försäljningskanal.',
                'usable-in-grid'         => 'Kan användas i rutnät',
                'is-filterable'          => 'Är filtrerbar',
                'ai-translate'           => 'AI Översättning',
                'invalid-swatch-type'    => 'Attributet :attribute är inte tillåtet för attributtyp :type med swatchtyp :swatch_type.',
                'single-object-only'     => 'Varje skapandeförfrågan får endast innehålla ett attributobjekt.',
                'option'                 => [
                    'color'    => 'Färgväljare',
                    'dropdown' => 'Rullgardinsmeny',
                    'image'    => 'Bildprov',
                    'save-btn' => 'Spara alternativ',
                    'text'     => 'Textprov',
                ],
            ],
            'edit' => [
                'add-attribute-options' => 'Lägg till attributalternativ',
                'add-option'            => 'Lägg till alternativ',
                'add-options-info'      => 'Skapa kombinationer av alternativ för attributet.',
                'add-row'               => 'Lägg till rad',
                'admin'                 => 'Admin',
                'admin-name'            => 'Administratörens namn',
                'back-btn'              => 'Tillbaka',
                'boolean'               => 'Boolean',
                'checkbox'              => 'Checkbox',
                'code'                  => 'Kod',
                'color'                 => 'Färg',
                'configuration'         => 'Konfiguration',
                'create-empty-option'   => 'Skapa tom standardalternativ',
                'date'                  => 'Datum',
                'datetime'              => 'Datum och tid',
                'decimal'               => 'Decimal',
                'default-value'         => 'Standardvärde',
                'email'                 => 'E-post',
                'enable-wysiwyg'        => 'Aktivera WYSIWYG-redigerare',
                'file'                  => 'Fil',
                'general'               => 'Allmänt',
                'image'                 => 'Bild',
                'input-options'         => 'Inmatningsalternativ',
                'input-validation'      => 'Inmatningsvalidering',
                'is-comparable'         => 'Detta attribut är jämförbart',
                'is-configurable'       => 'Används för att skapa konfigurerbara produkter',
                'is-required'           => 'Obligatorisk',
                'is-unique'             => 'Unik',
                'is-visible-on-front'   => 'Synlig på produktens detaljsida (frontend)',
                'label'                 => 'Etikett',
                'multiselect'           => 'Multivälj',
                'no'                    => 'Nej',
                'number'                => 'Nummer',
                'options'               => 'Alternativ',
                'position'              => 'Position',
                'price'                 => 'Pris',
                'regex'                 => 'Regex',
                'save-btn'              => 'Spara attribut',
                'select'                => 'Välj',
                'select-type'           => 'Välj attributtyp',
                'swatch'                => 'Prov Typ',
                'text'                  => 'Text',
                'textarea'              => 'Textområde',
                'title'                 => 'Redigera attribut',
                'type'                  => 'Typ',
                'url'                   => 'URL',
                'use-in-flat'           => 'Skapa i den platta produkt-tabellen',
                'validations'           => 'Valideringar',
                'value-per-channel'     => 'Värde per kanal',
                'value-per-locale'      => 'Värde per språk',
                'yes'                   => 'Ja',
                'same-code-error'       => 'Alternativets kod måste vara unik.',
                'locked-after-creation' => 'Den här inställningen anges vid skapandet och kan inte ändras senare.',
                'usable-in-grid'        => 'Kan användas i rutnät',
                'is-filterable'         => 'Är filtrerbar',
                'ai-translate'          => 'AI Översättning',
                'option'                => [
                    'save-btn'             => 'Spara alternativ',
                    'create-success'       => 'Attributalternativ skapades',
                    'update-success'       => 'Attributalternativ uppdaterades',
                    'sort-update-success'  => 'Sorteringsordningen uppdaterades',
                    'sort-update-failure'  => 'Det gick inte att uppdatera sorteringsordningen',
                    'delete-success'       => 'Attributalternativ raderades',
                    'invalid-swatch-image' => 'Provbilden är ogiltig eller innehåller osäkert innehåll.',
                    'color'                => 'Färgväljare',
                    'dropdown'             => 'Rullgardinsmeny',
                    'image'                => 'Bildprov',
                    'text'                 => 'Textprov',
                ],
            ],
            'create-success'    => 'Attribut skapad',
            'delete-failed'     => 'Det gick inte att ta bort attributet',
            'delete-success'    => 'Attribut borttaget',
            'update-success'    => 'Attribut uppdaterad',
            'user-define-error' => 'Det går inte att ta bort systemattribut',
            'immutable-fields'  => 'Följande fält kan inte ändras: :fields.',
            'not-found'         => 'Attributet med koden ":code" hittades inte',
        ],
        'attribute-options' => [
            'delete-success'       => 'Attributalternativ har tagits bort',
            'swatch-not-found'     => 'Ingen färgrutebild hittades för detta alternativ.',
            'create-success'       => 'Attributalternativ skapades',
            'update-success'       => 'Attributalternativ uppdaterades',
            'usable-in-grid-error' => 'Endast 4 attribut kan markeras som användbara i rutan.',
        ],
        'attribute-groups' => [
            'index' => [
                'create-btn' => 'Skapa attributgrupp',
                'title'      => 'Attributgrupper',
                'datagrid'   => [
                    'code'                => 'Kod',
                    'delete'              => 'Radera',
                    'mass-delete-success' => 'Valda attributgrupper har raderats',
                    'edit'                => 'Redigera',
                    'id'                  => 'ID',
                    'inactive'            => 'Inaktiv',
                    'name'                => 'Namn',
                    'position'            => 'Position',
                    'update-status'       => 'Uppdatera status',
                ],
            ],
            'create' => [
                'title'    => 'Lägg till attributgrupp',
                'save-btn' => 'Spara attributgrupp',
                'back-btn' => 'Tillbaka',
                'general'  => 'Allmänt',
                'code'     => 'Kod',
                'label'    => 'Etikett',
            ],
            'edit' => [
                'title'    => 'Redigera attributgrupp',
                'back-btn' => 'Tillbaka',
                'save-btn' => 'Spara attributgrupp',
                'general'  => 'Allmänt',
                'code'     => 'Kod',
                'label'    => 'Etikett',
            ],
            'create-success'        => 'Attributgrupp skapad',
            'delete-failed'         => 'Misslyckades med att ta bort attributgrupp',
            'delete-success'        => 'Attributgrupp borttagen',
            'update-success'        => 'Attributgrupp uppdaterad',
            'user-define-error'     => 'Du kan inte ta bort systemattributgrupper',
            'attribute-group-error' => 'Gruppen används i familjer.',
            'not-found'             => 'Attributgrupp med kod ":code" hittades inte',
        ],
        'categories' => [
            'index' => [
                'add-btn'  => 'Skapa kategori',
                'title'    => 'Kategorier',
                'datagrid' => [
                    'active'         => 'Aktiv',
                    'code'           => 'Kod',
                    'delete'         => 'Radera',
                    'delete-success' => 'Valda resurser har raderats :resource',
                    'edit'           => 'Redigera',
                    'id'             => 'ID',
                    'inactive'       => 'Inaktiv',
                    'name'           => 'Visningsnamn',
                    'category-name'  => 'اسم',
                    'no-of-products' => 'Antal produkter',
                    'position'       => 'Position',
                    'status'         => 'Synlig i menyn',
                    'update-status'  => 'Uppdatera status',
                ],
            ],
            'browse' => [
                'categories-count'      => ':count kategorier',
                'trees'                 => 'Träd',
                'channel-root'          => 'Kanalrot',
                'subcategories-count'   => ':count underkategorier',
                'tree-view'             => 'Träd',
                'list-view'             => 'Lista',
                'add-root'              => 'Lägg till rotkategori',
                'add-category'          => 'Lägg till kategori',
                'add-child'             => 'Lägg till underkategori',
                'search-placeholder'    => 'Sök kategorier',
                'searching'             => 'Söker...',
                'no-results'            => 'Inga kategorier matchar din sökning.',
                'load-more'             => 'Ladda fler',
                'loading'               => 'Laddar...',
                'empty'                 => 'Inga kategorier ännu.',
                'new-category'          => 'Ny kategori',
                'root-level'            => 'Rotnivå',
                'roots-count'           => ':count träd',
                'expand-all'            => 'Expandera',
                'collapse-all'          => 'Fäll ihop',
                'expand-all-hint'       => 'Öppnar ytterligare en nivå av trädet.',
                'search-failed'         => 'Kategorierna kunde inte sökas.',
                'children-failed'       => 'Underkategorierna kunde inte laddas.',
                'descendants-too-large' => 'Den här kategorin har för många underkategorier för att kunna väljas alla samtidigt.',
            ],

            'create' => [
                'back-btn'            => 'Tillbaka',
                'code'                => 'Kod',
                'general'             => 'Allmänt',
                'meta-description'    => 'Meta beskrivning',
                'meta-keywords'       => 'Meta nyckelord',
                'meta-title'          => 'Meta titel',
                'name'                => 'Namn',
                'parent-category'     => 'Föräldrakategori',
                'position'            => 'Position',
                'save-btn'            => 'Spara kategori',
                'select-display-mode' => 'Välj visningsläge',
                'seo-details'         => 'SEO detaljer',
                'settings'            => 'Inställningar',
                'slug'                => 'Slug',
                'title'               => 'Lägg till ny kategori',
                'visible-in-menu'     => 'Synlig i menyn',
                'right-section'       => 'Inställningar',
                'left-section'        => 'Allmänt',
            ],
            'edit' => [
                'back-btn'               => 'Tillbaka',
                'code'                   => 'Kod',
                'display-mode'           => 'Visningsläge',
                'enter-position'         => 'Ange position',
                'general'                => 'Allmänt',
                'name'                   => 'Namn',
                'save-btn'               => 'Spara kategori',
                'select-display-mode'    => 'Välj visningsläge',
                'select-parent-category' => 'Välj föräldrakategori',
                'seo-details'            => 'SEO detaljer',
                'settings'               => 'Inställningar',
                'slug'                   => 'Slug',
                'title'                  => 'Redigera kategori',
                'visible-in-menu'        => 'Synlig i menyn',
                'right-section'          => 'Inställningar',
                'left-section'           => 'Allmänt',
            ],
            'category'             => 'Kategori',
            'create-success'       => 'Kategori skapad',
            'delete-category-root' => 'Du kan inte ta bort rotkategorin som är kopplad till en kanal.',
            'delete-failed'        => 'Misslyckades med att ta bort kategori',
            'delete-success'       => 'Kategori raderad',
            'update-success'       => 'Kategori uppdaterad',
            'can-not-update'       => 'Denna rotkategori är kopplad till en kanal och kan inte ha en föräldrakategori.',
            'invalid-parent'       => 'Den valda överordnade kategorin är ogiltig. En kategori kan inte vara sin egen förälder eller en ättling till sig själv.',
            'unique-validation'    => 'Detta värde är redan i användning.',
            'not-found'            => 'Kategori med kod ":code" hittades inte',
            'unknown-fields'       => 'Okänt fält för kategori med kod ":fields"',
            'upload-success'       => 'Kategori fil uppladdad',
            'upload-failure'       => 'Kategorifilen kunde inte laddas upp.',
            'create-failure'       => 'Kategorin kunde inte skapas.',
            'update-failure'       => 'Kategorin kunde inte uppdateras.',
        ],
        'category_fields' => [
            'index' => [
                'create-btn' => 'Skapa Kategori Fält',
                'add-btn'    => 'Skapa Kategori Fält',
                'title'      => 'Kategori Fält',
                'datagrid'   => [
                    'channel-based'       => 'Kanalbaserat',
                    'code'                => 'Kod',
                    'created-at'          => 'Skapad',
                    'delete'              => 'Radera',
                    'edit'                => 'Redigera',
                    'id'                  => 'ID',
                    'locale-based'        => 'Språkinställningar',
                    'mass-delete-success' => 'Valt kategori fält raderat framgångsrikt',
                    'name'                => 'Namn',
                    'required'            => 'Krävs',
                    'type'                => 'Typ',
                    'unique'              => 'Unik',
                    'disable'             => 'Inaktivera',
                    'active'              => 'Aktivera',
                    'activated'           => 'Aktiverad',
                    'disabled'            => 'Inaktiverad',
                    'update-status'       => 'Uppdatera status',
                    'mass-update-success' => 'Uppdaterad framgångsrikt',
                    'status'              => 'Status',
                    'position'            => 'Position',
                    'delete-failed'       => 'Detta kategori fält kan inte raderas.',
                    'mass-delete-failed'  => 'De valda kategori fälten kan inte raderas.',
                ],
            ],
            'create' => [
                'add-field-options'   => 'Lägg till alternativ till kategori fält',
                'add-option'          => 'Lägg till alternativ',
                'add-options-info'    => 'För att skapa flera alternativ på en gång.',
                'add-row'             => 'Lägg till rad',
                'admin'               => 'Admin',
                'admin-name'          => 'Administratörens namn',
                'back-btn'            => 'Tillbaka',
                'boolean'             => 'Boolean',
                'checkbox'            => 'Checkbox',
                'code'                => 'Kod',
                'color'               => 'Färg',
                'configuration'       => 'Konfiguration',
                'create-empty-option' => 'Skapa tomt standardalternativ',
                'date'                => 'Datum',
                'datetime'            => 'Datum och tid',
                'decimal'             => 'Decimal',
                'default-value'       => 'Standardvärde',
                'email'               => 'E-post',
                'enable-wysiwyg'      => 'Aktivera WYSIWYG editor',
                'file'                => 'Fil',
                'general'             => 'Allmänt',
                'image'               => 'Bild',
                'input-options'       => 'Inmatningsalternativ',
                'input-validation'    => 'Inmatningsvalidering',
                'is-required'         => 'Är obligatorisk',
                'is-unique'           => 'Är unik',
                'label'               => 'Etikett',
                'multiselect'         => 'Flerval',
                'no'                  => 'Nej',
                'number'              => 'Nummer',
                'options'             => 'Alternativ',
                'position'            => 'Position',
                'regex'               => 'Regex',
                'save-btn'            => 'Spara kategori fält',
                'select'              => 'Välj',
                'select-type'         => 'Välj typ',
                'swatch'              => 'Prov',
                'text'                => 'Text',
                'textarea'            => 'Textområde',
                'title'               => 'Lägg till kategori fält',
                'type'                => 'Typ',
                'url'                 => 'URL',
                'use-in-flat'         => 'Skapa i produkt tabell',
                'validations'         => 'Valideringar',
                'value-per-channel'   => 'Värde per kanal',
                'value-per-locale'    => 'Värde per språk',
                'yes'                 => 'Ja',
                'status'              => 'Status',
                'set-section'         => 'Visa sektion',
                'set-section-left'    => 'Allmän sektion',
                'set-section-right'   => 'Konfigurations sektion',
                'settings'            => 'Inställningar',
                'same-code-error'     => 'Alternativkod måste vara unik.',
                'option'              => [
                    'dropdown' => 'Rullgardinslista',
                    'save-btn' => 'Spara alternativ',
                    'color'    => 'Färgmönster',
                    'image'    => 'Bildmönster',
                    'text'     => 'Textmönster',
                ],
            ],
            'edit' => [
                'add-field-options'   => 'Lägg till alternativ till kategori fält',
                'add-option'          => 'Lägg till alternativ',
                'add-options-info'    => 'För att skapa flera alternativ på en gång.',
                'add-row'             => 'Lägg till rad',
                'admin'               => 'Admin',
                'admin-name'          => 'Administratörens namn',
                'back-btn'            => 'Tillbaka',
                'boolean'             => 'Boolean',
                'checkbox'            => 'Checkbox',
                'code'                => 'Kod',
                'color'               => 'Färg',
                'configuration'       => 'Konfiguration',
                'create-empty-option' => 'Skapa tomt standardalternativ',
                'date'                => 'Datum',
                'datetime'            => 'Datum och tid',
                'decimal'             => 'Decimal',
                'default-value'       => 'Standardvärde',
                'email'               => 'E-post',
                'enable-wysiwyg'      => 'Aktivera WYSIWYG editor',
                'file'                => 'Fil',
                'general'             => 'Allmänt',
                'image'               => 'Bild',
                'input-options'       => 'Inmatningsalternativ',
                'input-validation'    => 'Inmatningsvalidering',
                'is-filterable'       => 'Använd i filtrering',
                'is-required'         => 'Är obligatorisk',
                'is-unique'           => 'Är unik',
                'label'               => 'Etikett',
                'multiselect'         => 'Flerval',
                'no'                  => 'Nej',
                'number'              => 'Nummer',
                'options'             => 'Alternativ',
                'position'            => 'Position',
                'regex'               => 'Regex',
                'save-btn'            => 'Spara kategori fält',
                'select'              => 'Välj',
                'select-type'         => 'Välj typ',
                'swatch'              => 'Prov',
                'text'                => 'Text',
                'textarea'            => 'Textområde',
                'title'               => 'Redigera kategori fält',
                'type'                => 'Typ',
                'url'                 => 'URL',
                'use-in-flat'         => 'Skapa i produkt tabell',
                'validations'         => 'Valideringar',
                'value-per-channel'   => 'Värde per kanal',
                'value-per-locale'    => 'Värde per språk',
                'yes'                 => 'Ja',
                'status'              => 'Status',
                'set-section'         => 'Visa sektion',
                'set-section-left'    => 'Allmän sektion',
                'set-section-right'   => 'Konfigurations sektion',
                'settings'            => 'Inställningar',
                'same-code-error'     => 'Alternativkod måste vara unik.',
                'option'              => [
                    'dropdown' => 'Rullgardinslista',
                    'save-btn' => 'Spara alternativ',
                    'color'    => 'Färgmönster',
                    'image'    => 'Bildmönster',
                    'text'     => 'Textmönster',
                ],
            ],
            'create-success'    => 'Kategori fält skapades framgångsrikt',
            'delete-failed'     => 'Det gick inte att radera kategori fält',
            'delete-success'    => 'Kategori fält raderades framgångsrikt',
            'update-success'    => 'Kategori fält uppdaterades framgångsrikt',
            'user-define-error' => 'Det går inte att radera systemdefinierade fält',
            'not-found'         => 'Kategori fält med kod ":code" kunde inte hittas',
            'immutable-fields'  => 'Följande fält kan inte ändras: :fields.',
        ],
        'association_types' => [
            'index' => [
                'title'      => 'Associationstyper',
                'create-btn' => 'Skapa Associationstyp',
                'add-btn'    => 'Skapa Associationstyp',

                'datagrid' => [
                    'code'                => 'Kod',
                    'name'                => 'Namn',
                    'status'              => 'Status',
                    'position'            => 'Position',
                    'edit'                => 'Redigera',
                    'delete'              => 'Radera',
                    'activated'           => 'Aktiverad',
                    'disabled'            => 'Inaktiverad',
                    'active'              => 'Aktivera',
                    'disable'             => 'Inaktivera',
                    'update-status'       => 'Uppdatera status',
                    'delete-failed'       => 'Denna associationstyp kan inte raderas.',
                    'mass-delete-success' => 'Valda associationstyper raderade framgångsrikt',
                    'mass-delete-failed'  => 'De valda associationstyperna kan inte raderas.',
                    'mass-update-success' => 'Uppdaterad framgångsrikt',
                ],
            ],

            'create-success'    => 'Associationstyp skapades framgångsrikt',
            'update-success'    => 'Associationstyp uppdaterades framgångsrikt',
            'delete-success'    => 'Associationstyp raderades framgångsrikt',
            'delete-failed'     => 'Det gick inte att radera associationstyp',
            'user-define-error' => 'Det går inte att radera systemdefinierad associationstyp',
            'unknown-fields'    => 'Fält för associationstyp med kod ":fields" kunde inte hittas',
            'not-found'         => 'Associationstyp med kod ":code" hittades inte',
            'immutable-fields'  => 'Följande fält kan inte ändras: :fields.',

            'create' => [
                'title'      => 'Skapa associationstyp',
                'name'       => 'Namn',
                'enter-name' => 'Ange namn',
                'code'       => 'Kod',
                'enter-code' => 'Ange kod',
                'save-btn'   => 'Spara associationstyp',
            ],

            'edit' => [
                'title'    => 'Redigera associationstyp',
                'save-btn' => 'Uppdatera associationstyp',
            ],

            'fields' => [
                'title'                => 'Fält',
                'info'                 => 'Lägg till anpassade fält för att fånga ytterligare data för denna associationstyp.',
                'add-field-btn'        => 'Lägg till fält',
                'modal-title'          => 'Lägg till fält',
                'edit-modal-title'     => 'Redigera fält',
                'save-field-btn'       => 'Spara fält',
                'same-code-error'      => 'Fältkoden måste vara unik.',
                'add-fields-info'      => 'För att skapa anpassade fält för denna associationstyp.',
                'create-success'       => 'Associationstyp-fält skapades framgångsrikt',
                'update-success'       => 'Associationstyp-fält uppdaterades framgångsrikt',
                'delete-success'       => 'Associationstyp-fält raderades framgångsrikt',
                'not-found'            => 'Fält för associationstyp med kod ":code" kunde inte hittas',
            ],
        ],

        'category-fields-options' => [
            'delete-success'      => 'Kategorifältalternativ har tagits bort',
            'create-success'      => 'Kategoriens fältalternativ skapades',
            'update-success'      => 'Kategoriens fältalternativ uppdaterades',
            'update-unknown-code' => 'Det finns inget kategorifältsalternativ med koden ":code".',
        ],
        'families' => [
            'index' => [
                'add'      => 'Skapa attributgrupp',
                'title'    => 'Attributfamiljer',
                'create'   => [
                    'name'          => 'Namn',
                    'enter-name'    => 'Ange namn',
                    'title'         => 'Skapa attributgrupp',
                    'code'          => 'Kod',
                    'enter-code'    => 'Ange kod',
                    'based-on'      => 'Baserat på',
                    'select-family' => 'Välj en grupp att kopiera strukturen från',
                    'based-on-info' => 'Kopierar grupper, attribut och variantstruktur från den valda gruppen. Lämna tomt för att börja med en Allmän grupp som innehåller SKU.',
                    'save-btn'      => 'Spara attributgrupp',
                ],
                'datagrid' => [
                    'code'           => 'Kod',
                    'delete'         => 'Ta bort',
                    'delete-success' => 'Den valda resursen :resource har tagits bort',
                    'edit'           => 'Redigera',
                    'id'             => 'ID',
                    'method-error'   => 'Fel! Ogiltig metod detekterad, kontrollera inställningarna för massåtgärder',
                    'name'           => 'Namn',
                    'no-resource'    => 'Otillräckliga resurser för denna åtgärd',
                    'partial-action' => 'Vissa åtgärder kunde inte utföras på grund av systemrestriktioner för :resource',
                    'update-success' => 'Den valda resursen :resource har uppdaterats',
                ],
            ],
            'edit' => [
                'locale'                           => 'Språk',
                'level-parent'                     => 'Överordnad produkt',
                'level-sub-parent'                 => 'Mellanliggande nivå',
                'level-variant'                    => 'Variant',
                'common-to-all'                    => 'Gemensamt för alla varianter',
                'axis'                             => 'Axel',
                'drop-here'                        => 'Släpp tillhörande attribut här',
                'drag-into-level'                  => 'Dra ett attribut till en nivå.',
                'search-attributes'                => 'Sök attribut...',
                'assign-group-to-level'            => 'Tilldela en hel grupp till en nivå:',
                'add-sub-parent'                   => '+ Lägg till mellanliggande nivå',
                'remove-level'                     => 'Ta bort den här nivån',
                'select-page'                      => 'Markera alla på sidan',
                'select-all-matching'              => 'Markera alla :total attribut',
                'all-selected'                     => 'Alla :total attribut markerade',
                'selected-to'                      => 'valda → till',
                'assign'                           => 'Tilldela',
                'clear'                            => 'rensa',
                'too-large'                        => 'Filtret är för stort för att sparas',
                'variant-saved'                    => 'Variantstruktur sparad.',
                'variant-structure-locked'         => 'Den här variantstrukturen har redan varianter, så dess axlar och nivåer kan inte längre ändras. Ta bort de befintliga varianterna först.',
                'variant-structure-deleted'        => 'Variantstrukturen har tagits bort.',
                'variant-structure-in-use'         => 'Den här variantstrukturen refereras fortfarande av produkter och kan därför inte tas bort. Omfördela eller ta bort dessa produkter först.',
                'no-axis-options'                  => 'Lägg till minst ett icke-omfångsbundet valattribut i den här familjen innan du skapar varianter.',
                'parent-child'                     => 'Överordnad → Underordnad',
                'parent-sub-parent-child'          => 'Överordnad → Mellanliggande → Underordnad',
                'structure'                        => 'Struktur',
                'level-1-axes'                     => 'Axlar för nivå 1',
                'level-2-axes'                     => 'Axlar för nivå 2',
                'back-to-variants'                 => 'Tillbaka till varianter',
                'variant-setup'                    => 'Variantinställning',
                'edit-variant'                     => 'Redigera variant',
                'delete-variant'                   => 'Ta bort variant',
                'parent-product'                   => 'Gemensamt',
                'parent-product-info'              => 'Värden underhålls en gång och ärvs av varje variant.',
                'sub-parent-product'               => 'Mellanliggande produkt',
                'sub-parent-product-info'          => 'Värden som delas av produkter inom varje grupp på nivå 1.',
                'variant-child-product'            => 'Underordnad produkt',
                'variant-child-product-info'       => 'Värden underhålls separat för varje slutlig variant.',
                'move-to-parent'                   => 'Flytta till Gemensamt',
                'move-to-sub-parent'               => 'Flytta till mellanliggande',
                'move-to-child'                    => 'Flytta till underordnad',
                'no-parent-attributes'             => 'Inga gemensamma attribut',
                'no-parent-attributes-info'        => 'Alla matchande attribut är tilldelade variantnivåer eller används som axlar.',
                'no-sub-parent-attributes'         => 'Inga mellanliggande attribut',
                'no-child-attributes'              => 'Inga underordnade attribut',
                'axis-badge'                       => 'Axel',
                'level-badge'                      => 'Nivå :number',
                'add-to-level'                     => 'Lägg till på nivå :number',
                'add-attributes'                   => 'Lägg till attribut',
                'move-to-level-info'               => 'De valda attributen lämnar Gemensamt och underhålls i stället på den här nivån.',
                'level-empty-info'                 => 'Hittills underhålls bara axeln här.',
                'cancel'                           => 'Avbryt',
                'axis-locked-info'                 => 'Variantaxel. Underhålls alltid på denna nivå och kan inte flyttas.',
                'drop-or-search-info'              => 'Släpp attribut här eller justera sökningen.',
                'name-code-required'               => 'Namn och kod krävs.',
                'select-axis-warning'              => 'Välj minst ett annat axelattribut för varje nivå.',
                'unable-save-variant-structure'    => 'Det gick inte att spara variantstrukturen.',
                'level-1-attribute'                => 'Attribut för nivå 1',
                'level-2-attribute'                => 'Attribut för nivå 2',
                'no-axis-selected'                 => 'Ingen axel vald',
                'variant-name-placeholder'         => 'Exempel: Färg + Storlek',
                'variant-code-placeholder'         => 'color_size',
                'attributes-selected'              => 'attribut valda',
                'assign-selected-to'               => 'Tilldela markerade attribut till en grupp',
                'differ-per'                       => 'Olika per',
                'differ-sub-help'                  => 'Attribut som ändras på den här nivån (t.ex. en delad bild för varje färg).',
                'differ-variant-help'              => 'Attribut som är unika för varje variant (t.ex. SKU, pris, lager).',
                'drop-attributes-here'             => 'Släpp attribut här',
                'main-column-info'                 => 'Grupper och attribut som ingår i den här familjen.',
                'no-assigned-groups'               => 'Inga grupper tilldelade',
                'no-assigned-groups-info'          => 'Tilldela en attributgrupp för att börja bygga den här familjen.',
                'remove-group-btn'                 => 'Ta bort från familj',
                'search'                           => 'Sök',
                'select-destination-group'         => 'Välj grupp',
                'variant-help'                     => 'Definiera hur produkter i den här familjen delas upp i varianter. Dra ett attribut till den nivå det tillhör.',
                'variant-help-easy'                => 'Som standard delas varje attribut av alla varianter. Dra bara de som skiljer sig åt till en lägre nivå.',
                'variant-levels'                   => 'Variantnivåer',
                'variants'                         => 'Varianter',
                'variant-structure'                => 'Variantstruktur',
                'assign-group-btn'                 => 'Tilldela attributgrupp',
                'assign-group-title'               => 'Tilldela attributgrupp',
                'back-btn'                         => 'Tillbaka',
                'create-group'                     => 'Skapa denna grupp',
                'code'                             => 'Kod',
                'label'                            => 'Etikett',
                'column'                           => 'Kolumn',
                'delete-group-btn'                 => 'Ta bort grupp',
                'edit-group-info'                  => 'Dubbelklicka för att redigera gruppen',
                'enter-code'                       => 'Ange kod',
                'enter-name'                       => 'Ange namn',
                'general'                          => 'Allmänt',
                'family-attributes'                => 'Familjens attribut',
                'group-code-already-exists'        => 'Attributgruppens kod finns redan.',
                'group-contains-system-attributes' => 'Denna grupp innehåller systemattribut. Flytta systemattribut till en annan grupp och försök igen.',
                'group-name-already-exists'        => 'Attributgruppens namn finns redan.',
                'groups'                           => 'Grupper',
                'attribute-groups'                 => 'Attributgrupper',
                'groups-info'                      => 'Hantera attributgrupper i attributfamiljen',
                'main-column'                      => 'Huvudkolumn',
                'name'                             => 'Namn',
                'removal-not-possible'             => 'Det går inte att ta bort systemattribut från attributfamiljen.',
                'right-column'                     => 'Högerkolumn',
                'save-btn'                         => 'Spara attributgrupp',
                'select-group'                     => 'Välj attributgrupp',
                'title'                            => 'Redigera attributgrupp',
                'unassigned-attributes'            => 'Ockuperade attribut',
                'unassigned-attributes-info'       => 'Dra ett enskilt attribut till en grupp, eller markera flera och tilldela dem tillsammans.',
                'unassigned-attribute-groups'      => 'Ockuperade attributgrupper',
                'assign-first-attribute-group'     => 'Tilldela en grupp till familjen.',
                'variant-options-label'            => 'Konfigurationsalternativ',
                'select-variant'                   => 'Välj attribut',
                'add-variant'                      => 'Lägg till variant',
                'all-common'                       => 'Alla attribut är gemensamma för alla varianter.',
                'common-attributes'                => 'Gemensamma attribut',
                'common-subtitle'                  => 'Delas av alla varianter. Dra de som skiljer sig till en nivå.',
                'differ-rest-common'               => 'attribut skiljer sig · allt annat är gemensamt.',
                'drop-differ'                      => 'Dra attribut som skiljer sig hit',
                'family-variants'                  => 'Familjevarianter',
                'family-variants-info'             => 'Sök efter varianter och öppna en för att hantera dess attribut.',
                'move'                             => 'Flytta',
                'move-group'                       => 'Flytta en hel grupp:',
                'move-to'                          => 'flytta till',
                'no-unassigned-attributes'         => 'Alla attribut är tilldelade',
                'no-unassigned-attributes-info'    => 'Ta bort en grupp eller ett attribut från familjen för att göra det tillgängligt här.',
                'one-level'                        => '1 nivå · Överordnad → Variant',
                'save-variant'                     => 'Spara variant',
                'search-assigned'                  => 'Sök tilldelade grupper eller attribut',
                'shown'                            => 'visas',
                'two-level'                        => '2 nivåer · Överordnad → Underordnad → Variant',
                'variant'                          => 'Variant',
                'variants-info'                    => 'Skapa och hantera hur produkter i den här familjen delas upp i överordnade, underordnade och undervarianter.',
            ],
            'attribute-family'                => 'Attributfamilj',
            'attribute-product-error'         => 'Familjen används i produkter.',
            'create-success'                  => 'Familjen skapades framgångsrikt.',
            'delete-failed'                   => 'Fel vid borttagning av familj.',
            'delete-success'                  => 'Familjen togs bort framgångsrikt.',
            'family'                          => 'Familj',
            'last-delete-error'               => 'Minst en familj krävs.',
            'update-success'                  => 'Familjen uppdaterades framgångsrikt.',
            'user-define-error'               => 'Du kan inte ta bort en systemattributfamilj.',
            'not-found'                       => 'Attributfamilj med kod ":code" hittades inte.',
            'can-not-update-variant-options'  => 'Kan inte uppdatera konfigurationsalternativ eftersom denna familj redan har produktvarianter.',
            'variant-axis-attribute-required' => 'Attributet ":attributes" kan inte tas bort eftersom det används som variantaxel i den här familjen.',
        ],
        'history' => [
            'view'  => 'Visa versionsdetaljer',
            'index' => [
                'datagrid' => [
                    'version'   => 'Version',
                    'user'      => 'Användare',
                    'date_time' => 'Datum / Tid',
                ],
            ],
        ],
    ],
    'settings' => [
        'locales' => [
            'index' => [
                'not-found'              => 'Språkinställning med koden :code hittades inte.',
                'create-btn'             => 'Skapa Lokalitet',
                'locale'                 => 'Lokalitet',
                'title'                  => 'Lokaliteter',
                'can-not-delete-error'   => 'Du kan inte radera en lokalitet kopplad till en kanal eller användare',
                'can-not-disable-error'  => 'Du kan inte inaktivera en lokalitet kopplad till en kanal eller användare',
                'partial-delete-success' => 'Valda lokaliteter raderades, förutom :count som är kopplade till en kanal eller användare.',
                'partial-update-success' => 'Valda lokaliteter uppdaterades, förutom :count som är kopplade till en kanal eller användare.',

                'datagrid'              => [
                    'actions'     => 'Åtgärder',
                    'code'        => 'Kod',
                    'delete'      => 'Radera',
                    'direction'   => 'Riktning',
                    'edit'        => 'Redigera',
                    'id'          => 'ID',
                    'ltr'         => 'LTR',
                    'name'        => 'Visningsnamn',
                    'rtl'         => 'RTL',
                    'mass-update' => 'Uppdatera Status',
                    'status'      => [
                        'title'    => 'Status',
                        'active'   => 'Aktivera',
                        'inactive' => 'Inaktivera',
                    ],
                ],
                'create' => [
                    'code'             => 'Kod',
                    'code-hint'        => 'Använd ISO-språkkoden, till exempel en_US eller fr_FR.',
                    'code-placeholder' => 'Använd ISO-kod, t.ex. en_US',
                    'direction'        => 'Riktning',
                    'locale-logo'      => 'Lokalitetens Logotyp',
                    'name'             => 'Namn',
                    'save-btn'         => 'Spara Lokalitet',
                    'select-direction' => 'Välj Riktning',
                    'title'            => 'Skapa Lokalitet',
                    'status'           => 'Status',
                ],
                'edit' => [
                    'title' => 'Redigera Lokalitet',
                ],
                'create-success'    => 'Lokalitet skapad framgångsrikt.',
                'delete-failed'     => 'Fel vid radering av lokalitet.',
                'delete-success'    => 'Lokalitet raderad framgångsrikt.',
                'delete-warning'    => 'Är du säker på att du vill utföra denna åtgärd?',
                'last-delete-error' => 'Minst en lokalitet krävs.',
                'update-success'    => 'Lokalitet uppdaterad framgångsrikt.',
            ],
        ],
        'currencies' => [
            'index' => [
                'not-found'              => 'Valuta med koden :code hittades inte.',
                'create-btn'             => 'Skapa Valuta',
                'currency'               => 'Valuta',
                'title'                  => 'Valutor',
                'can-not-delete-error'   => 'Du kan inte radera en valuta kopplad till en kanal',
                'can-not-disable-error'  => 'Du kan inte inaktivera en valuta kopplad till en kanal',
                'partial-update-success' => 'Valda valutor uppdaterades, förutom :count som är kopplade till en kanal.',
                'datagrid'               => [
                    'actions'        => 'Åtgärder',
                    'code'           => 'Kod',
                    'delete'         => 'Radera',
                    'edit'           => 'Redigera',
                    'id'             => 'ID',
                    'method-error'   => 'Fel! Fel metod upptäcktes, vänligen kontrollera massåtgärdsinställningen',
                    'name'           => 'Visningsnamn',
                    'no-resource'    => 'Resursen som tillhandahålls är otillräcklig för åtgärden',
                    'partial-action' => 'Vissa åtgärder har inte genomförts på grund av systembegränsningar på :resource',
                    'update-success' => 'Valt :resource uppdaterades framgångsrikt.',
                    'mass-update'    => 'Uppdatera Status',
                    'status'         => [
                        'title'    => 'Status',
                        'active'   => 'Aktivera',
                        'inactive' => 'Inaktivera',
                    ],
                ],
                'create' => [
                    'code'             => 'Kod',
                    'code-hint'        => 'Använd ISO 4217-valutakoden, till exempel USD eller EUR.',
                    'code-placeholder' => 'Använd ISO-kod, t.ex. USD',
                    'create-btn'       => 'Skapa Valuta',
                    'decimal'          => 'Decimal',
                    'delete-warning'   => 'Är du säker på att du vill utföra denna åtgärd?',
                    'general'          => 'Allmän',
                    'name'             => 'Namn',
                    'save-btn'         => 'Spara Valuta',
                    'symbol'           => 'Symbol',
                    'title'            => 'Skapa Ny Valuta',
                    'status'           => 'Status',
                ],
                'edit' => [
                    'title' => 'Redigera Valuta',
                ],
                'create-success'    => 'Valuta skapad framgångsrikt.',
                'delete-failed'     => 'Fel vid radering av valuta.',
                'delete-success'    => 'Valuta raderad framgångsrikt.',
                'last-delete-error' => 'Minst en valuta krävs.',
                'update-success'    => 'Valuta uppdaterad framgångsrikt.',
                'cannot-delete'     => 'Valuta kan inte raderas.',
            ],
        ],
        'data-transfer' => [
            'tracker' => [
                'index' => [
                    'title'    => 'Jobbspårare',
                    'datagrid' => [
                        'id'                   => 'ID',
                        'job_code'             => 'Jobb',
                        'type'                 => 'Typ',
                        'job_type'             => 'Jobbtyp',
                        'state'                => 'Status',
                        'processed_rows_count' => 'Bearbetade Rader',
                        'invalid_rows_count'   => 'Ogiltiga Rader',
                        'started_at'           => 'Startad',
                        'completed_at'         => 'Avslutad',
                        'user'                 => 'Användare',
                        'created_at'           => 'Skapad',
                        'updated_at'           => 'Uppdaterad',
                        'pending'              => 'Väntar',
                        'validating'           => 'Validerar',
                        'validated'            => 'Validerad',
                        'processing'           => 'Bearbetar',
                        'processed'            => 'Bearbetad',
                        'linking'              => 'Länkar',
                        'linked'               => 'Länkad',
                        'indexing'             => 'Indexerar',
                        'indexed'              => 'Indexerad',
                        'completed'            => 'Slutförd',
                        'paused'               => 'Pausad',
                        'cancelled'            => 'Avbruten',
                        'failed'               => 'Misslyckades',
                        'view'                 => 'Visa',
                    ],
                ],
                'import' => [
                    'back-btn'                => 'Tillbaka',
                    'completed-batches'       => 'Totalt Slutförda Omgångar:',
                    'download-error-report'   => 'Ladda ner Fullständig Rapport',
                    'edit-btn'                => 'Redigera',
                    'imported-info'           => 'Grattis! Din import lyckades.',
                    'exporting-info'          => 'Export pågår',
                    'indexing-info'           => 'Indexering av Resurser (Priser, Elasticsearch) pågår',
                    'linking-info'            => 'Länkar Resurser pågår',
                    'progress'                => 'Framsteg:',
                    'title'                   => 'Exportera',
                    'total-batches'           => 'Totalt Omgångar:',
                    'created'                 => 'Totalt Skapade Poster:',
                    'total-deleted'           => 'Totalt Raderade Poster:',
                    'total-errors'            => 'Totalt Fel:',
                    'total-invalid-rows'      => 'Totalt Ogiltiga Rader:',
                    'total-rows-processed'    => 'Totalt Bearbetade Rader:',
                    'total-updated'           => 'Totalt Uppdaterade Poster:',
                    'validate-info'           => 'Klicka på Validera Data för att kontrollera din import.',
                    'validate'                => 'Validera',
                    'validating-info'         => 'Data börjar läsas och valideras.',
                    'validation-failed-info'  => 'Din import är ogiltig. Korrigera följande fel och försök igen.',
                    'validation-success-info' => 'Din import är giltig. Klicka på Exportera för att starta importprocessen.',
                ],
                'summary' => [
                    'total-batches' => 'Totalt Omgångar:',
                    'created'       => 'Totalt Skapade Poster:',
                    'deleted'       => 'Totalt Raderade Poster:',
                    'errors'        => 'Totalt Fel:',
                    'invalid-rows'  => 'Totalt Ogiltiga Rader:',
                    'processed'     => 'Totalt Bearbetade Rader:',
                    'updated'       => 'Totalt Uppdaterade Poster:',
                    'skipped'       => 'Totalt Ignorerade Poster:',
                ],
                'info'                  => 'Grattis! Ditt jobb kördes framgångsrikt.',
                'running-time'          => 'Körtid',
                'estimated-remaining'   => 'Uppskattad återstående',
                'total-duration'        => 'Total varaktighet',
                'status'                => 'Status',
                'waiting'               => 'Väntar',
                'total-records'         => 'Totalt antal poster',
                'job-id'                => 'Jobb-ID',
                'records-created'       => 'Skapade poster',
                'records-updated'       => 'Uppdaterade poster',
                'records-deleted'       => 'Raderade poster',
                'run-again'             => 'Kör igen',
                'completed-success'     => 'Jobb slutfört framgångsrikt',
                'completed-info-sub'    => 'Alla poster har bearbetats och indexerats. Din katalog är uppdaterad.',
                'create-success'        => 'Export skapades framgångsrikt.',
                'delete-failed'         => 'Exporten misslyckades att radera.',
                'delete-success'        => 'Export raderades framgångsrikt.',
                'not-valid'             => 'Export är ogiltig',
                'nothing-to-import'     => 'Det finns inga resurser att importera.',
                'setup-queue-error'     => 'Ändra din ködrivrutin till "databas" eller "redis" för att starta importprocessen.',
                'update-success'        => 'Export uppdaterades framgångsrikt.',
                'failed-info'           => 'Jobbet misslyckades. Korrigera följande fel och försök igen.',
                'download-log-file'     => 'Ladda ner Loggfil',
                'pause'                 => 'Paus',
                'resume'                => 'Återuppta',
                'cancel'                => 'Avbryt',
                'paused'                => 'Importen har pausats.',
                'paused-export'         => 'Exporten har pausats.',
                'resumed'               => 'Importen har återupptagits.',
                'resumed-export'        => 'Exporten har återupptagits.',
                'zip-failed'            => 'Det gick inte att skapa zip-filen.',
                'nothing-to-archive'    => 'Det finns inga exporterade filer att ladda ner.',
                'cancelled'             => 'Importen har avbrutits.',
                'cancelled-export'      => 'Exporten har avbrutits.',
                'paused-info'           => 'Importen är pausad. Återstående batcher är parkerade.',
                'paused-info-export'    => 'Exporten är pausad. Återstående batcher är parkerade.',
                'cancelled-info'        => 'Importen avbröts. Delvisa resultat visas nedan.',
                'cancelled-info-export' => 'Exporten avbröts. Delvisa resultat visas nedan.',
                'cancel-confirm'        => 'Är du säker på att du vill avbryta detta jobb? Denna åtgärd kan inte ångras.',
                'pause-failed'          => 'Kunde inte pausa importen.',
                'pause-failed-export'   => 'Kunde inte pausa exporten.',
                'resume-failed'         => 'Kunde inte återuppta importen.',
                'resume-failed-export'  => 'Kunde inte återuppta exporten.',
                'cancel-failed'         => 'Kunde inte avbryta importen.',
                'cancel-failed-export'  => 'Kunde inte avbryta exporten.',
                'live'                  => 'Live',
                'import-progress'       => 'Importförlopp',
                'export-progress'       => 'Exportförlopp',
                'linking-progress'      => 'Länkningsförlopp',
                'indexing-progress'     => 'Indexeringsförlopp',
                'job-label'             => 'Jobb',
            ],
            'imports' => [
                'import-start-fail' => 'Importprocessen kunde inte startas. Försök igen.',
                'rerun-no-file'     => 'Det här jobbet har ingen importfil, så det kan inte köras igen.',
                'create'            => [
                    'code'                => 'Kod',
                    'action'              => 'Åtgärd',
                    'allowed-errors'      => 'Tillåtna Fel',
                    'allowed-file-types'  => 'Tillåtna filtyper (CSV, XLSX, XLS)',
                    'back-btn'            => 'Tillbaka',
                    'create-update'       => 'Skapa/Uppdatera',
                    'delete'              => 'Radera',
                    'media'               => 'Fil / Bilder',
                    'images'              => 'Bilder',
                    'download-sample'     => 'Ladda ner :resource Exempel CSV',
                    'field-separator'     => 'Fältseparator',
                    'separator-info'      => 'Används bara för CSV-filer',
                    'file-info-example'   => 'Placera bilder i storage/app/public/. För bilder i storage/app/public/import-images, inkludera import-images/ i sökvägen och använd endast filnamnet i importfilen.',
                    'file-info'           => 'Använd relativ sökväg till /project-root/storage/app/import, t.ex. produktbilder, import-images.',
                    'file'                => 'Fil',
                    'general'             => 'Allmän',
                    'upload_images'       => 'Ladda upp bilder till angiven sökväg',
                    'download-sample-zip' => 'Ladda ner Exempelbilder ZIP',
                    'zip-uploading'       => 'Laddar upp och extraherar ZIP...',
                    'zip-upload-success'  => 'ZIP uppladdad och extraherad framgångsrikt.',
                    'zip-upload-error'    => 'Uppladdning av ZIP misslyckades. Vänligen försök igen.',
                    'invalid-zip'         => 'Den uppladdade filen är inte ett giltigt ZIP-arkiv.',
                    'zip-upload-or'       => 'eller ladda upp en ZIP med bilder',
                    'zip-click-upload'    => 'Klicka för att ladda upp en ZIP med bilder',
                    'zip-files-extracted' => 'filer extraherade',
                    'zip-drop-hint'       => 'Klicka eller dra & släpp en ZIP-fil här',
                    'file-upload-info'    => 'CSV, XLSX, JSON (MAX. 2 MB)',
                    'images-directory'    => 'Sökväg',
                    'process-in-queue'    => 'Bearbeta i kö',
                    'results'             => 'Resultat',
                    'save-btn'            => 'Spara Import',
                    'settings'            => 'Inställningar',
                    'skip-errors'         => 'Hoppa över Fel',
                    'stop-on-errors'      => 'Stoppa vid Fel',
                    'title'               => 'Skapa Import',
                    'type'                => 'Typ',
                    'validation-strategy' => 'Valideringsstrategi',
                ],
                'edit' => [
                    'images'              => 'Bilder',
                    'action'              => 'Åtgärd',
                    'allowed-errors'      => 'Tillåtna Fel',
                    'allowed-file-types'  => 'Tillåtna filtyper (CSV, XLSX, XLS)',
                    'back-btn'            => 'Tillbaka',
                    'create-update'       => 'Skapa/Uppdatera',
                    'delete'              => 'Radera',
                    'download-sample'     => 'Ladda ner Exempel',
                    'field-separator'     => 'Fältseparator',
                    'separator-info'      => 'Används bara för CSV-filer',
                    'file-info-example'   => 'Placera bilder i storage/app/public/. För bilder i storage/app/public/import-images, inkludera import-images/ i sökvägen och använd endast filnamnet i importfilen.',
                    'file-info'           => 'Använd relativ sökväg till /project-root/storage/app/import, t.ex. produktbilder, import-images.',
                    'file'                => 'Fil',
                    'general'             => 'Allmän',
                    'images-directory'    => 'Sökväg',
                    'process-in-queue'    => 'Bearbeta i kö',
                    'results'             => 'Resultat',
                    'save-btn'            => 'Spara Import',
                    'settings'            => 'Inställningar',
                    'skip-errors'         => 'Hoppa över Fel',
                    'stop-on-errors'      => 'Stoppa vid Fel',
                    'title'               => 'Redigera Import',
                    'type'                => 'Typ',
                    'validation-strategy' => 'Valideringsstrategi',
                ],
                'batch' => [
                    'title' => 'Import tillagd i kön',
                ],
                'index' => [
                    'button-title' => 'Skapa Import',
                    'title'        => 'Importer',
                    'datagrid'     => [
                        'actions'               => 'Åtgärder',
                        'completed-at'          => 'Slutförd Vid',
                        'created'               => 'Skapad',
                        'delete'                => 'Radera',
                        'deleted'               => 'Raderad',
                        'edit'                  => 'Redigera',
                        'error-file'            => 'Fel Fil',
                        'id'                    => 'ID',
                        'code'                  => 'Kod',
                        'type'                  => 'Typ',
                        'action'                => 'Åtgärd',
                        'started-at'            => 'Startad Vid',
                        'state'                 => 'Tillstånd',
                        'summary'               => 'Sammanfattning',
                        'updated'               => 'Uppdaterad',
                        'uploaded-file'         => 'Uppladdad Fil',
                        'images_directory_path' => 'Bilder Fil',
                        'import'                => 'Importera',
                    ],
                ],
                'import' => [
                    'action'                  => 'Åtgärdsläge',
                    'back-btn'                => 'Tillbaka',
                    'completed-batches'       => 'Totalt Slutförda Batchar:',
                    'download-error-report'   => 'Ladda ner Fullständig Rapport',
                    'edit-btn'                => 'Redigera',
                    'exporting-info'          => 'Export pågår',
                    'exporting-info-sub'      => 'Genererar exportfil och skriver poster. Vänligen vänta.',
                    'file_path'               => 'Fil Sökväg',
                    'imported-info'           => 'Grattis! Din import var framgångsrik.',
                    'import-now'              => 'Importera Nu',
                    'importing-info'          => 'Import pågår',
                    'importing-info-sub'      => 'Bearbetar batcher och skriver poster till databasen.',
                    'indexing-info'           => 'Resurser Indexering (Pris, Elastic Search) Pågår',
                    'indexing-info-sub'       => 'Bygger om sökindex och priscacheminnen efter import. Detta steg körs automatiskt.',
                    'indexing-type'           => 'Pris + ES',
                    'linking-info'            => 'Resurser Länkning Pågår',
                    'linking-info-sub'        => 'Länkar relaterade resurser och associationer mellan poster.',
                    'pending-info'            => 'Jobb i kö — väntar på att arbetaren ska starta...',
                    'pending-info-sub'        => 'Ditt jobb har lagts till i bearbetningskön. En arbetare kommer att ta hand om det inom kort.',
                    'pending-step-complete'   => 'Slutförd',
                    'pending-step-export'     => 'Exporterar',
                    'pending-step-import'     => 'Importerar',
                    'pending-step-queue'      => 'I kö',
                    'pending-step-validate'   => 'Validerar',
                    'profile'                 => 'Importprofil',
                    'progress'                => 'Framsteg:',
                    'title'                   => 'Importera',
                    'total-batches'           => 'Totalt antal batcher:',
                    'total-created'           => 'Totalt skapade poster:',
                    'total-deleted'           => 'Totalt raderade poster:',
                    'total-errors'            => 'Totalt antal fel:',
                    'total-invalid-rows'      => 'Totalt ogiltiga rader:',
                    'total-rows-processed'    => 'Totalt bearbetade rader:',
                    'total-updated'           => 'Totalt uppdaterade poster:',
                    'validate'                => 'Validera',
                    'validate-info'           => 'Klicka på Validera Data för att kontrollera din import.',
                    'validating-info'         => 'Läser fil och validerar data...',
                    'validating-info-sub'     => 'Kontrollerar schemaintegritet, fältmappningar och datatyper innan importen börjar.',
                    'validation-failed-info'  => 'Din import är ogiltig. Vänligen åtgärda följande fel och försök igen.',
                    'validation-success-info' => 'Din import är giltig. Startar import automatiskt...',
                ],
                'create-success'             => 'Import skapad framgångsrikt.',
                'delete-failed'              => 'Import radering misslyckades oväntat.',
                'delete-success'             => 'Importen raderades framgångsrikt.',
                'invalid-zip'                => 'Den uppladdade filen är inte ett giltigt ZIP-arkiv.',
                'not-valid'                  => 'Importen är ogiltig',
                'zip-compression-suspicious' => 'Posten ":entry" expanderar långt utöver sin komprimerade storlek, så arkivet avvisades som osäkert.',
                'zip-contents-too-large'     => 'Bilderna i denna ZIP uppgår tillsammans till mer än :limit MB efter uppackning. Dela upp dem i mindre arkiv.',
                'zip-exceeds-server-limit'   => 'Denna ZIP är större än de :limit KB som denna server accepterar vid en enda uppladdning. Öka upload_max_filesize och post_max_size i php.ini, eller dela upp arkivet.',
                'zip-too-many-entries'       => 'Denna ZIP innehåller :count filer, och som mest kan :limit importeras åt gången. Dela upp den i mindre arkiv eller öka IMAGE_IMPORT_MAX_ENTRIES.',
                'nothing-to-import'          => 'Det finns inga resurser att importera.',
                'setup-queue-error'          => 'Vänligen ändra din ködrivrutin till "databas" eller "redis" för att starta importprocessen.',
                'update-success'             => 'Importen uppdaterades framgångsrikt.',
                'zip-upload-success'         => 'ZIP uppladdad och extraherad framgångsrikt.',
            ],
            'exports' => [
                'create' => [
                    'code'                      => 'Kod',
                    'action'                    => 'Åtgärd',
                    'allowed-errors'            => 'Tillåtna Fel',
                    'back-btn'                  => 'Tillbaka',
                    'create-update'             => 'Skapa/Uppdatera',
                    'delete'                    => 'Radera',
                    'media'                     => 'Fil / Bilder',
                    'images'                    => 'Bilder',
                    'download-sample'           => 'Ladda Ner Exempel :resource',
                    'field-separator'           => 'Fält Separator',
                    'file-info-example'         => 'Till exempel, i fallet med produktbilder, ska filerna placeras i /project-root/storage/app/export/product-images.',
                    'file-info'                 => 'Använd relativ sökväg till /project-root/storage/app/export, ex: product-images, export-images.',
                    'file'                      => 'Fil',
                    'general'                   => 'Allmänt',
                    'upload_images'             => 'Ladda Upp Bilder för att Ställa in Väg',
                    'download-sample-zip'       => 'Ladda Ner Exempelbilder i ZIP',
                    'images-directory'          => 'Sökväg',
                    'process-in-queue'          => 'Bearbeta i Kö',
                    'results'                   => 'Resultat',
                    'save-btn'                  => 'Spara Export',
                    'settings'                  => 'Inställningar',
                    'scope-filters'             => 'Data att exportera',
                    'product-filters'           => 'Produktfilter',
                    'output'                    => 'Utdata',
                    'format-settings'           => 'Formatinställningar',
                    'custom-attribute-select'   => 'Välj attribut',
                    'custom-attribute-value'    => 'Värde',
                    'custom-attribute-remove'   => 'Ta bort',
                    'attribute-conditions'      => 'Attributvillkor',
                    'attribute-conditions-info' => 'Endast produkter som uppfyller alla villkor exporteras.',
                    'no-categories'             => 'Inga kategorier tillgängliga.',
                    'search-categories'         => 'Sök kategorier efter namn eller kod',
                    'add-condition'             => 'Lägg till ytterligare ett attributvillkor',
                    'operator-select'           => 'Villkor',
                    'no-value-needed'           => 'Inget värde behövs',
                    'value-number-placeholder'  => 't.ex. 49.99',
                    'range-from'                => 'Från',
                    'range-to'                  => 'Till',
                    'condition-yes'             => 'Ja',
                    'condition-no'              => 'Nej',
                    'operators'                 => [
                        'in'                 => 'I listan',
                        'not_in'             => 'Inte i listan',
                        'empty'              => 'Är tomt',
                        'not_empty'          => 'Är inte tomt',
                        'contains'           => 'Innehåller',
                        'equals'             => 'Lika med',
                        'less_than'          => 'Mindre än',
                        'less_than_equal'    => 'Mindre än eller lika med',
                        'greater_than'       => 'Större än',
                        'greater_than_equal' => 'Större än eller lika med',
                        'between'            => 'Mellan',
                        'before'             => 'Före',
                        'after'              => 'Efter',
                    ],
                    'filters'             => 'Filter',
                    'skip-errors'         => 'Hoppa över Fel',
                    'stop-on-errors'      => 'Stoppa vid Fel',
                    'title'               => 'Skapa Export',
                    'type'                => 'Typ',
                    'validation-strategy' => 'Valideringsstrategi',
                ],
                'edit' => [
                    'action'              => 'Åtgärd',
                    'allowed-errors'      => 'Tillåtna Fel',
                    'back-btn'            => 'Tillbaka',
                    'create-update'       => 'Skapa/Uppdatera',
                    'delete'              => 'Radera',
                    'download-sample'     => 'Ladda Ner Exempel',
                    'field-separator'     => 'Fält Separator',
                    'file-info-example'   => 'Till exempel, i fallet med produktbilder, ska filerna placeras i /project-root/storage/app/export/product-images.',
                    'file-info'           => 'Använd relativ sökväg till /project-root/storage/app/export, ex: product-images, export-images.',
                    'file'                => 'Fil',
                    'general'             => 'Allmänt',
                    'images-directory'    => 'Sökväg till Bilder',
                    'process-in-queue'    => 'Bearbeta i Kö',
                    'results'             => 'Resultat',
                    'save-btn'            => 'Spara Export',
                    'settings'            => 'Inställningar',
                    'skip-errors'         => 'Hoppa över Fel',
                    'stop-on-errors'      => 'Stoppa vid Fel',
                    'title'               => 'Redigera Export',
                    'type'                => 'Typ',
                    'validation-strategy' => 'Valideringsstrategi',
                ],
                'batch' => [
                    'title' => 'Export har lagts till i kön',
                ],
                'index' => [
                    'button-title' => 'Skapa Export',
                    'title'        => 'Exporter',
                    'datagrid'     => [
                        'actions'               => 'Åtgärder',
                        'completed-at'          => 'Slutförd vid',
                        'created'               => 'Skapad',
                        'delete'                => 'Radera',
                        'deleted'               => 'Raderad',
                        'edit'                  => 'Redigera',
                        'error-file'            => 'Fel-fil',
                        'id'                    => 'ID',
                        'code'                  => 'Kod',
                        'type'                  => 'Typ',
                        'action'                => 'Åtgärd',
                        'started-at'            => 'Startad vid',
                        'state'                 => 'Tillstånd',
                        'summary'               => 'Sammanfattning',
                        'updated'               => 'Uppdaterad',
                        'uploaded-file'         => 'Uppladdad Fil',
                        'images_directory_path' => 'Väg till Bildfil',
                        'export'                => 'Exportera',
                    ],
                ],
                'export' => [
                    'back-btn'                => 'Tillbaka',
                    'completed-batches'       => 'Totalt Slutförda Batchar:',
                    'download-error-report'   => 'Ladda Ner Fullständig Rapport',
                    'download-created-file'   => 'Ladda Ner Exporterade Filer',
                    'edit-btn'                => 'Redigera',
                    'exported-info'           => 'Grattis! Din export lyckades.',
                    'exporting-info'          => 'Export pågår',
                    'export-now'              => 'Exportera Nu',
                    'indexing-info'           => 'Indexering av resurser (pris, Elastic Search) pågår',
                    'linking-info'            => 'Länkning av resurser pågår',
                    'progress'                => 'Framsteg:',
                    'title'                   => 'Exportera',
                    'total-batches'           => 'Totalt Batchar:',
                    'total-created'           => 'Totalt Skapade Registreringar:',
                    'total-deleted'           => 'Totalt Raderade Registreringar:',
                    'total-errors'            => 'Totalt Fel:',
                    'total-invalid-rows'      => 'Totalt Ogiltiga Rader:',
                    'total-rows-processed'    => 'Totalt Bearbetade Rader:',
                    'total-updated'           => 'Totalt Uppdaterade Registreringar:',
                    'validate-info'           => 'Klicka på Validera Data för att kontrollera din export.',
                    'validate'                => 'Validera',
                    'validating-info'         => 'Läser och validerar data',
                    'validation-failed-info'  => 'Din export är ogiltig. Vänligen rätta till följande fel och försök igen.',
                    'validation-success-info' => 'Din export är giltig. Klicka på Exportera för att starta processen.',
                    'profile'                 => 'Exportprofil',
                    'file-format'             => 'Filformat',
                    'with-media'              => 'Med Media',
                ],
                'create-success'    => 'Export skapad med framgång.',
                'delete-failed'     => 'Radering av export misslyckades oväntat.',
                'delete-success'    => 'Export raderad med framgång.',
                'not-valid'         => 'Export är ogiltig',
                'nothing-to-export' => 'Det finns inga resurser att exportera.',
                'setup-queue-error' => 'Vänligen ändra din ködrivrutin till "database" eller "redis" för att starta exportprocessen.',
                'update-success'    => 'Export uppdaterad med framgång.',
            ],
        ],
        'channels' => [
            'index' => [
                'not-found'            => 'Kanal med koden :code hittades inte.',
                'create-btn'           => 'Skapa kanal',
                'delete-failed'        => 'Kanals borttagning misslyckades',
                'delete-success'       => 'Kanal borttagen.',
                'last-delete-error'    => 'Standardkanalen kan inte tas bort.',
                'can-not-delete-error' => 'Du kan inte ta bort kanalen ":channel" eftersom din PIM måste ha minst en kanal.',
                'title'                => 'Kanaler',
                'datagrid'             => [
                    'code'          => 'Kod',
                    'delete'        => 'Ta bort',
                    'edit'          => 'Redigera',
                    'host-name'     => 'Värdnamn',
                    'id'            => 'ID',
                    'name'          => 'Namn',
                    'root-category' => 'Rotkategori',
                ],
            ],
            'create' => [
                'allowed-ips'            => 'Tillåtna IP-adresser',
                'cancel'                 => 'Tillbaka',
                'code'                   => 'Kod',
                'create-success'         => 'Kanal skapad.',
                'currencies'             => 'Valutor',
                'currencies-and-locales' => 'Valutor och Lokalisationer',
                'description'            => 'Beskrivning',
                'design'                 => 'Design',
                'ui-locale'              => 'UI-Lokalisation',
                'favicon'                => 'Favicon',
                'favicon-size'           => 'Bildens upplösning ska vara 16px X 16px',
                'general'                => 'Allmänt',
                'hostname'               => 'Värdnamn',
                'hostname-placeholder'   => 'https://www.exempel.com (Lägg inte till snedstreck i slutet.)',
                'last-delete-error'      => 'Minst en kanal krävs.',
                'locales'                => 'Lokalisationer',
                'logo'                   => 'Logotyp',
                'logo-size'              => 'Bildens upplösning ska vara 192px X 50px',
                'maintenance-mode-text'  => 'Meddelande',
                'name'                   => 'Namn',
                'root-category'          => 'Rotkategori',
                'save-btn'               => 'Spara kanal',
                'select-root-category'   => 'Välj rotkategori',
                'select-theme'           => 'Välj tema',
                'seo'                    => 'SEO för startsidan',
                'seo-description'        => 'Meta beskrivning',
                'seo-keywords'           => 'Meta nyckelord',
                'seo-title'              => 'Meta titel',
                'settings'               => 'Inställningar',
                'status'                 => 'Status',
                'theme'                  => 'Tema',
                'title'                  => 'Skapa kanal',
            ],
            'edit' => [
                'allowed-ips'            => 'Tillåtna IP-adresser',
                'back-btn'               => 'Tillbaka',
                'code'                   => 'Kod',
                'currencies'             => 'Valutor',
                'currencies-and-locales' => 'Valutor och Lokalisationer',
                'default-currency'       => 'Standardvaluta',
                'description'            => 'Beskrivning',
                'ui-locale'              => 'UI-Lokalisation',
                'design'                 => 'Design',
                'favicon'                => 'Favicon',
                'favicon-size'           => 'Bildens upplösning ska vara 16px X 16px',
                'general'                => 'Allmänt',
                'hostname'               => 'Värdnamn',
                'hostname-placeholder'   => 'https://www.exempel.com (Lägg inte till snedstreck i slutet.)',
                'last-delete-error'      => 'Minst en kanal krävs.',
                'locales'                => 'Lokalisationer',
                'select-locales'         => 'Välj lokalisationer',
                'select-currencies'      => 'Välj valutor',
                'logo'                   => 'Logotyp',
                'logo-size'              => 'Bildens upplösning ska vara 192px X 50px',
                'maintenance-mode'       => 'Underhållsläge',
                'maintenance-mode-text'  => 'Meddelande',
                'name'                   => 'Namn',
                'root-category'          => 'Rotkategori',
                'save-btn'               => 'Spara kanal',
                'seo'                    => 'SEO för startsidan',
                'seo-description'        => 'Meta beskrivning',
                'seo-keywords'           => 'Meta nyckelord',
                'seo-title'              => 'Meta titel',
                'status'                 => 'Status',
                'name-translations'      => 'Namnet översättningar',
                'theme'                  => 'Tema',
                'title'                  => 'Redigera kanal',
                'update-success'         => 'Kanal uppdaterad framgångsrikt',
            ],
        ],
        'users' => [
            'index' => [
                'admin'  => 'Administratör',
                'title'  => 'Användare',
                'user'   => 'Användare',

                'confirm-password-before-delete' => 'Bekräfta lösenord före radering',
                'enter-current-password'         => 'Ange nuvarande lösenord',
                'password'                       => 'Lösenord',
                'confirm-delete-account'         => 'Bekräfta radering av detta konto',

                'create' => [
                    'confirm-password'    => 'Bekräfta Lösenord',
                    'email'               => 'E-post',
                    'name'                => 'Namn',
                    'password'            => 'Lösenord',
                    'role'                => 'Roll',
                    'save-btn'            => 'Spara Användare',
                    'status'              => 'Status',
                    'title'               => 'Skapa Användare',
                    'upload-image-info'   => 'Ladda upp en profilbild (110px X 110px), eller använd din Gravatar.com-bild kopplad till din e-post.',
                    'user-timezone'       => 'Tidszon',
                    'catalog-locale'      => 'Katalogspråk',
                    'catalog-locale-info' => 'Språket den här användaren skriver katalog­innehåll på. Skilt från användarens gränssnittsspråk.',
                    'default-channel'     => 'Standardkanal',
                    'select'              => 'Välj',
                ],
                'datagrid' => [
                    'actions'  => 'Åtgärder',
                    'active'   => 'Aktiv',
                    'delete'   => 'Ta bort',
                    'edit'     => 'Redigera',
                    'email'    => 'E-post',
                    'id'       => 'ID',
                    'inactive' => 'Inaktiv',
                    'name'     => 'Namn',
                    'role'     => 'Roll',
                    'status'   => 'Status',
                    'image'    => 'Bild',
                ],
                'edit' => [
                    'title' => 'Redigera Användare',
                ],

                'delete-account' => [
                    'confirm-password-title' => 'Bekräfta lösenord före borttagning',
                    'enter-current-password' => 'Ange nuvarande lösenord',
                    'password'               => 'Lösenord',
                    'confirm-delete'         => 'Bekräfta borttagning av detta konto',
                ],
            ],
            'edit' => [
                'back-btn'         => 'Tillbaka',
                'confirm-password' => 'Bekräfta Lösenord',
                'email'            => 'E-post',
                'general'          => 'Allmänt',
                'name'             => 'Namn',
                'password'         => 'Lösenord',
                'role'             => 'Roll',
                'save-btn'         => 'Spara Användare',
                'status'           => 'Status',
                'title'            => 'Redigera Användare',
            ],
            'activate-warning'                 => 'Ditt konto har ännu inte aktiverats, vänligen kontakta administratören.',
            'cannot-change'                    => 'Användaren kan inte ändras',
            'cannot-assign-unheld-permissions' => 'Du kan inte tilldela en roll som har behörigheter du inte själv har.',
            'cannot-escalate-role'             => 'Du har inte behörighet att tilldela en roll med full åtkomst.',
            'create-success'                   => 'Användare skapades framgångsrikt.',
            'delete-failed'                    => 'Det gick inte att ta bort användaren.',
            'delete-success'                   => 'Användaren togs bort framgångsrikt.',
            'delete-warning'                   => 'Är du säker på att du vill utföra denna åtgärd?',
            'incorrect-password'               => 'Felaktigt lösenord',
            'last-delete-error'                => 'Det gick inte att ta bort den sista användaren',
            'login-error'                      => 'Kontrollera dina uppgifter och försök igen.',
            'update-success'                   => 'Användare uppdaterad framgångsrikt.',
            'current-user-delete-error'        => 'Den inloggade användaren kan inte tas bort.',
            'last-all-access-delete-error'     => 'Den sista aktiva administratören med full åtkomst kan inte tas bort.',
        ],
        'system-settings' => [
            'title'              => 'Systeminställningar',
            'back'               => 'Tillbaka',
            'info'               => 'Hantera systeminställningar från ett och samma ställe.',
            'search-placeholder' => 'Sök inställningar',
            'save-btn'           => 'Spara',
            'save-message'       => 'Inställningarna har sparats.',
            'system'             => [
                'title' => 'System',
                'info'  => 'Grundläggande programinställningar.',
            ],
            'email'              => [
                'title' => 'E-post',
                'info'  => 'SMTP-avsändare och autentiseringsuppgifter.',
            ],
            'debug'              => [
                'title' => 'Felsökning',
                'info'  => 'Felsökning och loggning av programmet.',
            ],
            'microsoft-sso'      => [
                'title' => 'Microsoft SSO',
                'info'  => 'Microsoft Entra ID enkel inloggning för administratörsinloggning.',
            ],
        ],

        'appearance' => [
            'title'          => 'Appearance',
            'info'           => 'Logotyp och favicon.',
            'section-title'  => 'Visual Branding',
            'section-info'   => 'Customize how the admin panel looks by uploading your own logo and favicon. Changes preview instantly and apply once saved.',
            'logo'           => 'Logo',
            'logo-size'      => 'Recommended image resolution: 192px X 50px',
            'favicon'        => 'Favicon',
            'favicon-size'   => 'Recommended image resolution: 16px X 16px',
            'no-logo'        => 'No logo uploaded yet.',
            'no-favicon'     => 'No favicon uploaded yet.',
            'upload-cta'     => 'Click to upload',
            'upload-hint'    => ' or drag and drop',
            'save-btn'       => 'Save Appearance',
            'update-success' => 'Appearance updated successfully.',
        ],
        'roles' => [
            'index' => [
                'create-btn' => 'Skapa Roll',
                'title'      => 'Roller',
                'datagrid'   => [
                    'delete'          => 'Ta bort',
                    'edit'            => 'Redigera',
                    'id'              => 'ID',
                    'name'            => 'Namn',
                    'permission-type' => 'Behörighetstyp',
                ],
            ],
            'create' => [
                'access-control'     => 'Åtkomstkontroll',
                'all'                => 'Alla',
                'all-description'    => 'Ge full åtkomst till alla behörigheter.',
                'back-btn'           => 'Tillbaka',
                'custom'             => 'Anpassad',
                'custom-description' => 'Välj enskilda behörigheter från trädet nedan.',
                'description'        => 'Beskrivning',
                'general'            => 'Allmänt',
                'name'               => 'Namn',
                'permissions'        => 'Behörigheter',
                'save-btn'           => 'Spara Roll',
                'title'              => 'Skapa Roll',
            ],
            'edit' => [
                'access-control'     => 'Åtkomstkontroll',
                'all'                => 'Alla',
                'all-description'    => 'Ge full åtkomst till alla behörigheter.',
                'back-btn'           => 'Tillbaka',
                'custom'             => 'Anpassad',
                'custom-description' => 'Välj enskilda behörigheter från trädet nedan.',
                'description'        => 'Beskrivning',
                'general'            => 'Allmänt',
                'name'               => 'Namn',
                'permissions'        => 'Behörigheter',
                'save-btn'           => 'Spara Roll',
                'title'              => 'Redigera Roll',
            ],
            'being-used'                      => 'Rollen används redan av en administratör',
            'being-used-by'                   => 'Rollen används redan av :name',
            'cannot-grant-unheld-permissions' => 'Du kan inte bevilja behörigheter som du inte själv har.',
            'create-success'                  => 'Rollen skapades framgångsrikt',
            'delete-failed'                   => 'Det gick inte att ta bort rollen',
            'delete-success'                  => 'Rollen togs bort framgångsrikt',
            'last-delete-error'               => 'Den sista rollen kan inte tas bort',
            'update-success'                  => 'Rollen uppdaterades framgångsrikt',
        ],
    ],
    'configuration' => [
        'index' => [
            'delete'          => 'Radera',
            'no-result-found' => 'Inget resultat hittades',
            'save-btn'        => 'Spara Konfiguration',
            'save-message'    => 'Konfigurationen sparades framgångsrikt',
            'search'          => 'Sök',
            'title'           => 'Konfiguration',
            'emails'          => [
                'info'      => 'Configure the mail sender identity and SMTP transport used for outgoing emails.',
                'title'     => 'Email',
                'configure' => [
                    'info'           => '',
                    'title'          => 'Configure',
                    'email-settings' => [
                        'title'           => 'Email Settings',
                        'info'            => 'Sender identity and SMTP server credentials for outgoing mail. Leave SMTP fields blank to keep the values from your environment file.',
                        'sender-name'     => 'Sender Name',
                        'sender-email'    => 'Sender Email',
                        'admin-name'      => 'Admin Name',
                        'admin-email'     => 'Admin Email',
                        'smtp-host'       => 'SMTP Host',
                        'smtp-port'       => 'SMTP Port',
                        'smtp-username'   => 'SMTP Username',
                        'smtp-password'   => 'SMTP Password',
                        'smtp-encryption' => 'SMTP Encryption',
                    ],
                ],
            ],
            'general' => [
                'info'    => '',
                'title'   => 'Allmänt',
                'general' => [
                    'info'  => '',
                    'title' => 'Allmänt',
                ],
                'debug' => [
                    'info'     => 'Enable verbose debug output for specific IP addresses without exposing it to everyone.',
                    'title'    => 'Debug',
                    'settings' => [
                        'info'        => 'When enabled, requests from the allow-listed IPs render with debug output and the debug bar.',
                        'title'       => 'Debug Settings',
                        'enabled'     => 'Enable IP-based Debug',
                        'allowed-ips' => 'Allowed IP Addresses (comma-separated)',
                    ],
                ],
                'microsoft-sso' => [
                    'info'  => 'Konfigurera Microsoft (Entra ID) enkel inloggning för administratörsinloggning.',
                    'title' => 'Microsoft SSO',

                    'settings' => [
                        'info'                => 'Registrera en app i Microsoft Entra ID och klistra in dess autentiseringsuppgifter här. Lämna tomt för att återgå till miljövärden. Omdirigerings-URI: /admin/login/sso/microsoft/callback.',
                        'title'               => 'Microsoft SSO-inställningar',
                        'enabled'             => 'Aktivera Microsoft SSO',
                        'tenant'              => 'Tenant ID',
                        'client-id'           => 'Client ID',
                        'client-secret'       => 'Client Secret',
                        'redirect-uri'        => 'Omdirigerings-URI',
                        'redirect-uri-info'   => 'Registrera exakt denna URL i din Microsoft Entra ID-appregistrering under Autentisering → Omdirigerings-URI:er, med plattformen Web.',
                        'redirect-uri-copy'   => 'Kopiera',
                        'redirect-uri-copied' => 'Kopierat till urklipp',
                    ],
                ],

                'magic-ai' => [
                    'info'     => 'Ställ in Magic AI alternativ.',
                    'title'    => 'Magisk AI',
                    'settings' => [
                        'enabled'         => 'Aktiverad',
                        'ai-platform'     => 'Standardplattform',
                        'ai-model'        => 'Standardmodell',
                        'max-tokens'      => 'Max antal utdatatokens',
                        'max-tokens-info' => 'Standardgränsen för hur mycket en enskild AI-generering får producera. HTML-utdata förbrukar tokens på markup, så höj värdet om genererat innehåll klipps av mitt i en mening.',
                        'title'           => 'Allmänna inställningar',
                        'title-info'      => 'Förbättra din upplevelse med Magic AI genom att ange din exklusiva API-nyckel och indikera den relevanta organisationen för smidig integration. Ta kontroll över dina OpenAI-uppgifter och anpassa inställningarna enligt dina specifika behov.',
                    ],
                    'image-generation' => [
                        'enabled'     => 'Aktiverad',
                        'ai-platform' => 'Standardplattform',
                        'ai-model'    => 'Standardmodell',
                        'title'       => 'Bildgenerering',
                        'title-info'  => 'Konfigurera den standard AI-plattform och modell för att generera produktbilder. Endast plattformar som stöder bildgenerering (OpenAI, Gemini, xAI) listas.',
                    ],
                    'translation' => [
                        'title'                       => 'Översättning',
                        'enabled'                     => 'Aktiverad',
                        'title-info'                  => 'Denna funktion aktiverar Magic AI för att översätta text, vilket gör det möjligt att smidigt konvertera text från ett språk till ett annat.',
                        'ai-platform'                 => 'Standardplattform',
                        'global-source-channel'       => 'Källkanal',
                        'global-source-locale'        => 'Källspråk',
                        'translation-model'           => 'Översättningsmodell',
                        'replace-existing-value'      => 'Ersätt befintligt värde',
                        'target-channel'              => 'Målkanal',
                        'target-locales'              => 'Målspråk',
                        'replace-existing-value-info' => 'Ersätt det befintliga värdet om det redan finns.',
                    ],
                    'agentic-pim' => [
                        'title'                     => 'Agentisk PIM',
                        'title-info'                => 'Konfigurera AI-agentchatt, autonoma berikningsarbetsflöden och kvalitetsövervakning. Dessa funktioner möjliggör konversationsbaserad produkthantering och proaktivt katalogunderhåll.',
                        'enabled'                   => 'Aktivera AI-agentchatt',
                        'enabled-info'              => 'Aktivera den flytande AI-chattwidgeten för konversationsbaserad produkthantering med över 27 verktygsåtgärder.',
                        'open-by-default'           => 'Öppna chattpanelen vid sidladdning',
                        'open-by-default-info'      => 'När detta är aktiverat öppnas AI-agentens chattpanel automatiskt första gången en administratör kommer till en sida. Användare kan fortfarande stänga den manuellt — inställningen kommer ihåg under resten av webbläsarsessionen.',
                        'max-steps'                 => 'Max agentsteg per tur',
                        'max-steps-info'            => 'Maximalt antal verktygsanropsiterationer som AI-agenten kan utföra per användarmeddelande. Högre värden tillåter mer komplexa flerstegsoperationer.',
                        'daily-token-budget'        => 'Daglig tokenbudget',
                        'daily-token-budget-info'   => 'Maximala AI-tokens (indata + utdata) som kan förbrukas per dag över alla användare. Ställ in på 0 för obegränsat. Hjälper till att kontrollera API-kostnader.',
                        'auto-enrichment'           => 'Auto-berikning vid produktskapande',
                        'auto-enrichment-info'      => 'Generera automatiskt saknade beskrivningar, kategorier och SEO-innehåll när nya produkter skapas eller importeras.',
                        'quality-monitor'           => 'Katalogkvalitetsmonitor',
                        'quality-monitor-info'      => 'Kör schemalagda skanningar för att upptäcka ofullständiga produkter, saknade översättningar, dubbletter och prisavvikelser.',
                        'confidence-threshold'      => 'Förtroendetröskel',
                        'confidence-threshold-info' => 'Minsta förtroendepoäng (0-1) för att AI-genererade ändringar ska tillämpas automatiskt. Under denna tröskel köas ändringar för mänsklig granskning.',
                        'approval-mode'             => 'Ändringsgodkännandeläge',
                        'approval-mode-info'        => 'Styr hur AI-agenten hanterar skrivoperationer. "Bekräfta och tillämpa": agenten föreslår värden i chatten, väntar på bekräftelse och utför sedan. "Strikt bekräftelse": samma sak men verifierar också efter varje ändring. "Endast föreslå": beskriver ändringar men utför aldrig (skrivskyddad).',
                        'custom-instructions'       => 'Anpassade agentinstruktioner',
                        'custom-instructions-info'  => 'Extra instruktioner som läggs till AI-agentens systemprompt — använd dem för företagskonventioner, ton eller katalogregler som agenten alltid måste följa.',
                    ],
                ],
            ],
        ],
        'integrations' => [
            'index' => [
                'create-btn' => 'Skapa',
                'title'      => 'Integrationer',
                'datagrid'   => [
                    'delete'          => 'Radera',
                    'edit'            => 'Redigera',
                    'id'              => 'ID',
                    'name'            => 'Namn',
                    'user'            => 'Användare',
                    'client-id'       => 'Klient-ID',
                    'permission-type' => 'Tillståndstyp',
                ],
            ],
            'create' => [
                'access-control' => 'Åtkomstkontroll',
                'all'            => 'Alla',
                'back-btn'       => 'Tillbaka',
                'custom'         => 'Egen',
                'general'        => 'Allmänt',
                'name'           => 'Namn',
                'permissions'    => 'Tillstånd',
                'save-btn'       => 'Spara',
                'title'          => 'Ny Integration',
            ],
            'edit' => [
                'access-control'              => 'Åtkomstkontroll',
                'all'                         => 'Alla',
                'back-btn'                    => 'Tillbaka',
                'custom'                      => 'Egen',
                'general'                     => 'Allmänt',
                'name'                        => 'Namn',
                'credentials'                 => 'Kredentialer',
                'client-id'                   => 'Klient-ID',
                'secret-key'                  => 'Sekret Nyckel',
                'api-username'                => 'API-användarnamn',
                'api-password'                => 'API-lösenord',
                'credentials-once-note'       => 'Dessa autentiseringsuppgifter visas endast en gång. Förvara dem säkert — lösenordet kan inte hämtas igen senare.',
                'credentials-info'            => 'Använd dessa autentiseringsuppgifter för att autentisera din integration.',
                'password-forgot-note'        => 'Glömt lösenordet? Generera ett nytt här — det nya lösenordet visas bara en gång.',
                'copy'                        => 'Kopiera',
                'copied'                      => 'Kopierat till urklipp',
                'regenerate-password-success' => 'API-lösenordet har genererats om.',
                'regenerate-password-btn'     => 'Generera nytt lösenord',
                'api-docs-link'               => 'Visa API-dokumentation',
                'generate-btn'                => 'Generera',
                're-secret-btn'               => 'Generera Om Sekret Nyckel',
                'permissions'                 => 'Tillstånd',
                'save-btn'                    => 'Spara',
                'title'                       => 'Redigera Integration',
            ],
            'being-used'                     => 'API-integration används redan av Admin-användaren',
            'create-success'                 => 'API-integration skapades framgångsrikt',
            'delete-failed'                  => 'API-integration kunde inte raderas',
            'delete-success'                 => 'API-integration raderades framgångsrikt',
            'last-delete-error'              => 'Sista API-integration kan inte raderas',
            'update-success'                 => 'API-integration uppdaterades framgångsrikt',
            'generate-key-success'           => 'API-nyckel genererades framgångsrikt',
            're-generate-secret-key-success' => 'API-sekret-nyckel genererades om framgångsrikt',
            'client-not-found'               => 'Klient hittades inte',
        ],
        'prompt' => [
            'index' => [
                'title' => 'Prompts',
            ],
            'datagrid' => [
                'id'               => 'ID',
                'title'            => 'Titel',
                'prompt'           => 'Uppmaning',
                'type'             => 'Typ',
                'purpose'          => 'Syfte',
                'text-generation'  => 'Textgenerering',
                'image-generation' => 'Bildgenerering',
                'translation'      => 'Översättning',
                'created-at'       => 'Skapad',
                'updated-at'       => 'Uppdaterad',
                'actions'          => 'Åtgärder',
                'edit'             => 'Redigera',
                'delete'           => 'Radera',
                'product'          => 'Produkt',
                'category'         => 'Kategori',
            ],
            'create' => [
                'title'               => 'Uppmaning',
                'create-btn'          => 'Skapa Uppmaning',
                'edit-title'          => 'Redigera Uppmaning',
                'create-title'        => 'Skapa Ny Uppmaning',
                'label-title'         => 'Titel',
                'purpose'             => 'Syfte',
                'text-generation'     => 'Textgenerering',
                'image-generation'    => 'Bildgenerering',
                'select-purpose'      => 'Välj syfte',
                'select-model'        => 'Välj modell',
                'loading-models'      => 'Laddar modeller...',
                'no-models-available' => 'Inga modeller tillgängliga. Välj en plattform eller konfigurera en under fliken Plattformar.',
                'type'                => 'Typ',
                'prompt'              => 'Uppmaning',
                'save-btn'            => 'Spara Uppmaning',
            ],
            'message' => [
                'save-success'   => 'Uppmaningen sparades framgångsrikt.',
                'update-success' => 'Uppmaningen uppdaterades framgångsrikt.',
                'delete-success' => 'Uppmaningen raderades framgångsrikt.',
                'delete-fail'    => 'Misslyckades med att radera uppmaningen.',
            ],
        ],
        'system-prompt' => [
            'index' => [
                'title' => 'Systemprompts',
            ],
            'datagrid' => [
                'id'          => 'ID',
                'title'       => 'Titel',
                'tone'        => 'Ton',
                'max-tokens'  => 'Max tokens',
                'temperature' => 'Temperatur',
                'status'      => 'Status',
                'created-at'  => 'Skapad',
                'updated-at'  => 'Uppdaterad',
                'actions'     => 'Åtgärder',
                'save'        => 'Spara',
                'edit'        => 'Redigera',
                'delete'      => 'radera',
            ],
            'create' => [
                'title'               => 'Systemprompt',
                'create-btn'          => 'Skapa systemprompt',
                'edit-title'          => 'Redigera systemprompt',
                'create-title'        => 'Skapa ny systemprompt',
                'label-title'         => 'Titel',
                'max-tokens'          => 'Max utdata-tokens',
                'temperature'         => 'Temperatur',
                'type'                => 'Typ',
                'prompt'              => 'Prompt',
                'save-btn'            => 'Spara prompt',
                'prompt-tone'         => 'Promptton',
                'max-tokens-info'     => 'Tillåtet intervall för utdatatokens: 100 till 5000 tokens.',
                'temperature-info'    => 'Temperatur styr kreativiteten. Intervall: 0 till 2. Lägre värden (t.ex. 0,4) ger mer exakta och fokuserade svar.',
                'max-tokens-tooltip'  => 'Tillåtet intervall för maximala utdata-tokens: 100 till 5000 tokens',
                'temperature-tooltip' => 'Temperatur styr kreativiteten. Intervall: 0 till 2. Lägre värden (t.ex. 0,4) ger mer exakta och fokuserade svar.',
            ],
            'message' => [
                'save-success'   => 'Systemprompt sparad framgångsrikt.',
                'update-success' => 'Systemprompt uppdaterad framgångsrikt.',
                'delete-success' => 'Systemprompt raderad framgångsrikt.',
                'delete-fail'    => 'Radering av systemprompt misslyckades.',
            ],
        ],
        'platform' => [
            'title'           => 'AI-plattformar',
            'create-btn'      => 'Lägg till plattform',
            'create-title'    => 'Lägg till AI-plattform',
            'edit-title'      => 'Redigera AI-plattform',
            'save-btn'        => 'Spara',
            'saving'          => 'Sparar',
            'test-connection' => 'Testa anslutning',
            'setup'           => [
                'title'              => 'Kom igång med Magic AI',
                'description'        => 'Anslut din AI-leverantör för att låsa upp innehållsgenerering, bildskapande och översättning med AI. Lägg till din första plattform för att börja.',
                'step-1'             => 'Välj en leverantör',
                'step-2'             => 'Ange din API-nyckel',
                'step-3'             => 'Välj modeller och spara',
                'add-first'          => 'Lägg till din första plattform',
                'no-default-warning' => 'Ingen standardplattform är angiven. Vänligen ange en plattform som standard för att aktivera AI-funktioner i hela applikationen.',
                'no-platform-hint'   => 'Inga plattformar konfigurerade ännu. Gå till fliken Plattformar för att lägga till din första AI-leverantör.',
                'lightning-icon'     => 'Magisk AI',
                'warning-icon'       => 'Varning',
            ],

            'errors' => [
                'no-models-returned' => 'Inga modeller returnerades. Lägg till modeller manuellt nedan.',
                'fetch-failed'       => 'Kunde inte hämta. Du kan lägga till modeller manuellt nedan.',
                'save-failed'        => 'Sparandet misslyckades. Försök igen.',
                'generic'            => 'Ett fel uppstod.',
            ],
            'testing'         => 'Testar',
            'fetch-models'    => 'Hämta modeller',
            'fetching-models' => 'Hämtar',
            'set-default'     => 'Ange som standard',
            'managed-badge'   => 'Hanterad',
            'datagrid'        => [
                'label'      => 'Etikett',
                'provider'   => 'Leverantör',
                'models'     => 'Modeller',
                'default'    => 'Standard',
                'status'     => 'Status',
                'created-at' => 'Skapad',
                'edit'       => 'Redigera',
                'delete'     => 'Radera',
            ],
            'fields' => [
                'label'                    => 'Etikett',
                'provider'                 => 'Leverantör',
                'select-provider'          => 'Välj leverantör',
                'api-url'                  => 'API-URL',
                'api-url-hint'             => 'Ifylld med standardändpunkten. Redigera bara om du använder en proxy eller anpassad distribution.',
                'api-key'                  => 'API-nyckel',
                'models'                   => 'Modeller',
                'custom-model-placeholder' => 'Ange anpassat modell-ID...',
                'add'                      => 'Lägg till',
                'enter-key-to-fetch'       => 'Ange din API-nyckel ovan för att automatiskt hämta tillgängliga modeller.',
                'search-models'            => 'Sök modeller...',
                'no-models-match'          => 'Inga modeller matchar din sökning.',
                'azure-deployment'         => 'Distributionsnamn',
                'azure-api-version'        => 'API-version',
                'is-default'               => 'Ange som standard',
                'status'                   => 'Status',
                'use-default'              => 'Använd standardplattform',
                'use-default-hint'         => 'Lämna tomt för att använda plattformen markerad som standard. Plattformar markerade med * är standard.',
                'no-image-platform'        => 'Ingen bildkompatibel plattform konfigurerad. Lägg till en OpenAI-, Gemini- eller xAI-plattform först.',
                'image-platform-hint'      => 'Endast plattformar som stöder bildgenerering visas (OpenAI, Gemini, xAI).',
                'remove-model'             => 'Ta bort modell :model',
                'invalid-model-name'       => 'Ogiltigt modellnamn. Använd endast bokstäver, siffror, bindestreck, punkter, kolon och snedstreck (t.ex. gpt-4o, claude-3-sonnet).',
            ],
            'message' => [
                'save-success'                       => 'Plattform sparad framgångsrikt.',
                'update-success'                     => 'Plattform uppdaterad framgångsrikt.',
                'delete-success'                     => 'Plattform raderad framgångsrikt.',
                'delete-fail'                        => 'Radering av plattform misslyckades.',
                'not-found'                          => 'Den valda plattformen finns inte längre. Uppdatera sidan och försök igen.',
                'cannot-delete-default'              => 'Kan inte radera den enda standardplattformen.',
                'set-default-success'                => 'Plattform angiven som standard framgångsrikt.',
                'test-success'                       => 'Anslutning verifierad framgångsrikt!',
                'test-fail'                          => 'Anslutningstest misslyckades',
                'no-test-model'                      => 'Välj minst en textkapabel modell för att köra anslutningstestet.',
                'custom-api-url-required'            => 'API URL krävs för anpassade leverantörer så att anslutningstestet når din egen slutpunkt.',
                'unsafe-api-url'                     => 'API URL:en pekar på en begränsad eller ogiltig adress och kan inte användas.',
                'extras-reserved-key'                => 'Fältet extra kan inte åsidosätta :keys. Använd fälten API-URL och API-nyckel i stället.',
                'extras-invalid-json'                => 'Fältet extra måste innehålla ett giltigt JSON-objekt.',
                'extras-too-large'                   => 'Fältet extra är för stort.',
                'custom-provider-error'              => 'Fel hos anpassad leverantör',
                'ai-response-timeout'                => 'AI-svaret tar längre tid än väntat. Försök igen.',
                'no-model-configured'                => 'Ingen modell är konfigurerad för den valda plattformen.',
                'no-platform-configured'             => 'Ingen AI-plattform är konfigurerad. Lägg till en plattform under Konfiguration > Magic AI > Plattformar.',
                'images-unsupported'                 => 'Leverantören :provider stöder inte bildgenerering. Använd OpenAI, Gemini eller xAI.',
                'api-key-corrupted'                  => 'Den lagrade API-nyckeln kunde inte dekrypteras (:error). Applikationens krypteringsnyckel kan ha ändrats. Ange din API-nyckel på nytt och spara.',
                'fetch-models-fail'                  => 'Kunde inte hämta modeller',
                'fetch-models-http-error'            => 'Slutpunkten på :host returnerade HTTP :status.',
                'fetch-models-unreachable'           => 'Slutpunkten på :host kunde inte nås.',
                'invalid-model-names'                => 'Ogiltiga modellnamn: :names. Modellnamn måste börja med en bokstav eller siffra och får bara innehålla bokstäver, siffror, bindestreck, punkter, kolon och snedstreck.',
                'managed-connection-locked'          => 'Den här API-nyckeln kan bara användas med sin egen leverantör och sin egen slutpunkt.',
                'managed-models-only'                => 'Den här API-nyckeln kan bara använda dessa modeller: :allowed.',
                'managed-not-configured'             => 'Den hanterade AI-plattformen är inte konfigurerad. Ange MAGIC_AI_MANAGED_MODELS.',
                'managed-provisioned'                => 'Den hanterade AI-plattformen ":label" är klar.',
                'managed-cannot-delete'              => 'Den hanterade plattformen kan inte tas bort.',
                'managed-key-prompt'                 => 'API-nyckel för den hanterade plattformen (lämna tomt för att behålla den sparade nyckeln)',
                'managed-key-required'               => 'En API-nyckel krävs för att skapa den hanterade plattformen. Kör kommandot interaktivt och ange nyckeln.',
                'default-requires-enabled'           => 'En inaktiverad plattform kan inte markeras som standard. Aktivera plattformen först.',
            ],
        ],
    ],
    'components' => [
        'accordion' => [
            'expand'   => 'Expandera',
            'collapse' => 'Fäll ihop',
        ],

        'associations' => [
            'type-search' => [
                'add-btn'            => 'Lägg till',
                'already-added'      => 'Tillagd',
                'empty-info'         => 'Prova en annan sökning.',
                'empty-title'        => 'Inga associationstyper hittades',
                'search-placeholder' => 'Sök efter namn eller kod',
                'select-all'         => 'Välj alla',
                'title'              => 'Lägg till associationstyp',
            ],
            'product-picker' => [
                'add-btn'  => 'Lägg till valda',
                'selected' => ':count produkter valda',
                'title'    => 'Välj produkter',
            ],
        ],
        'pagination' => [
            'page' => 'Sida',
        ],
        'search' => [
            'placeholder' => 'Sök',
        ],
        'flash-group' => [
            'close' => 'Stäng',
        ],
        'form' => [
            'translatable-field' => [
                'translated-count' => ':filled av :total översatta',
            ],
            'searchable-menu' => [
                'empty'  => 'Inga resultat',
                'search' => 'Sök',
            ],
            'tags'            => [
                'placeholder'     => 'Skriv eller klistra in värden separerade med kommatecken eller nya rader',
                'remove'          => 'Ta bort :value',
                'clear-all'       => 'Rensa alla',
                'tag-placeholder' => 'Tryck på Enter för att lägga till',
            ],
            'ajax-error'      => 'Något gick fel när det skulle sparas. Försök igen.',
            'file-uploader'   => [
                'upload-cta'  => 'Klicka för att ladda upp',
                'upload-hint' => 'eller dra och släpp',
            ],
            'unsaved-changes' => [
                'title'           => 'Du har osparade ändringar',
                'sections'        => ':count sektioner ändrade',
                'fields'          => ':count fält ändrade',
                'section'         => ':count sektion ändrad',
                'field'           => ':count fält ändrat',
                'discard'         => 'Ångra',
                'save'            => 'Spara ändringar',
                'saving'          => 'Sparar…',
                'save-failed'     => 'Dina ändringar sparades inte. Granska de markerade fälten.',
                'field-badge'     => 'Osparat',
                'leave-message'   => 'Lämna sidan? Dina osparade ändringar går förlorade.',
                'stay'            => 'Stanna på sidan',
                'leave'           => 'Lämna',
                'discard-title'   => 'Ångra ändringar?',
                'discard-message' => 'Detta återställer alla dina osparade ändringar i detta formulär.',
                'cancel'          => 'Avbryt',
            ],
        ],
        'layouts' => [
            'side-rail' => [
                'collapse' => 'Fäll ihop panelen',
                'expand'   => 'Expandera panelen',
            ],
            'breadcrumbs' => [
                'label' => 'Brödsmulor',
            ],

            'header' => [
                'logo'          => 'Logotyp',
                'account-title' => 'Konto',
                'app-version'   => 'Version : :version',
                'logout'        => 'Logga ut',
                'my-account'    => 'Mitt Konto',
                'notifications' => 'Meddelanden',
                'visit-shop'    => 'Besök Butik',
                'theme-auto'    => 'Theme: Auto',
                'theme-dark'    => 'Theme: Dark',
                'theme-light'   => 'Theme: Light',
            ],
            'sidebar' => [
                'attribute-families' => 'Attributfamiljer',
                'attribute-groups'   => 'Attributgrupper',
                'attributes'         => 'Attribut',
                'history'            => 'Historik',
                'edit-section'       => 'Data',
                'general'            => 'Allmänt',
                'catalog'            => 'Katalog',
                'categories'         => 'Kategorier',
                'category_fields'    => 'Kategorifält',
                'association_types'  => 'Associationstyper',
                'channels'           => 'Kanaler',
                'collapse'           => 'Kollapsa',
                'configure'          => 'Konfiguration',
                'currencies'         => 'Valutor',
                'dashboard'          => 'Instrumentpanel',
                'data-transfer'      => 'Dataöverföring',
                'groups'             => 'Grupper',
                'help'               => 'Hjälp',
                'tracker'            => 'Jobb Tracker',
                'imports'            => 'Importer',
                'exports'            => 'Exporter',
                'locales'            => 'Lokaliseringsinställningar',
                'magic-ai'           => 'Magisk AI',
                'mode'               => 'Mörkt Läge',
                'products'           => 'Produkter',
                'roles'              => 'Roller',
                'settings'           => 'Inställningar',
                'system'             => 'System',
                'system-information' => 'Systeminformation',
                'system-settings'    => 'Systeminställningar',
                'smtp'               => 'SMTP Configuration',
                'debug'              => 'Debug',
                'appearance'         => 'Appearance',
                'themes'             => 'Teman',
                'users'              => 'Användare',
                'integrations'       => 'Integrationer',
                'prompt'             => 'Prompt',
                'general-tab'        => 'Konfiguration',
            ],
        ],
        'datagrid' => [
            'index' => [
                'no-records-selected'              => 'Inga poster har valts.',
                'must-select-a-mass-action-option' => 'Du måste välja ett massaktionsval.',
                'must-select-a-mass-action'        => 'Du måste välja en massaktion.',
                'link-copied'                      => 'Länken har kopierats till urklipp.',
                'copy-failed'                      => 'Kopiering misslyckades. Kopiera länken manuellt.',
            ],
            'toolbar' => [
                'length-of'           => ':length av',
                'of'                  => 'av',
                'per-page'            => 'Per Sida',
                'results'             => ':total Resultat',
                'selected'            => ':total Valda',
                'select-all-matching' => 'Markera alla :total',
                'all-selected'        => 'Alla matchande poster markerade',
                'clear-selection'     => 'Rensa markering',
                'selection-options'   => 'Markeringsalternativ',
                'mass-actions'        => [
                    'submit'        => 'Skicka',
                    'select-option' => 'Välj Alternativ',
                    'select-action' => 'Välj Aktion',
                ],
                'filter' => [
                    'title' => 'Filter',
                ],
                'search_by' => [
                    'code'        => 'Sök efter kod',
                    'code_or_id'  => 'Sök efter kod eller ID',
                    'sku_or_user' => 'Sök efter SKU eller användare',
                ],
                'search' => [
                    'title' => 'Sök',
                ],
                'manage-columns' => [
                    'title' => 'kolumner',
                ],
                'pagination' => [
                    'first-page'    => 'Första sidan',
                    'previous-page' => 'Föregående sida',
                    'next-page'     => 'Nästa sida',
                    'last-page'     => 'Sista sidan',
                    'page-number'   => 'Sidnummer',
                ],
            ],
            'filters' => [
                'select'          => 'Välj',
                'title'           => 'Använd Filter',
                'save'            => 'Tillämpa',
                'add-filter'      => 'Lägg till filter',
                'back'            => 'Tillbaka',
                'done'            => 'Klar',
                'remove-filter'   => 'Ta bort filter',
                'search-filter'   => 'Sök...',
                'no-value'        => 'Alla',
                'values-selected' => ':count valda',
                'saved-filters'   => [
                    'title'        => 'Sparade filter',
                    'none'         => 'Inga sparade filter',
                    'unsaved'      => 'Osparade ändringar',
                    'clear'        => 'Rensa alla filter',
                    'name'         => 'Filternamn',
                    'share'        => 'Dela med alla administratörer',
                    'save'         => 'Spara filter',
                    'saved'        => 'Filtret sparades',
                    'deleted'      => 'Filtret togs bort',
                    'shared'       => 'Delat',
                    'delete'       => 'Ta bort filter',
                ],
                'dropdown'        => [
                    'searchable' => [
                        'atleast-two-chars' => 'Skriv minst 2 tecken...',
                        'no-results'        => 'Inga resultat funna...',
                    ],
                ],
                'custom-filters' => [
                    'clear-all' => 'Rensa Alla',
                    'clear'     => 'Rensa',
                    'title'     => 'Anpassade Filter',
                ],
                'boolean-options' => [
                    'false' => 'Falskt',
                    'true'  => 'Sann',
                ],
                'date-options' => [
                    'last-month'        => 'Sista Månaden',
                    'last-six-months'   => 'Sista 6 Månaderna',
                    'last-three-months' => 'Sista 3 Månaderna',
                    'this-month'        => 'Denna Månaden',
                    'this-week'         => 'Denna Vecka',
                    'this-year'         => 'Detta Året',
                    'today'             => 'Idag',
                    'yesterday'         => 'Igår',
                ],
            ],
            'table' => [
                'actions'              => 'Åtgärder',
                'no-records-available' => 'Inga poster tillgängliga.',
                'no-records-filtered'  => 'Inga poster matchar de filter du har använt.',
                'thumbnail'            => 'Miniatyr',
            ],
            'manage-columns' => [
                'title'             => 'Hantera kolumner',
                'available-columns' => 'Tillgängliga kolumner',
                'selected-columns'  => 'Valda kolumner',
                'apply-btn'         => 'Tillämpas',
            ],
        ],
        'modal' => [
            'confirm' => [
                'agree-btn'    => 'Håller med',
                'disagree-btn' => 'Oenig',
                'message'      => 'Är du säker på att du vill utföra denna åtgärd?',
                'title'        => 'Är du säker?',
            ],
            'delete' => [
                'agree-btn'    => 'Radera',
                'disagree-btn' => 'Avbryt',
                'message'      => 'Är du säker på att du vill radera?',
                'title'        => 'Bekräfta Radering',
            ],
            'history' => [
                'title'           => 'Historik Förhandsgranskning',
                'subtitle'        => 'Granska snabbt dina uppdateringar och ändringar.',
                'close-btn'       => 'Stäng',
                'version-label'   => 'Version',
                'date-time-label' => 'Datum/Tid',
                'user-label'      => 'Användare',
                'name-label'      => 'Nyckel',
                'old-value-label' => 'Gammal Värde',
                'new-value-label' => 'Ny Värde',
                'no-history'      => 'Ingen historik funnen',
            ],
        ],
        'products' => [
            'search' => [
                'search-by-sku' => 'Sök efter SKU',
                'add-btn'       => 'Lägg Till Vald Produkt',
                'empty-info'    => 'Inga produkter tillgängliga för sökterm.',
                'empty-title'   => 'Inga produkter funna',
                'product-image' => 'Produktbild',
                'qty'           => ':qty Tillgängliga',
                'sku'           => 'SKU - :sku',
                'title'         => 'Välj Produkter',
            ],
        ],
        'media' => [
            'images' => [
                'add-image-btn'      => 'Lägg Till Bild',
                'drag-drop-hint'     => 'Dra och släpp eller klicka för att ladda upp',
                'add-media-btn'      => 'Lägg Till Media',
                'generate-with-ai'   => 'Generera med AI',
                'upload-from-device' => 'Ladda upp från enhet',
                'cancel'             => 'Avbryt',
                'ai-add-image-btn'   => 'Magisk AI',
                'ai-btn-info'        => 'Generera Bild',
                'allowed-types'      => 'gif, jfif, jif, jpeg, jpg, pdf, png, psd, tif, tiff, webp, bmp',
                'not-allowed-error'  => 'Endast bildfiler (.jpeg, .jpg, .png, ..) är tillåtna.',
                'upload-media-btn'   => 'Ladda upp media',
                'preview-image'      => 'Förhandsgranska bild',
                'close-preview'      => 'Stäng förhandsvisning',
                'replace-image'      => 'Ersätt bild',
                'delete-image'       => 'Ta bort bild',
                'ai-generation'      => [
                    '1024x1024'        => '1024x1024',
                    '1024x1792'        => '1024x1792',
                    '1792x1024'        => '1792x1024',
                    'apply'            => 'Använd',
                    'dall-e-2'         => 'Dall.E 2',
                    'dall-e-3'         => 'Dall.E 3',
                    'generate'         => 'Generera',
                    'generating'       => 'Genererar...',
                    'hd'               => 'HD',
                    'model'            => 'Modell',
                    'number-of-images' => 'Antal Bilder',
                    'prompt'           => 'Prompt',
                    'quality'          => 'Kvalitet',
                    'regenerate'       => 'Regenerera',
                    'regenerating'     => 'Regenererar...',
                    'size'             => 'Storlek',
                    'standard'         => 'Standard',
                    'title'            => 'AI Bildgenerering',
                ],
                'placeholders' => [
                    'front'     => 'Fram',
                    'next'      => 'Nästa',
                    'size'      => 'Storlek',
                    'use-cases' => 'Användningsområden',
                    'zoom'      => 'Zooma',
                ],
            ],
            'image-viewer' => [
                'actual-size'   => 'Verklig storlek',
                'close'         => 'Stäng',
                'fit-to-screen' => 'Anpassa till skärm',
                'reset-all'     => 'Återställ',
                'rotate-left'   => 'Rotera åt vänster',
                'rotate-right'  => 'Rotera åt höger',
                'zoom-in'       => 'Zooma in',
                'zoom-out'      => 'Zooma ut',
            ],

            'gallery' => [
                'not-allowed-error' => 'Endast bild- och videofiler är tillåtna. (.mp4, .jpg ..)',
            ],
            'videos' => [
                'add-video-btn'     => 'Lägg Till Video',
                'allowed-types'     => 'mp4, webm, mkv',
                'not-allowed-error' => 'Endast videofiler (.mp4, .mov, .ogg ..) är tillåtna.',
            ],
            'files' => [
                'add-file-btn'        => 'Lägg Till Fil',
                'allowed-types'       => 'csv, doc, docx, mp3, pdf, ppt, pptx, rtf, svg, txt, wav',
                'not-allowed-error'   => 'Endast PDF-filer är tillåtna',
                'preview-unavailable' => 'Förhandsvisning är inte tillgänglig för den här filtypen.',
            ],
        ],
        'tinymce' => [
            'ai-btn-tile'   => 'Magisk AI',
            'ai-generation' => [
                'apply'                  => 'Använd',
                'generate'               => 'Generera',
                'generated-content'      => 'Genererat Innehåll',
                'generated-content-info' => 'AI-genererat innehåll kan vara vilseledande. Vänligen granska det genererade innehållet innan du använder det.',
                'generating'             => 'Genererar...',
                'prompt'                 => 'Prompt',
                'title'                  => 'AI Assistans',
                'platform'               => 'Plattform',
                'model'                  => 'Modell',
                'rich-preview'           => 'Rik förhandsgranskning',
                'system-prompt'          => 'Systemprompt',
                'select-system-prompt'   => 'Välj systemprompt',
                'tone-instructions'      => 'Ton / Instruktioner',
                'max-tokens'             => 'Max tokens',
                'temperature'            => 'Temperatur',
                'truncated'              => 'Modellen nådde sin tokengräns, så innehållet kan vara avklippt. Höj max antal tokens och generera igen.',
                'gpt-3-5-turbo'          => 'OpenAI gpt-3.5-turbo',
                'llama2'                 => 'Llama 2',
                'mistral'                => 'Mistral',
                'dolphin-phi'            => 'Delfin Phi',
                'phi'                    => 'Phi-2',
                'starling-lm'            => 'Starling',
                'llama2-uncensored'      => 'Llama 2 Uncensurerad',
                'llama2:13b'             => 'Llama 2 13B',
                'llama2:70b'             => 'Llama 2 70B',
                'orca-mini'              => 'Orca Mini',
                'vicuna'                 => 'Vicuna',
                'llava'                  => 'LLaVA',
                'default-prompt'         => 'Standardprompt',
                'select-prompt-template' => 'Välj en promptmall',
            ],
        ],
    ],
    'help' => [
        'index' => [
            'title'       => 'Hjälp & Resurser',
            'subtitle'    => 'Allt du behöver för att få ut mesta möjliga av UnoPim — hosting, support och professionella tjänster, plus tillägg och utvecklardokumentation.',
            'services'    => 'Tjänster',
            'resources'   => 'Resurser & Dokumentation',
            'system-info' => 'Systeminformation',
        ],
        'system-info' => [
            'title'    => 'Systeminformation',
            'subtitle' => 'Körmiljö, serverkonfiguration och installerade paket för den här UnoPim-instansen.',
            'enabled'  => 'Aktiverad',
            'disabled' => 'Inaktiverad',
            'package'  => 'Paket',

            'search-packages' => 'Sök paket...',
            'no-packages'     => 'Inga paket matchar din sökning.',
            'sections'        => [
                'application' => 'Applikation',
                'server'      => 'Server',
                'database'    => 'Databas',
                'services'    => 'Tjänster',
                'extensions'  => 'PHP-tillägg',
                'packages'    => 'Installerade paket',
            ],
            'fields' => [
                'unopim_version'        => 'UnoPim-version',
                'laravel_version'       => 'Laravel-version',
                'php_version'           => 'PHP-version',
                'environment'           => 'Miljö',
                'debug_mode'            => 'Felsökningsläge',
                'timezone'              => 'Tidszon',
                'locale'                => 'Språkinställning',
                'url'                   => 'Applikations-URL',
                'operating_system'      => 'Operativsystem',
                'server_software'       => 'Serverprogramvara',
                'php_sapi'              => 'PHP SAPI',
                'memory_limit'          => 'Minnesgräns',
                'max_execution_time'    => 'Maximal körningstid',
                'upload_max_filesize'   => 'Maximal filstorlek för uppladdning',
                'post_max_size'         => 'Maximal POST-storlek',
                'connection'            => 'Anslutning',
                'driver'                => 'Drivrutin',
                'version'               => 'Version',
                'cache_driver'          => 'Cachedrivrutin',
                'queue_connection'      => 'Köanslutning',
                'session_driver'        => 'Sessionsdrivrutin',
                'mail_mailer'           => 'E-posttjänst',
                'elasticsearch_enabled' => 'Elasticsearch',
            ],
        ],
        'cards' => [
            'cloud-hosting' => [
                'title'       => 'Molnhosting',
                'description' => 'Kostnadseffektiv, hanterad molnhosting — testa och lansera UnoPim i molnet på några minuter, fullt optimerad och skalbar.',
            ],
            'support' => [
                'title'       => 'Support & Underhåll',
                'description' => 'Dedikerad teknisk support och löpande underhållsplaner för att hålla din PIM säker, uppdaterad och i smidig drift.',
            ],
            'services' => [
                'title'       => 'Betaltjänster',
                'description' => 'Expertstöd för modulintegration, anpassning, datamigrering, versionsuppgraderingar och skräddarsydd utveckling.',
            ],
            'extensions' => [
                'title'       => 'Tillägg',
                'description' => 'Bläddra bland officiella och community-tillägg för att utöka UnoPim med nya kopplingar, kanaler och funktioner.',
            ],
            'user-guide' => [
                'title'       => 'Användarguide',
                'description' => 'Utvecklarguider, handledningar och de senaste artiklarna som hjälper dig att bygga, konfigurera och hålla dig uppdaterad.',
            ],
            'api-docs' => [
                'title'       => 'API-dokumentation',
                'description' => 'Fullständig REST API-referens med endpoints, autentisering och exempel för att integrera UnoPim med din miljö.',
            ],
        ],
        'cta' => [
            'title'  => 'Behöver du fortfarande hjälp?',
            'sub'    => 'Prata med UnoPim-teamet om hosting, anpassad utveckling eller något annat.',
            'button' => 'Kontakta oss',
        ],
        'banners' => [
            'cloud' => [
                'tag'     => 'Molnhosting',
                'message' => 'Lansera UnoPim på fullt hanterad molnhosting — <b>snabba, säkra och kostnadseffektiva planer.</b>',
                'cta'     => 'Visa planer',
            ],
            'upgrade' => [
                'tag'     => 'Uppdatering tillgänglig',
                'message' => 'Du kör <b>UnoPim v:version.</b> En nyare version är tillgänglig — uppgradera för de senaste funktionerna &amp; korrigeringarna.',
                'cta'     => 'Välj uppgraderingstjänst',
            ],
            'dont-show-again' => 'Visa inte igen',
            'dismissed'       => 'Inställningen sparad.',
        ],
    ],
    'acl' => [
        'passports'                => 'Produktpass',
        'publish'                  => 'Publicera',
        'withdraw'                 => 'Dra tillbaka',
        'addresses'                => 'Adresser',
        'attribute-families'       => 'Attributfamiljer',
        'attribute-groups'         => 'Attributgrupper',
        'attributes'               => 'Attribut',
        'cancel'                   => 'Avbryt',
        'catalog'                  => 'Katalog',
        'categories'               => 'Kategorier',
        'channels'                 => 'Kanaler',
        'configure'                => 'Konfigurera',
        'configuration'            => 'Konfiguration',
        'copy'                     => 'Kopiera',
        'create'                   => 'Skapa',
        'currencies'               => 'Valutor',
        'dashboard'                => 'Instrumentpanel',
        'data-transfer'            => 'Dataöverföring',
        'delete'                   => 'Radera',
        'edit'                     => 'Redigera',
        'email-templates'          => 'E-postmallar',
        'events'                   => 'Händelser',
        'groups'                   => 'Grupper',
        'help'                     => 'Hjälp',
        'import'                   => 'Importera',
        'imports'                  => 'Importer',
        'invoices'                 => 'Fakturor',
        'locales'                  => 'Lokaler',
        'magic-ai'                 => 'Magisk AI',
        'magic-ai-general'         => 'Allmänt',
        'magic-ai-prompt'          => 'Prompt',
        'magic-ai-system-prompt'   => 'Systemprompt',
        'marketing'                => 'Marknadsföring',
        'newsletter-subscriptions' => 'Nyhetsbrevsprenumerationer',
        'note'                     => 'Anteckning',
        'orders'                   => 'Beställningar',
        'products'                 => 'Produkter',
        'promotions'               => 'Erbjudanden',
        'refunds'                  => 'Återbetalningar',
        'reporting'                => 'Rapportering',
        'reviews'                  => 'Recensioner',
        'roles'                    => 'Roller',
        'appearance'               => 'Appearance',
        'sales'                    => 'Försäljning',
        'search-seo'               => 'Sök & SEO',
        'search-synonyms'          => 'Söktermer & synonymer',
        'search-terms'             => 'Söktermer',
        'settings'                 => 'Inställningar',
        'shipments'                => 'Frakt',
        'sitemaps'                 => 'Sitemap',
        'subscribers'              => 'Prenumeranter',
        'smtp'                     => 'SMTP Configuration',
        'debug'                    => 'Debug',
        'system'                   => 'System',
        'system-information'       => 'Systeminformation',
        'system-settings'          => 'Systeminställningar',
        'tax-categories'           => 'Skattekategorier',
        'tax-rates'                => 'Skattesatser',
        'taxes'                    => 'Skatter',
        'themes'                   => 'Teman',
        'integration'              => 'Integration',
        'url-rewrites'             => 'URL-omskrivningar',
        'users'                    => 'Användare',
        'user-management'          => 'Användarhantering',
        'category_fields'          => 'Kategori-fält',
        'association_types'        => 'Associationstyper',
        'association_type_fields'  => 'Associationstypfält',
        'view'                     => 'Visa',
        'execute'                  => 'Utföra',
        'history'                  => 'Historik',
        'restore'                  => 'Återställ',
        'integrations'             => 'Integrationer',
        'api'                      => 'API',
        'tracker'                  => 'Uppgiftsspårning',
        'exports'                  => 'Exporter',
        'mass-update'              => 'Massuppdatering',
        'mass-delete'              => 'Massradering',
        'bulk-edit'                => 'Massredigering',
        'quick-export'             => 'Snabbexport',
        'variant-structures'       => 'Variantstrukturer',
        'completeness'             => 'Fullständighet',
    ],
    'errors' => [
        'dashboard' => 'Instrumentpanel',
        'go-back'   => 'Tillbaka',
        'support'   => 'Om problemet kvarstår, vänligen kontakta oss via <a href=":link" class=":class">:email</a> för hjälp.',
        '404'       => [
            'description' => 'Oops! Sidan du söker är på semester. Det verkar som att vi inte kan hitta vad du söker.',
            'title'       => '404 Sidan inte hittad',
        ],
        '401' => [
            'description' => 'Oops! Det verkar som att du inte har rätt behörigheter för att komma åt denna sida. Det verkar som att du saknar de nödvändiga behörigheterna.',
            'title'       => '401 Otillåten',
            'message'     => 'Autentiseringen misslyckades på grund av ogiltiga behörigheter eller utgången token.',
        ],
        '403' => [
            'description' => 'Oops! Denna sida är begränsad. Det verkar som att du inte har behörighet att se detta innehåll.',
            'title'       => '403 Förbjuden',
            'message'     => 'Du har inte behörighet att komma åt den här sidan. Kontakta din administratör om du anser att detta är ett fel.',
        ],
        '413' => [
            'description' => 'Oops! Det verkar som att du försöker ladda upp en för stor fil. Om du vill ladda upp den, vänligen uppdatera PHP-konfigurationen.',
            'title'       => '413 Innehåll för stort',
        ],
        '419' => [
            'description' => 'Oops! Din session har löpt ut. Vänligen uppdatera sidan och logga in igen för att fortsätta.',
            'title'       => '419 Sessionen har löpt ut',
        ],
        '405' => [
            'description' => 'Hoppsan! Begäranmetoden är inte tillåten för denna URL. Använd rätt metod eller återgå till föregående sida.',
            'title'       => '405 Metod Ej Tillåten',
        ],
        '429' => [
            'description' => 'Oops! Too many requests have been made in a short time. Please wait a moment and try again.',
            'title'       => '429 Too Many Requests',
        ],
        '500' => [
            'description' => 'Oops! Något gick fel. Det verkar som att vi har problem med att ladda den sida du söker.',
            'title'       => '500 Intern serverfel',
        ],
        '503' => [
            'description' => 'Oops! Det verkar som att vi är tillfälligt otillgängliga för underhåll. Kontrollera igen snart.',
            'title'       => '503 Tjänsten inte tillgänglig',
        ],
    ],
    'export' => [
        'csv'                  => 'CSV',
        'download'             => 'Ladda ner',
        'export'               => 'Snabb export',
        'no-records'           => 'Inga poster att exportera',
        'xls'                  => 'XLS',
        'xlsx'                 => 'XLSX',
        'with-media'           => 'Med media',
        'export-success'       => 'Exporten lyckades!',
        'error'                => 'Något gick fel!',
        'product-not-selected' => 'Inga produkter valda för snabbexport',
    ],
    'validations' => [
        'slug-being-used' => 'Denna slug används i antingen kategorier eller produkter.',
        'slug-reserved'   => 'Denna slug är reserverad.',
        'invalid-locale'  => 'Ogiltiga lokaler :locales',
    ],
    'footer' => [
        'copy-right' => 'Driftad av <a href="https://unopim.com/" target="_blank">UnoPim</a>, Ett community-projekt från <a href="https://webkul.com/" target="_blank">Webkul</a>',
    ],
    'emails' => [
        'dear'   => 'Kära :admin_name',
        'hello'  => 'Hej,',
        'thanks' => 'Om du behöver någon form av hjälp, vänligen kontakta oss på <a href=":link" style=":style">:email</a>.<br/>Tack!',
        'admin'  => [
            'forgot-password' => [
                'description'    => 'Du får detta e-postmeddelande eftersom vi mottog en begäran om att återställa lösenordet för ditt konto.',
                'greeting'       => 'Glömt Lösenord!',
                'reset-password' => 'Återställ Lösenord',
                'subject'        => 'Återställ Lösenord E-post',
            ],
        ],

        'data-transfer' => [
            'body' => ':type ":code" är :state.',
        ],
    ],
    'common' => [
        'yes'               => 'Ja',
        'no'                => 'Nej',
        'true'              => 'Sann',
        'false'             => 'Falsk',
        'enable'            => 'Aktiverad',
        'disable'           => 'Inaktiverad',
        'no-match-found'    => 'Ingen matchning hittades!',
        'unauthorized'      => 'You do not have permission to perform this action.',
        'file-size-exceeds' => 'The uploaded file exceeds the maximum allowed size of :max.',
    ],
    'default-prompt' => [
        'detailed' => [
            'title'  => 'Detaljerad produktbeskrivning',
            'prompt' => 'Skriv en detaljerad beskrivning av @name, inklusive dess funktioner, fördelar, tekniska specifikationer och användarinstruktioner.',
        ],
        'overview' => [
            'title'  => 'Produktöversikt',
            'prompt' => 'Skapa en detaljerad översikt av @name, som lyfter fram dess viktigaste funktioner, fördelar och målgrupp, som @brand och @color.',
        ],
        'features' => [
            'title'  => 'Produktfunktioner och fördelar',
            'prompt' => 'Lista de viktigaste funktionerna och fördelarna med @name, och förklara hur de tillför värde till kundens liv.',
        ],
        'technical' => [
            'title'  => 'Produktens tekniska specifikationer',
            'prompt' => 'Ge en detaljerad lista över tekniska specifikationer för @name, inklusive mått, material och kompatibilitet.',
        ],
        'care' => [
            'title'  => 'Skötsel och underhåll av produkten',
            'prompt' => 'Skriv en guide om hur du tar hand om och underhåller @name, inklusive tips för rengöring, förvaring och felsökning.',
        ],
        'tagline' => [
            'title'  => 'Produktslogan',
            'prompt' => 'Skriv en catchy och kort slogan för @name som fångar dess essens och fördelar.',
        ],
        'summary' => [
            'title'  => 'Produktsammandrag',
            'prompt' => 'Sammanfatta de viktigaste funktionerna och fördelarna med @name i 50-60 ord.',
        ],
        'headline' => [
            'title'  => 'Produkttitel',
            'prompt' => 'Skapa en uppmärksamhetsväckande titel för @name som lyfter fram dess unika försäljningspunkt.',
        ],
        'brief' => [
            'title'  => 'Kort produktbeskrivning',
            'prompt' => 'Skriv en kort och koncis beskrivning av @name, med fokus på dess viktigaste fördelar och målgrupp.',
        ],
        'elevator' => [
            'title'  => 'Produktpitch',
            'prompt' => 'Skapa en kort pitch för @name, som sammanfattar dess syfte, fördelar och unika försäljningspunkt i 30-40 ord.',
        ],
    ],
];
