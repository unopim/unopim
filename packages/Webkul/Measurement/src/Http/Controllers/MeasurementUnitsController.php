...
class MeasurementUnitsController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $grid = resolve(UnitDataGrid::class);
            $grid->setFamilyId($id);
        ...
        return view('measurement::measurement-families.edit', ['family' => $family, 'locales' => $locales, 'operationOptions' => $operationOptions, 'familyUsedInProducts' => $familyUsedInProducts]);
    }
    ...
    public function editUnit(int $familyId, string $code): JsonResponse
    {
        ...
        abort_unless($unit, 404, trans('measurement::app.messages.unit.units_not_found'));
    ...
    public function updateUnit($familyId, $code)
    {
        ...
        abort_unless($family, 404, trans('measurement::app.messages.family.not_found'));
        ...
        foreach ($units as &$unit) {
            ...
            $unit['symbol'] = request()->has('symbol')
                ? request('symbol')
                : ($unit['symbol'] ?? null);
    ...
    public function deleteUnit($familyId, $code)
    {
        ...
        $updatedUnits = array_filter($units, fn (array $unit) => isset($unit['code']) && $unit['code'] !== $code);
