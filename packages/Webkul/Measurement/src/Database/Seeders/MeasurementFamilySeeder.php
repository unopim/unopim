...
{
    /**
     * Seed the default measurement families and units.
     */
    public function run($parameters = []): void
    {
        $parameters ??= [];
        $defaultLocale = $parameters['default_locale'] ?? config('app.locale');
        $locales = $parameters['allowed_locales'] ?? $this->getActiveLocales($defaultLocale);

        $makeLabels = function ($value) use ($locales): array {
            $key = 'measurement::app.seeder.'.str_replace(' ', '_', $value);
            ...
            foreach ($locales as $locale) {
                ...
                $labels[$locale] = $translated === $key ? ucfirst($value) : $translated;
            }
        ...
        $repository = resolve(MeasurementFamilyRepository::class);

        MeasurementFamily::withoutAuditing(function () use ($repository, $families): void {
    ...
    /**
     * Locales the seeded labels should be written for when the installer does
     * not supply them, so a multi-locale install is not limited to its default.
     */
    protected function getActiveLocales(string $defaultLocale): array
    {
        try {
            $locales = resolve(LocaleRepository::class)->getActiveLocales()->pluck('code')->all();
        } catch (\Throwable) {
            $locales = [];
        }
