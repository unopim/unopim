...
class MeasurementFamilyDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        $queryBuilder = DB::table('measurement_families')
            ->leftJoin('measurement_family_translations as mft_loc', function ($join) use ($locale): void {
                $join->on('mft_loc.measurement_family_id', '=', 'measurement_families.id')
                    ->where('mft_loc.locale', '=', $locale);
            })
            ->leftJoin('measurement_family_translations as mft_en', function ($join): void {
     ...
     */
    protected function getFamilyLabelQuery(string $driver, string $tablePrefix = ''): string
    {
        if ($driver === 'pgsql') {
            return "COALESCE(
     ...
     */
    protected function getStandardUnitLabelQuery(string $driver, string $tablePrefix = ''): string
    {
     ...
     */
    protected function getStandardUnitFilterQuery(string $driver, string $standardUnitLabelQuery, string $tablePrefix = ''): string
    {
        if ($driver === 'pgsql') {
     ...
     */
    protected function getUnitCountQuery(string $tablePrefix = ''): string
    {
        return "(
            SELECT COUNT(*)
    ...
    public function prepareColumns(): void
    {
        $this->addColumn([
            ...
            'sortable'   => true,
            'filterable' => true,
            'closure'    => fn ($row) => $row->family_label,
        ]);
        ...
        $this->addColumn([
            ...
            'sortable'   => false,
            'filterable' => true,
            'closure'    => fn ($row) => $row->standard_unit_label,
        ]);
    ...
    public function prepareActions(): void
    {
        if (bouncer()->hasPermission('catalog.measurements.families.edit')) {
            $this->addAction([
                ...
                'method' => 'GET',
                'url'    => fn ($row) => route('admin.measurement.families.edit', $row->id),
            ]);
        }
        ...
        if (bouncer()->hasPermission('catalog.measurements.families.delete')) {
            $this->addAction([
                ...
                'method' => 'DELETE',
                'url'    => fn ($row) => route('admin.measurement.families.delete', $row->id),
            ]);
        }
    }
