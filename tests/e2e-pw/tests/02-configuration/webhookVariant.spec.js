...
async function createConfigurableProduct(page, sku) {
  ...
  // Select configurable attribute (Brand) when prompted
  await page.locator('p').filter({ hasText: 'Brand' }).locator('span').click();
  await page.getByRole('button', { name: 'Save Product' }).click();
...
async function addVariant(page, variantSku, attributeLabel) {
  ...
  // Fill in the variant SKU
  await page.locator('input[name="sku"]').last().fill(variantSku);

  // Select the attribute option in the variant modal
  // The modal contains a select/multiselect for the super attribute (e.g. Brand)
  const modalSelect = page.locator('.multiselect__tags').last();
  await modalSelect.click();
  await page.locator('.multiselect__content-wrapper').last().waitFor({ state: 'visible', timeout: 5000 });
  await page.getByRole('option', { name: attributeLabel }).first().click();
  await page.keyboard.press('Escape');

  // Click the "Add" button to save the variant
  await page.getByRole('button', { name: 'Add' }).click();

  // Wait for variant to appear in the list (the modal closes automatically)
...
test.describe('Webhook - Variant Product Create Event', () => {
  ...
  test('should create webhook log entries when variants are added to a configurable product', async ({ adminPage }) => {
    ...
    await addVariant(adminPage, variantSku, 'Brand A');
