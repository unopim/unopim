...
test('9.7 - Edit existing platform shows pre-populated fields with fetched models', async ({ adminPage }) => {
  ...
  // First, create a platform to ensure one exists to edit
  const uid = generateUid();
  const uniqueLabel = `E2E Edit Test ${uid}`;
  ...
  await adminPage.getByRole('button', { name: 'Save' }).click();
  await expect(adminPage.locator('#app').getByText(/saved successfully|created successfully/i)).toBeVisible({ timeout: 20000 });

  // Now navigate back and click edit on our newly created platform
  await adminPage.goto(MAGIC_AI_PLATFORM_URL, { waitUntil: 'networkidle' });

  // Click the edit anchor on the row matching our label
  const editLink = adminPage.locator('div', { hasText: uniqueLabel }).locator('a[title*="Edit" i], [title*="Edit" i]').first();
  await expect(editLink).toBeVisible({ timeout: 10000 });
  await editLink.click();

  await expect(adminPage.locator('#app').getByText('Edit AI Platform')).toBeVisible({ timeout: 10000 });
  ...
  // Cleanup: delete the platform we created
  const deleteBtn = adminPage.locator('div', { hasText: uniqueLabel }).locator('[title*="Delete" i]').first();
