...
- Added `php artisan unopim:magic-ai:managed-platform`, an interactive wizard that creates or edits a Magic AI platform with the same fields and validation as the admin form, fetches the provider's models to pick from, and stores the API key encrypted in the database (never in `.env`). It can also run unattended from options, reading the key from standard input with `--key-stdin`. A platform saved as managed carries an `is_managed` flag that only the command can set: while it runs on its stored key it is locked to its own saved provider, endpoint and model list (models can be deselected but not added), and it cannot be deleted. Entering a client's own key lifts the lock and clears the flag, and every other platform stays unrestricted.
