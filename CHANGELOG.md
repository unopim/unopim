...
- Added a managed Magic AI platform, configured through the `MAGIC_AI_MANAGED_*` settings and provisioned with `php artisan unopim:magic-ai:managed-platform`. Its key is limited to the configured models and pinned to its own provider and endpoint; a platform on any other key, including a client's own Concentrate AI key, is unrestricted.
