...
- Added a managed Magic AI platform for hosted installs, provisioned with `php artisan unopim:magic-ai:managed-platform`: the command prompts for the API key and stores it encrypted in the database (never in `.env`), while `MAGIC_AI_MANAGED_*` settings define its provider, endpoint and allowed models. The platform carries an `is_managed` flag that only the command can set; while it runs on its stored key it is limited to those models, pinned to its provider and endpoint, and cannot be deleted. Entering a client's own key lifts the restriction and clears the flag, and every other platform stays unrestricted.
